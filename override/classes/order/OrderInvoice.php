...
class OrderInvoiceCore extends ObjectModel
{
    ...
    public function getProducts($products = false, $selected_products = false, $selected_qty = false)
    {
        ...
        foreach ($products as $row) {
            ...
            // Ajouter un message client uniquement si c'est le dernier produit
            if (end($products) === $row) {
                // Récupérer le message du client et s'assurer qu'il ne contient pas de messages système
                $firstMessage = $order->getFirstMessage();
        
                // Vérification si le message est un message client et non un message de système
                if ($firstMessage && !preg_match("/Action réalisée avec succès|Transaction approuvée|Authentification 3DS|Source de l'IPN|UUID de transaction|Type de l'authentification|Certificat 3DS/", $firstMessage)) {
                    $row['first_order_message'] = nl2br($firstMessage);
                } else {
                    $row['first_order_message'] = ''; // Aucune information à afficher si le message est un message système
                }
            }
            //End Modif 
        
            // Changer la quantité si sélectionnée
            if ($selected_qty) {
            ...
            // Ajouter les informations pour le produit virtuel
            if ($row['download_hash'] && !empty($row['download_hash'])) {
                $row['filename'] = ProductDownload::getFilenameFromIdProduct((int) $row['product_id']);
                // Obtenir le nom d'affichage du fichier
            ...
            // Utiliser les valeurs de order_detail et non du produit
            ...
            $row['ecotax_tax_excl'] = $ecotax; // alias pour cohérence
