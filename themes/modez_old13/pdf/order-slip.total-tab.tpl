...
<table id="total-tab" width="100%">
	...
	{/if}
      ...
      {* INVOKE MINIMUM CART HOOK HOOK RIGHT HERE *}
        {hook h='displayMinimumCartFeeSlip' object=$order_slip}
  ...
  {* ─── Bloc TOTAL HT ───────────────────────────────── *}
	{if $tax_excluded_display}
		{if $minimum_cart_fee_amount > 0}
			{* ─── Affichage ajusté si frais minimum présent ─── *}
			<tr class="bold">
			<td class="grey" width="70%">{l s='Total (Tax Excl.)' d='Shop.Pdf' pdf='true'}</td>
			<td class="white" width="30%">
				- {displayPrice currency=$order->id_currency price=$minimum_cart_fee_amount}
			</td>
			</tr>
		{else}
			{* ─── Affichage classique si pas de frais ───────── *}
			<tr class="bold">
			<td class="grey" width="70%">{l s='Total (Tax Excl.)' d='Shop.Pdf' pdf='true'}</td>
			<td class="white" width="30%">
				{if $total_cart_rule}
	...
	{* ─── Bloc TOTAL TTC ───────────────────────────────── *}
	{if $minimum_cart_fee_amount > 0}
		{* ─── Version avec déduction du frais ───────────── *}
		<tr class="bold">
			<td class="grey" width="70%">{l s='Total (Tax Incl.)' d='Shop.Pdf' pdf='true'}</td>
			<td class="white" width="30%">
				- {displayPrice currency=$order->id_currency price=$minimum_cart_fee_amount}
			</td>
		</tr>
	{else}
	{* ─── Version classique sans frais ──────────────── *}
		<tr class="bold">
			<td class="grey" width="70%">{l s='Total (Tax Incl.)' d='Shop.Pdf' pdf='true'}</td>
			<td class="white" width="30%">
			{if $total_cart_rule}
