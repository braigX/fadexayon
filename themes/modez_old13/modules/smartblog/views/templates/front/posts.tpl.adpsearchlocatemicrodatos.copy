...
{block name='page_content'}
		...
		<div id="content" class="block">
			<div   id="sdsblogArticle" class="blog-post smart-blog-single-post">
				<div class="title_block h1 products-section-title">{$meta_title|escape:'htmlall':'UTF-8'}</div>
				<div class="sdsarticleHeader">
					<span>
						{if $smartshowauthor ==1} {l s='Posted by ' d='Modules.Smartblog.Posts'}
						<span >{if $smartshowauthorstyle != 0}{$firstname} {$lastname}{else}{$lastname} {$firstname}{/if}</span>{/if}
						&nbsp;&nbsp;|&nbsp;&nbsp;<span >{$created|escape:'htmlall':'UTF-8'}</span>
                   		<span >
                       ...
                       &nbsp;&nbsp;|&nbsp;&nbsp;
                       {if $countcomment != ''}{$countcomment|escape:'htmlall':'UTF-8'}{else}{l s='0' d='Modules.Smartblog.Posts'}{/if}
				...
				<div >
					<div class="articleContent">
	                    {if isset($ispost) && !empty($ispost)}
	                   		<a  href="{$smartbloglink->getSmartBlogPostLink($post.id_post,$post.cat_link_rewrite)|escape:'htmlall':'UTF-8'}" title="{$post.meta_title|escape:'htmlall':'UTF-8'}" class="imageFeaturedLink">
	                    {/if}

	                    {if $smartbloglink->getImageLink($post.link_rewrite, $post.id_post, 'single-default') != 'false'}
	                        <img  alt="{$post.meta_title|escape:'htmlall':'UTF-8'}" src="{$smartbloglink->getImageLink($post.link_rewrite, $post.id_post, 'single-default')}" class="imageFeatured">
	                    {/if}
	            	...
	            	{if $tags != ''}
		                <div class="sdstags-update">
		                    <span class="tags"><b>{l s='Tags:' d='Modules.Smartblog.Posts'} </b>
			...
			<ul class="footer_links clearfix">
				{foreach from=$posts_previous item="post"}
        			{if isset($post.id_smart_blog_post)}
						<li>
							<a title="{l s='Previous Post' d='Modules.Smartblog.Posts'}" href="{$smartbloglink->getSmartBlogPostLink($post.id_smart_blog_post,$post.link_rewrite)|escape:'htmlall':'UTF-8'}" class="btn btn-default button button-small"><span> {l s='Previous Post' d='Modules.Smartblog.Posts'}</span></a>
						</li>
					{/if}
				{/foreach}
				{foreach from=$posts_next item="post"}
					{if isset($post.id_smart_blog_post)}
						<li class="pull-right">
							<a title="{l s='Next Post' d='Modules.Smartblog.Posts'}" href="{$smartbloglink->getSmartBlogPostLink($post.id_smart_blog_post,$post.link_rewrite)|escape:'htmlall':'UTF-8'}" class="btn btn-default button button-small"><span>{l s='Next Post' d='Modules.Smartblog.Posts'}  </span></a>
						</li>
					{/if}
				{/foreach}
			</ul>
					...
					<div class="smartblogcomments" id="respond">
						...
						<div id="commentInput">
							<form action="" method="post" id="commentform">
								<table>
									<tbody>
										...
										{else}
											<tr>
												<td>
													<span class="required">*</span> <b>{l s='Name:'  d='Modules.Smartblog.Posts'} </b>
												</td>
												<td>
        									...
        									<tr>
												<td>
													<span class="required">*</span> <b>{l s='E-mail:' d='Modules.Smartblog.Posts'} </b><span class="note">{l s='(Not Published)'  d='Modules.Smartblog.Posts'}</span>
												</td>
												<td>
											...
											<tr>
												<td>&nbsp;&nbsp;&nbsp;<b>{l s='Website:'  d='Modules.Smartblog.Posts'} </b><span class="note"> {l s='(Site url with' d='Modules.Smartblog.Posts'} http://)</span></td>
												<td><input type="text" tabindex="3" value="" name="website" class="form-control grey"></td>
											</tr>
										{/if}
										<tr>
											<td><span class="required">*</span> <b> {l s='Comment:'  d='Modules.Smartblog.Posts'}</b></td>
										...
										{if Configuration::get('smartcaptchaoption') == '1'}
											...
											<tr>
												<td><b>{l s='Type Code' d='Modules.Smartblog.Posts'}</b></td><td><input type="text" tabindex="" value="" name="smartblogcaptcha" class="smartblogcaptcha form-control grey"></td>
											</tr>
										{/if}
									</tbody>
								</table>
								...
								<div class="right">
							        <div class="submit">
										<button type="submit" name="addComment" id="submitComment" class="bbutton btn btn-default button-medium" >{l s='Submit' d='Modules.Smartblog.Posts'}</button>
									</div>
								</div>
    						</form>
						</div>
