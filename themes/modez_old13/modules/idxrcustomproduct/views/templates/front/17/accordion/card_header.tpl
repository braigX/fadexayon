...
<div class="card-header"  style="border-color: {$step.color|escape:'htmlall':'UTF-8'}; background-color: {$step.color|escape:'htmlall':'UTF-8'};">
    <div class="card-header-h5"> 
        <a class="d-block" data-toggle="collapse" data-parent="#component_steps_container" href="#step_title_{$step.id_component|intval}" >
            {if $step.icon_exist}
                <img class="step-img" src="{$modules_dir|escape:'htmlall':'UTF-8'}idxrcustomproduct/img/icon/{$step.id_component|intval}.png" alt="Icône pour le composant"/> 
            {/if}
