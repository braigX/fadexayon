...
<div class="prestawpblock block psv{$psvwd|intval} {if $pswp_block->hook == 'displayLeftColumn' || $pswp_block->hook == 'displayRightColumn'}block-categories{/if}">
	...
	{*<div class="readall-wrp">
		<a {if $pswp_blank}target="_blank"{/if} class="readall btn" href="{if $pswp_enable_posts_page}{$pswp_posts_page_url|escape:'html':'UTF-8'}{else}{$pswp_wp_path|escape:'html':'UTF-8'}{/if}">{l s='Read All' mod='prestawp'}</a>
	</div>*}
</div>