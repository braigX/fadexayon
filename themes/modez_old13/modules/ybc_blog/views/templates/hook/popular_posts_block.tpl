...
*}
{if $posts}
    <div class="block ybc_block_popular {$blog_config.YBC_BLOG_RTL_CLASS|escape:'html':'UTF-8'} {if isset($blog_page) && $blog_page}page_{$blog_page|escape:'html':'UTF-8'}{else}page_blog{/if} {if isset($blog_page) && $blog_page=='home'}{if isset($blog_config.YBC_BLOG_HOME_POST_TYPE) && $blog_config.YBC_BLOG_HOME_POST_TYPE=='default' || count($posts)<=1} ybc_block_default{else} ybc_block_slider{/if}{else}{if isset($blog_config.YBC_BLOG_SIDEBAR_POST_TYPE) && $blog_config.YBC_BLOG_SIDEBAR_POST_TYPE=='default' || count($posts)<=1} ybc_block_default{else} ybc_block_slider{/if}{/if}">
        <h4 class="title_blog title_block">{l s='Popular posts' mod='ybc_blog'}</h4>
        {assign var='product_row' value=$blog_config.YBC_BLOG_HOME_PER_ROW|intval}
        <div class="block_content row">
        <div class="ybc_blog_content_block {if $blog_config.YBC_BLOG_RTL_CLASS == 'ybc_blog_rtl_mode'}owl-rtl {/if}{if count($posts)>1}{if isset($blog_page) && $blog_page=='home' && $blog_config.YBC_BLOG_HOME_POST_TYPE!='default'}blog_type_slider{elseif (!isset($blog_page)||(isset($blog_page) && $blog_page!='home')) && $blog_config.YBC_BLOG_SIDEBAR_POST_TYPE!='default'}blog_type_slider{/if}{/if}">
            {foreach from=$posts item='post'}
                <div class="ybc_blog_content_block_item{if $blog_page=='home'} col-xs-12 col-sm-4 col-lg-{12/$product_row|intval}{/if}">
                    {if $post.thumb}<a class="ybc_item_img" href="{$post.link|escape:'html':'UTF-8'}">
                        <img width="{Configuration::get('YBC_BLOG_IMAGE_BLOG_THUMB_WIDTH'|escape:'html':'UTF-8')}" height="{Configuration::get('YBC_BLOG_IMAGE_BLOG_THUMB_HEIGHT'|escape:'html':'UTF-8')}" src="{$post.thumb|escape:'html':'UTF-8'}" alt="{$post.title|escape:'html':'UTF-8'}" title="{$post.title|escape:'html':'UTF-8'}" /></a>{/if}
                        ...
                        {if $post.categories}
                            <div class="ybc-blog-sidear-post-meta">
                                ...
                                <span class="post-date">
                                    {dateFormat date=$post.datetime_added full=0}
                                </span>
                            </div>
                        {/if}
                            ...
                            <div class="ybc-blog-latest-toolbar">                                         
                                {if $show_views}
                                    <span class="ybc-blog-latest-toolbar-views"><i class="ets_svg">
                                                            ...
                                                            </i> {$post.click_number|intval}
                                        {if $post.click_number!=1}{l s='views' mod='ybc_blog'}{else}{l s='view' mod='ybc_blog'}{/if}</span>
                                {/if} 
                                {if $allowComments && $post.comments_num>0}
                                    <span class="ybc-blog-latest-toolbar-comments"><i class="ets_svg"><svg width="14" height="14" viewBox="0 0 1792 1792" xmlns="http://www.w3.org/2000/svg"><path d="M896 384q-204 0-381.5 69.5t-282 187.5-104.5 255q0 112 71.5 213.5t201.5 175.5l87 50-27 96q-24 91-70 172 152-63 275-171l43-38 57 6q69 8 130 8 204 0 381.5-69.5t282-187.5 104.5-255-104.5-255-282-187.5-381.5-69.5zm896 512q0 174-120 321.5t-326 233-450 85.5q-70 0-145-8-198 175-460 242-49 14-114 22h-5q-15 0-27-10.5t-16-27.5v-1q-3-4-.5-12t2-10 4.5-9.5l6-9 7-8.5 8-9q7-8 31-34.5t34.5-38 31-39.5 32.5-51 27-59 26-76q-157-89-247.5-220t-90.5-281q0-174 120-321.5t326-233 450-85.5 450 85.5 326 233 120 321.5z"/></svg>
                                                                </i> {$post.comments_num|intval}
                                        {if $post.comments_num!=1}{l s='comments' mod='ybc_blog'}{else}{l s='comment' mod='ybc_blog'}{/if}</span>
                                {/if}
                                {if $allow_like}
                                    <span title="{if $post.liked}{l s='Liked' mod='ybc_blog'}{else}{l s='Like this post' mod='ybc_blog'}{/if}" class="ybc-blog-like-span ybc-blog-like-span-{$post.id_post|intval} {if $post.liked}active{/if}"  data-id-post="{$post.id_post|intval}">                        
                                        ...
                                        </i> <span class="ben_{$post.id_post|intval}">{$post.likes|intval}</span>
        ...
        {if $blog_config.YBC_BLOG_DISPLAY_BUTTON_ALL_HOMEPAGE || $blog_page!='home'}
            <div class="blog_view_all_button">
                <a href="{$popular_link|escape:'html':'UTF-8'}" class="view_all_link">{l s='View all popular posts' mod='ybc_blog'}</a>
            </div>
        {/if}
        </div>
        <div class="clear"></div>
