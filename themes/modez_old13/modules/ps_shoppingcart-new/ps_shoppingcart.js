  ...
  $(document).ready(function () {
    prestashop.on(
      ...
      function (event) {
        ...
        var refreshURL = $('.block_cart').data('refresh-url');
        var requestData = {};
        ...
        $.post(refreshURL, requestData).then(function (resp) {
          $('.block_cart').replaceWith($(resp.preview).find('.block_cart'));
          ...
          count_new > 0 ? $('.prod_count').addClass('active') : $('.prod_count').removeClass('active');
            ...
            } else if ($('body').hasClass('add3')) {
                $('.product_add_mini').addClass('show');
                setTimeout(function(){
                    $('.product_add_mini').removeClass('show');
                }, 1100);
