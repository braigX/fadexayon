...
(function () {
      const shopColors = {
        "pannellipolicarbonato.it": "#d1b12a",
        "plexiglasszuschnitt.de": "#ff6a1e",
        "plexiglassmedida.es": "#ff6a1e",

      };

      const hostname = window.location.hostname.replace(/^www\./, "");
      const color = shopColors[hostname];

      if (color) {
        // Injecte un <style> immédiatement sans attendre le DOM
        document.write(
          '<style>#ps_topbanner_wrapper{background-color:' + color + '!important}</style>'
        );
      }
    })();
/************************ */
...
function applyBannerStyles() {
  const wrapper = document.getElementById('ps_topbanner_wrapper');
  const span = document.querySelector('#ps_topbanner_wrapper .bandeau span');
  if (!wrapper || !span) return false;

  if (window.innerWidth <= 768) {
    wrapper.style.minHeight = '70px';
    wrapper.style.maxHeight = '70px';
    wrapper.style.height = '70px';
    wrapper.style.marginBottom = '65px';
...
document.addEventListener('DOMContentLoaded', () => {
  if (!applyBannerStyles()) {
    // If banner not present yet, observe DOM for it
...
/***review button check the varient block exist or not*/
document.addEventListener('DOMContentLoaded', () => {
    moveReviewButton();
    observeReviewBlock();
});
function moveReviewButton() {
    ...
    if (!form) return;
    form.querySelectorAll('.ets-rv-product-comments-additional-info').forEach(el => el.remove());
    const review = document.querySelector('.product-additional-info .ets-rv-product-comments-additional-info');
    if (review) {
        form.insertBefore(review, form.firstChild);
        console.log('Moved review button into form.');
    }
}
function observeReviewBlock() {
    const target = document.querySelector('.product-additional-info');
    if (!target) return;
    ...
    const observer = new MutationObserver((mutations) => {
        mutations.forEach(mutation => {
            mutation.addedNodes.forEach(node => {
                if (
                    node.nodeType === 1 &&  
                    node.matches('.ets-rv-product-comments-additional-info')
                ) {
                    moveReviewButton();  
                }
            });
    ...
    if (window.prestashop && typeof window.prestashop.on === 'function') {
        window.prestashop.on('updatedProduct', moveReviewButton);
    }
}
...
(function() {
    'use strict';
    function attachVoucherHandler() {
        const voucherForm = document.querySelector('.js-cart-voucher form[data-link-action="add-voucher"]');
        if (!voucherForm) return;
        const newForm = voucherForm.cloneNode(true);
        voucherForm.parentNode.replaceChild(newForm, voucherForm);
        newForm.addEventListener('submit', function(e) {
            e.preventDefault();
            const form = this;
            const formData = new FormData(form);
            const submitButton = form.querySelector('button[type="submit"]');
            const errorContainer = document.querySelector('.js-cart-voucher .js-error');
            const errorText = document.querySelector('.js-cart-voucher .js-error-text');
            if (submitButton) {
            ...
            .then(data => {
                if (data.hasError) {
                    if (errorContainer && errorText) {
                        errorText.textContent = data.errors[0] || 'Code invalide';
                        errorContainer.style.display = 'block';
                    }
                    if (submitButton) {
                        submitButton.disabled = false;
                        submitButton.innerHTML = '<span>Ajouter</span>';
                    }
                } else {
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', attachVoucherHandler);
    } else {
        attachVoucherHandler();
    }
    if (window.prestashop) {
        prestashop.on('updateCart', function(event) {
            console.log('Cart updated:', event);
            setTimeout(attachVoucherHandler, 500);
        });
    }
    document.addEventListener('click', function(e) {
        const removeLink = e.target.closest('a[data-link-action="remove-voucher"], .js-remove-voucher, a[href*="deleteDiscount"]');
        if (removeLink) {
            console.log('Voucher remove clicked');
            setTimeout(function() {
                console.log('Reloading after voucher removal...');
                window.location.reload();
            }, 800);
        }
    }, true);
    const observeCart = function() {
        const cartContainer = document.querySelector('.cart-summary, .js-cart, #cart-summary-product-list');
        if (!cartContainer) return;
        const observer = new MutationObserver(function(mutations) {
            const voucherUpdated = mutations.some(function(mutation) {
                return mutation.target.closest('.js-cart-voucher') || 
                       mutation.target.classList?.contains('js-discount') ||
                       mutation.target.closest('.js-discount');
            });
            if (voucherUpdated) {
                console.log('Voucher section updated, re-attaching handler...');
                setTimeout(attachVoucherHandler, 300);
            }
        });
