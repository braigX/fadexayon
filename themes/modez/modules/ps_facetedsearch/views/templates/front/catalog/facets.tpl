...
{if $displayedFacets|count}
  <div id="search_filters">
    {*block name='facets_title'}
      <div class="h4 hidden-sm-down">{l s='Filter By' d='Shop.Theme.Actions'}</div>
    {/block*}
                  ...
                  <label class="facet-label{if $filter.active} active {/if}" for="facet_input_{$_expand_id}_{$filter_key}">
                    {if $facet.multipleSelectionAllowed}
                      <span class="custom-checkbox">
                        ...
                        {else}
                          <span {if !$js_enabled} class="ps-shown-by-js" {/if}>{*<i class="material-icons rtl-no-flip checkbox-checked">&#xE5CA;</i>*} <i aria-hidden="true" class="fas fa-check checkbox-checked rtl-no-flip"></i></span>
                        {/if}
                      </span>
                    {else}
