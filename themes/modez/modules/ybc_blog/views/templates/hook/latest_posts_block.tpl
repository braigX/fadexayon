    ...
    <div class="block ybc_block_latest {$blog_config.YBC_BLOG_RTL_CLASS|escape:'html':'UTF-8'} {if isset($blog_page) && $blog_page}page_{$blog_page|escape:'html':'UTF-8'}{else}page_blog{/if} {if isset($blog_page) && $blog_page=='home'}{if isset($blog_config.YBC_BLOG_HOME_POST_TYPE) && $blog_config.YBC_BLOG_HOME_POST_TYPE=='default' || count($posts)<=1} ybc_block_default{else} ybc_block_slider{/if}{else}{if isset($blog_config.YBC_BLOG_SIDEBAR_POST_TYPE) && $blog_config.YBC_BLOG_SIDEBAR_POST_TYPE=='default' || count($posts)<=1} ybc_block_default{else} ybc_block_slider{/if}{/if}">
       {* <h4 class="title_blog title_block">{l s='Latest posts' mod='ybc_blog'}</h4>*}
        ...
        <div class="block_content row">
            <div class="ybc_blog_content_block {if count($posts)>1}{if isset($blog_page) && $blog_page=='home' && $blog_config.YBC_BLOG_HOME_POST_TYPE!='default'}blog_type_slider{elseif (!isset($blog_page)||(isset($blog_page) && $blog_page!='home')) && $blog_config.YBC_BLOG_SIDEBAR_POST_TYPE!='default'}blog_type_slider{/if}{/if}">
                    ...
                    <div class="ybc_blog_content_block_item{if $blog_page=='home'} col-xs-12 col-sm-4 col-lg-{12/$product_row|intval}{/if}">
                        {if $post.thumb}
                            <a class="ybc_item_img" href="{$post.link|escape:'html':'UTF-8'}">
                                <img width="{Configuration::get('YBC_BLOG_IMAGE_BLOG_THUMB_WIDTH'|escape:'html':'UTF-8')}" height="{Configuration::get('YBC_BLOG_IMAGE_BLOG_THUMB_HEIGHT'|escape:'html':'UTF-8')}" src="{$post.thumb|escape:'html':'UTF-8'}" alt="{$post.title|escape:'html':'UTF-8'}" title="{$post.title|escape:'html':'UTF-8'}" />
                            </a>
                        {/if}
                        <div class="ybc-blog-latest-post-content">
                            ...
                            <div class="ybc-blog-sidear-post-meta">
                                ...
                                {/if}
                                {*<span class="post-date">{dateFormat date=$post.datetime_added full=0}</span>*}
								<span class="post-date">{$post.datetime_added|date_format:"%b %d, %Y"}</span>							
                            </div>
							<h3><a class="ybc_title_block" href="{$post.link|escape:'html':'UTF-8'}">{$post.title|escape:'html':'UTF-8'}</a></h3>
                            ...
                            {if $display_desc}                          
                                {if $post.short_description}
                                    <div class="blog_description">
                                        <p>
                                            {$post.short_description|strip_tags:'UTF-8'|truncate:100:'...'|escape:'html':'UTF-8'}
                                        </p>
                                    </div>
