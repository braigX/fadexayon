        ...
        <div class="form-fields">
          {block name='delivery_options'}
            <div class="delivery-options">
			 ...
			 {foreach from=$delivery_options item=carrier key=carrier_id name=carrierLoop}
  {if $smarty.foreach.carrierLoop.index < 4} 
      ...
      <label for="delivery_option_{$carrier.id}" class="col-sm-11 delivery-option-2">
        <div class="row">
          ...
          <div class="col-sm-3 col-xs-12">
            ...
            {if $carrier.price === 'gratuit'}
             <!-- Ne rien afficher -->
             <span class="hidden">{$carrier.price}</span>
            {else}
             {$carrier.price}
            {/if}
            </span>
