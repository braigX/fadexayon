<?php
/**
 * Builds real page-cache entries by issuing signed front-office requests that
 * bypass cache reads but still store the final optimized HTML.
 */

class PrestaLoadBetaCacheGenerator
{
    ...
    public function generateAll()
    {
        ...
        foreach ($this->getLanguages() as $language) {
            ...
            foreach ($pages as $page) {
                $report['pages'][] = $this->generatePageEntry($page, $language, $variants, $report['summary'], $totalTime);
            }
        }
    ...
    public function generatePage($pageKey, $languageId)
    {
        @set_time_limit(0);

        $language = $this->findLanguageById($languageId);
        if (empty($language)) {
            throw new Exception('Selected language was not found for beta cache generation.');
        }

        $page = $this->findPageForLanguage($pageKey, (int) $language['id_lang']);
        if (empty($page)) {
            throw new Exception('Selected page was not found for beta cache generation.');
        }
        ...
        $variants = $this->buildVariantsForLanguage($language);
        $pageEntry = $this->generatePageEntry($page, $language, $variants, $summary, $totalTime);
    ...
    private function generatePageEntry(array $page, array $language, array $variants, array &$summary, &$totalTime)
    {
        $pageEntry = [
    ...
    private function performRequest($url, array $options)
    {
        ...
        $headers = [
            'X-PrestaLoad-Beta-Generate: 1',
            'X-PrestaLoad-Beta-Token: ' . PrestaLoadInternalAuth::buildBetaGenerateToken(),
            'X-PrestaLoad-Generator: admin-beta-cache',
        ];
        ...
        return [
            'ok' => $statusCode >= 200 && $statusCode < 400 && $this->hasStoredHeader($headerBlock),
            'status_code' => $statusCode,
            'time_ms' => $timeMs,
            'error' => $error,
    ...
    private function hasStoredHeader($headerBlock)
    {
        return stripos((string) $headerBlock, 'X-PrestaLoad-Cache: MISS-STORE') !== false
            || stripos((string) $headerBlock, 'X-PrestaLoad-Cache: HIT') !== false;
    }
    ...
    {
        return $this->modulePath . '/cache/prestaload-beta-cache-generator.json';
    }
