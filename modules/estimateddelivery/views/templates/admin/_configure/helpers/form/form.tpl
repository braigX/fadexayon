...
<form id="{if isset($fields.form.form.id_form)}{$fields.form.form.id_form|escape:'html':'UTF-8'}{else}{if $table == null}configuration_form{else}{$table}_form{/if}{if isset($smarty.capture.table_count) && $smarty.capture.table_count}_{$smarty.capture.table_count|intval}{/if}{/if}" class="defaultForm form-horizontal{if isset($name_controller) && $name_controller} {$name_controller}{/if}"{if isset($current) && $current} action="{$current|escape:'html':'UTF-8'}{if isset($token) && $token}&amp;token={$token|escape:'html':'UTF-8'}{/if}"{/if} method="post" enctype="multipart/form-data"{if isset($style)} style="{$style}"{/if} novalidate>
	...
	{foreach $fields as $f => $fieldset}
		...
		<div class="panel" id="{$f}{* {if isset($smarty.capture.identifier_count) && $smarty.capture.identifier_count}_{$smarty.capture.identifier_count|intval}{/if}{if $smarty.capture.fieldset_name > 1}_{($smarty.capture.fieldset_name - 1)|intval}{/if} *}">
						...
						{else}
							...
							{block name="field"}
									...
									<div class="row">
										...
										<div class="col-lg-3">
											...
											{foreach from=$input.button.attributes key=name item=value}
												...
												{/if}
											{/foreach} >
											{if isset($input.button.icon)}<i class="icon-{$input.button.icon|escape:'htmlall':'UTF-8'}"></i>{/if}{$input.button.label}
											</button>
										</div>
								...
								{elseif $input.type == 'checkbox'}
									...
									{/foreach}
                                {elseif $input.type == 'checklimit'}
                                    {for $w=0 to 6}
                                        <div class="checklimit">
                                            <input type="checkbox" name="{$input.name|escape:'htmlall':'UTF-8'}_{$w|intval}" id="{$input.name|escape:'htmlall':'UTF-8'}_{$w|intval}" class="" {if $fields_value[$input.name][$w] eq '1'}checked="checked"{/if}>
                                            <input type="text" class="time" name="{$input.name2|escape:'htmlall':'UTF-8'}_{$w|intval}" value="{$fields_value[$input.name2][$w]}">
                                            <label for="{$input.name|escape:'htmlall':'UTF-8'}_{$w|intval}">{$weekdays.$w|escape:'htmlall':'UTF-8'}</label>
                                        </div>
                                    {/for}
...
{if $firstCall}
	<script type="text/javascript">
		...
		$(document).ready(function() {
            ...
            if (typeof dniRequired !== 'undefined') {
			    dniRequired();
