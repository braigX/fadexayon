...
$offset = (int)Configuration::get('rc_cms_offset');
$limit = (int)Configuration::get('rc_cms');

if($limit < 1)
	$limit = 1;
  $sql = 'SELECT  id_cms FROM `'._DB_PREFIX_.'cms`  WHERE active = 1 LIMIT '.(int)$limit.' OFFSET '.(int)$offset;	
  $results = Db::getInstance()->executeS($sql);
  $url = '';

if(!$results)
{
	Configuration::updateValue('rc_cms_offset', 0);	
...
if($results)
{
	...
	Configuration::updateValue('rc_cms_offset', $offset + count($results)  );	
