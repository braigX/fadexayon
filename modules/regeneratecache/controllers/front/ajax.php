...
class regeneratecacheajaxModuleFrontController extends ModuleFrontController
{
	public $ssl = true;
	...
	public function initContent()
	{
		...
		if (Tools::getIsset('action'))
		{
			...
			if (method_exists('regeneratecacheajaxModuleFrontController', $function_name))
				$this->$function_name();
		}
	...
	private function get_url_content($url)
	{
	  ...
	  $rc_cache_header = Configuration::get('rc_cache_header');
    ...
    curl_setopt($curl, CURLOPT_USERAGENT, $rc_cache_header); 
	...
	private function ajax_regenerate_products()
	{
	    ...
	    if(!empty(Tools::getValue('rc_id_shop')))
	    	$sql .= ' AND id_shop ='.(int)Tools::getValue('rc_id_shop');
	   
	    $sql .= ' ORDER by id_product DESC  LIMIT 1 OFFSET '.(int)$offset;	
		...
		if($results)
		{
			...
			foreach($langs as $lang)
			{
				...
				if(!empty(Tools::getValue('rc_id_shop')))
					$url =  $link->getProductLink((int)$results['id_product'], null, null, null, (int)$lang['id_lang'], Tools::getValue('rc_id_shop'));
				else
	...
	private function ajax_regenerate_categories()
	{
		...
		$offset = (int)Tools::getValue('offset') - 1;

		$url = '';
	    $sql = 'SELECT  id_category FROM `'._DB_PREFIX_.'category_shop`  ';
	    
	    if(!empty(Tools::getValue('rc_id_shop')))
	    	$sql .= ' WHERE id_shop ='.(int)Tools::getValue('rc_id_shop');
		...
		if($results)
		{
			...
			foreach($langs as $lang)
			{
				...
				if(empty(Tools::getValue('rc_id_shop')))
					...
					$url =  $link->getCategoryLink((int)$results['id_category'], null, (int)$lang['id_lang'], null, Tools::getValue('rc_id_shop'));
...
private function ajax_regenerate_manufacturers()
{
	    ...
	    $sql = 'SELECT  id_manufacturer FROM `'._DB_PREFIX_.'manufacturer_shop`  ';
		...
		if($results)
		{
			...
			foreach($langs as $lang)
			{
				if(empty(Tools::getValue('rc_id_shop')))
					...
					$url = $link->getManufacturerLink((int)$results['id_manufacturer'], NULL, (int)$lang['id_lang'], Tools::getValue('rc_id_shop'));
...
private function ajax_regenerate_suppliers()
{
	    ...
	    $sql = 'SELECT  id_supplier FROM `'._DB_PREFIX_.'supplier_shop`  ';
		...
		if($results)
		{
			...
			foreach($langs as $lang)
			{
				if(empty(Tools::getValue('rc_id_shop')))
					...
					$url = $link->getSupplierLink((int)$results['id_supplier'], NULL, (int)$lang['id_lang'], Tools::getValue('rc_id_shop'));
		...
		if($results)
		{
			...
			foreach($langs as $lang)
			{
				if(empty(Tools::getValue('rc_id_shop')))
					...
					$url = $link->getCMSLink((int)$results['id_cms'], NULL, NULL, (int)$lang['id_lang'], Tools::getValue('rc_id_shop'));
