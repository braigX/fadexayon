...
trait EncodingAutoTrait
{
    ...
    private function convertTwoAndSelectSmallest()
    {
        ...
        $sizedestinationLossless = 0;
        $sizedestinationLossy = 0;

        if(file_exists($destinationLossless))
           $sizedestinationLossless = filesize($destinationLossless);

        if(file_exists($destinationLossy))
           $sizedestinationLossy = filesize($destinationLossy);
            
        if ($sizedestinationLossless > $sizedestinationLossy ) 
        {
