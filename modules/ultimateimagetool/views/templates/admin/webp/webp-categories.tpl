...
*}
<div class="small_panel panel active" id="webp_categories"  >
    <div class="panel-heading"><i class="icon-minus"></i> {l s='Convert category images' mod='ultimateimagetool'}</div>
    <div>
    	{l s='In the table bellow, you will see the number of categories not images, it will convert all of the images of each category.' mod='ultimateimagetool'}
    </div>  
	    ...
	    <tbody id="samdha_warper">
	    	<tr>
	    		...
	    		<td>
		    		...
		    		<select id="image_quality_cat_convert" name="image_quality_cat" rel="product">
	    	...
	    	<tr style="display:none">
	    		...
	    		<td>
		    		<select id="max_parallel_cat_convert">
	    	...
	    	<tr class="cat_type_original_convert">
	    		...
	    		<td>
	    			...
	    			<button class="UltimateImageTool_categoryplayConvert"><span class="ui-button-icon-primary ui-icon ui-icon-play"></span></button>
	    			<button class="UltimateImageTool_categorystopConvert"><span class="ui-button-icon-primary ui-icon ui-icon-stop"></span></button>
	    			<button class="UltimateImageTool_categoryresetConvert"><span class="ui-button-icon-primary ui-icon ui-icon-refresh"></span></button>
				</td>
	    		<td>
	    			<div class="progress ui-progressbar ui-widget ui-widget-content ui-corner-all" ><div class="text"> <span class="category_current">{$original_offset_cat_convert|escape:'htmlall':'UTF-8'}</span> {l s='of' mod='ultimateimagetool'} <span id="categories_max">{$category_count|escape:'htmlall':'UTF-8'}</span></div><div class="{$category_count|escape:'htmlall':'UTF-8'}_progress  ui-progressbar-value ui-widget-header ui-corner-left" style="width: {$original_percent_cat_convert|escape:'htmlall':'UTF-8'}%;"></div></div>
	    		</td>
	    	
	    		
	    	</tr>

	    	{foreach from=$category_sizes_convert item=reg_item}
	    	<tr class="cat_type_{$reg_item.name|escape:'htmlall':'UTF-8'}_convert">
