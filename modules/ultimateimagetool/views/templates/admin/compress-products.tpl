...
*}
<div class=" panel" id="regenerate-products"  >
    <div class="panel-heading"><i class="icon-minus"></i> {l s='Compress product images' mod='ultimateimagetool'}</div>
    <div>
    	{l s='In the table bellow, you will see the number of products not images, it will compress all of the images of each product.' mod='ultimateimagetool'}
    </div>
    ...
    <table class="table">
	    ...
	    <tbody id="samdha_warper">
	    	...
	    	<tr>
	    		...
	    		<td>
		    		<select id="image_quality" name="image_quality" rel="product">
	    	...
	    	<tr class="type_original">
	    		...
	    		<td>
	    			...
	    			<button class="UltimateImageTool_productplay"><span class="ui-button-icon-primary ui-icon ui-icon-play"></span></button>
	    			<button class="UltimateImageTool_productstop"><span class="ui-button-icon-primary ui-icon ui-icon-stop"></span></button>
	    			<button class="UltimateImageTool_productreset"><span class="ui-button-icon-primary ui-icon ui-icon-refresh"></span></button>
				</td>
	    		<td>
	    			<div class="progress ui-progressbar ui-widget ui-widget-content ui-corner-all" ><div class="text"> <span class="category_current">{$original_offset|escape:'htmlall':'UTF-8'}</span> {l s='of' mod='ultimateimagetool'} <span id="categories_max">{$products_count|escape:'htmlall':'UTF-8'}</span></div><div class="{$products_count|escape:'htmlall':'UTF-8'}_progress  ui-progressbar-value ui-widget-header ui-corner-left" style="width: {$original_percent|escape:'htmlall':'UTF-8'}%;"></div></div>
	    		</td>
	    		<td  class="saved_after">{$original_saved_space|escape:'htmlall':'UTF-8'} {l s='Kb' mod='ultimateimagetool'}</td>
	    		
	    	</tr>

	    	{foreach from=$product_sizes item=reg_item}
	    	<tr class="type_{$reg_item.name|escape:'htmlall':'UTF-8'}">
	    		...
	    		<td class="saved_after">{$reg_item.saved_space|escape:'htmlall':'UTF-8'} {l s='Kb' mod='ultimateimagetool'}</td>
	    	</tr>
	    	{/foreach}
	    </tbody>
