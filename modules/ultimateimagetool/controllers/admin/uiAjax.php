...

{
...

    {
...

        {
            ...
            $this->tabAccess = Profile::getProfileAccess(
                ...
                Tab::getIdFromClassName($this->name)
    ...
    private function get_optimized_image_url($img_src, $quality = 70)
    ...
    public static function get_optimized_image($image_path, $quality = 70)

    {
        ...
        $output = new CURLFile($image_path, $mime, $name);
...

        {
             ...
             $o = Tools::jsonDecode( trim($result), true);



   

             if(isset($o->error) || isset($o['error']) || $o == NULL)
...

        {
            ...
            {   
...

                {
                    ...
                    $optimized_image = $this->get_optimized_image($image_path, $image_quality);
...

                                {
...

                                    {
...

                                        {
                                            ...
                                            $file_size = getimagesizefromstring($new_image);

                                            

                                            //echo '<pre>'; print_r($file_size); echo '</pre>';
...

    {
...

        {
            ...
            $webp_image_path = str_replace(array('.jpg','.png'), '.webp', $image_path);
                ...
                $is_imagick = false;
                    ...
                    try {
...

                        {
                            ...
                            $true = \WebPConvert\WebPConvert::convert($image_path, str_replace(array('.jpg','.png'), '.webp', $image_path), array( 'converters' => [ 'imagick', 'imagemagick', 'gd'], 'lossless' => $lossless,  'max-quality' => $image_quality , 'quality' => $image_quality , 'default-quality' => $image_quality ,  'fail' => 'original',  'serve-image' => ['headers' => [  'cache-control' => true,   'vary-accept' => true  ],   'cache-control-header' => 'max-age=2' ],   'convert' => [  'quality' => $image_quality   ]));    
...

                {
                    ...
                    if (isset($optimized_image['error'])) {
                        ...
                        die(Tools::jsonEncode(array(
                                        ...
                                        'error_msg' => $optimized_image['error_long']
...

                            {
...

                                 {
...

                                    {
                                        ...
                                        die(Tools::jsonEncode($optimized_image));
