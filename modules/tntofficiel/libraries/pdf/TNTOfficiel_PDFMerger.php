 ...
 * Output to a file, browser, download, or return as a string.
 ...
 * PDFMerger has all the limitations of the FPDI package - essentially, it cannot import dynamic content
...
{
    private $_files;    //['form.pdf']  ["1,2,4, 5-19"] ['<CONTENT>']
    private $_fpdi;
    ...
    {
        //require_once _PS_MODULE_DIR_.'tntofficiel/libraries/pdf/fpdf/TNTOfficiel_FPDF.php';
     ...
     * @return void
     */
    public function addPDF($filepath, $pages = 'all', $binArgData = null)
    {
        if (file_exists($filepath) || strlen($binArgData) > 0) {
            if (strtolower($pages) != 'all') {
                $pages = $this->_rewritepages($pages);
            }

            $this->_files[] = array($filepath, $pages, $binArgData);
        } else {
    ...
    public function merge($outputmode = 'browser', $outputpath = 'newfile.pdf')
    {
        ...
        foreach ($this->_files as $file) {
            ...
            $count = $fpdi->setSourceFile($filename, $binArgData);
