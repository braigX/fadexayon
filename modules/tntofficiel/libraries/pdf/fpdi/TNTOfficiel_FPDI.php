...
// Check for TNTOfficiel_TCPDF and remap TNTOfficiel_TCPDF to TNTOfficiel_FPDF
...
/**
 * Class TNTOfficiel_FPDI
 *
 * Free PDF Template Injection
 */
class TNTOfficiel_FPDI extends TNTOfficiel_FPDF_TPL
{
     ...
     * Parser-Objects
     *
     * @var TNTOfficiel_fpdi_pdf_parser[]
     */
     ...
     */
    public function setSourceFile($filename, $binArgData = null)
    {
        ...
        if (!isset($this->parsers[$filename])) {
            $this->parsers[$filename] = $this->_getPdfParser($filename, $binArgData);
            $this->setPdfVersion(
     ...
     *
     * @return TNTOfficiel_fpdi_pdf_parser
     */
    protected function _getPdfParser($filename, $binArgData)
    {
        return new TNTOfficiel_fpdi_pdf_parser($filename, $binArgData);
    }
     ...
     * @param bool $adjustPageSize If set to true the current page will be resized to fit the dimensions
     *                                of the template
     *
        ...
        foreach ($this->parsers as $filename => $p) {
            ...
            while (($n = key($this->_objStack[$filename])) !== null) {
                try {
                    ...
                    $nObj = array(TNTOfficiel_pdf_parser::TYPE_OBJECT, TNTOfficiel_pdf_parser::TYPE_NULL);
                }

                $this->_newobj($this->_objStack[$filename][$n][0]);

                if ($nObj[0] == TNTOfficiel_pdf_parser::TYPE_STREAM) {
    ...
    public function _newobj($objId = false, $onlyNewObj = false)
    {
        ...
        //Begin a new object
        if (!$onlyNewObj) {
            $this->offsets[$objId] = is_subclass_of($this, 'TNTOfficiel_TCPDF') ?
    ...
    {
        // Check if the current class is a subclass of 'TNTOfficiel_TCPDF'.
        if (is_subclass_of($this, 'TNTOfficiel_TCPDF')) {
            parent::_prepareValue($value);
        }

        switch ($value[0]) {
            case TNTOfficiel_pdf_parser::TYPE_TOKEN:
                // Direct output of the token followed by a space.
                $this->_straightOut($value[1] . ' ');
                break;
            case TNTOfficiel_pdf_parser::TYPE_NUMERIC:
            case TNTOfficiel_pdf_parser::TYPE_REAL:
                if (is_float($value[1]) && $value[1] != 0) {
                    // Handling floating-point numbers.
                    $this->_straightOut(rtrim(rtrim(sprintf('%F', $value[1]), '0'), '.') . ' ');
                } else {
                    // Direct output for other numeric types.
                    $this->_straightOut($value[1] . ' ');
                }
                break;

            case TNTOfficiel_pdf_parser::TYPE_ARRAY:
            ...
            case TNTOfficiel_pdf_parser::TYPE_DICTIONARY:
                // A dictionary.
                $this->_straightOut('<<');
                foreach ($value[1] as $k => $v) {
                    $this->_straightOut($k . ' ');
                    // Recursive writing of dictionary values.
                    $this->_writeValue($v);
                }
                $this->_straightOut('>>');
                break;

            case TNTOfficiel_pdf_parser::TYPE_OBJREF:
            ...
            case TNTOfficiel_pdf_parser::TYPE_STRING:
                // Enclosing a string within parentheses.
                $this->_straightOut('(' . $value[1] . ')');
                break;

            case TNTOfficiel_pdf_parser::TYPE_STREAM:
            ...
            case TNTOfficiel_pdf_parser::TYPE_HEX:
                // Enclosing a hexadecimal string within chevrons.
                $this->_straightOut('<' . $value[1] . '>');
                break;

            case TNTOfficiel_pdf_parser::TYPE_BOOLEAN:
                // Outputting true or false.
                $this->_straightOut($value[1] ? 'true ' : 'false ');
                break;

            case TNTOfficiel_pdf_parser::TYPE_NULL:
                // Outputting the null value.
        ...
        if (!is_subclass_of($this, 'TNTOfficiel_TCPDF')) {
            ...
            if ($this->state == 2) {
                if ($this->inxobj) {
                    ...
                    and isset($this->footerlen[$this->page])
                    and ($this->footerlen[$this->page] > 0)
                ) {
    ...
    public function cleanUp()
    {
        while (($parser = array_pop($this->parsers)) !== null) {
            /**
             * @var TNTOfficiel_fpdi_pdf_parser $parser
             */
            $parser->closeFile();
        }
    }
