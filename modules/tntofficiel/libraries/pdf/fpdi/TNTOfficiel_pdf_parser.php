...
class TNTOfficiel_pdf_parser
{
    ...
    public function __construct($filename)
    {
        ...
        $this->_c = new TNTOfficiel_pdf_context($this->_f);
     ...
     * Reads a PDF value
     *
     * @param TNTOfficiel_pdf_context $c
    ...
    protected function _unFilterStream($obj)
    {
        ...
        if (isset($obj[1][1]['/Filter'])) {
            ...
            if ($filter[0] == TNTOfficiel_pdf_parser::TYPE_OBJREF) {
                $tmpFilter = $this->resolveObject($filter);
                $filter = $tmpFilter[1];
            }

            if ($filter[0] == TNTOfficiel_pdf_parser::TYPE_TOKEN) {
                $filters[] = $filter;
            } elseif ($filter[0] == TNTOfficiel_pdf_parser::TYPE_ARRAY) {
                $filters = $filter[1];
            }
        }
        ...
        foreach ($filters as $filter) {
            switch ($filter[1]) {
                ...
                case '/Fl':
                    ...
                    break;
                case '/LZWDecode':
                    require_once 'filters/TNTOfficiel_FilterLZW.php';
                    $decoder = new TNTOfficiel_FilterLZW();
                    $stream = $decoder->decode($stream);
                    break;
                case '/ASCII85Decode':
                    require_once 'filters/TNTOfficiel_FilterASCII85.php';
                    $decoder = new TNTOfficiel_FilterASCII85();
                    $stream = $decoder->decode($stream);
                    break;
                case '/ASCIIHexDecode':
                    require_once 'filters/TNTOfficiel_FilterASCIIHexDecode.php';
                    $decoder = new TNTOfficiel_FilterASCIIHexDecode();
                    $stream = $decoder->decode($stream);
                    break;
                case null:
                    break;
