...
class TNTOfficiel_fpdi_pdf_parser extends TNTOfficiel_pdf_parser
{
     ...
     */
    public function __construct($filename, $binArgData = null)
    {
        if ($binArgData === null) {
            ...
            $this->_f = @fopen('php://memory', 'wb+');
            fwrite($this->_f, $binArgData);
            fseek($this->_f, 0);
    ...
    protected function _getPageResources($obj)
    {
        ...
        if (isset($obj[1][1]['/Resources'])) {
            ...
            if (is_array($res)
                ...
                && array_key_exists(1, $res)
                && $res[0] == TNTOfficiel_pdf_parser::TYPE_OBJECT
            ) {
                return $res[1];
            }
    ...
    protected function _getPageContent($contentRef)
    {
        ...
        if ($contentRef[0] == TNTOfficiel_pdf_parser::TYPE_OBJREF) {
            $content = $this->resolveObject($contentRef);
            if ($content[1][0] == TNTOfficiel_pdf_parser::TYPE_ARRAY) {
                $contents = $this->_getPageContent($content[1]);
            } else {
                $contents[] = $content;
            }
        } elseif ($contentRef[0] == TNTOfficiel_pdf_parser::TYPE_ARRAY) {
    ...
    protected function _getPageBox($page, $boxIndex, $k)
    {
        ...
        if (!is_null($box) && $box[0] == TNTOfficiel_pdf_parser::TYPE_OBJREF) {
            $tmp_box = $this->resolveObject($box);
            $box = $tmp_box[1];
        }

        if (!is_null($box) && $box[0] == TNTOfficiel_pdf_parser::TYPE_ARRAY) {
            $b = $box[1];
            return array(
                'x' => $b[0][1] / $k,
                'y' => $b[1][1] / $k,
