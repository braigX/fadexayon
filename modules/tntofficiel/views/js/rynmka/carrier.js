...
function TNTOfficiel_getCheckoutTNTRadio(boolArgChecked) {
    var strDeliveryOptionRadioTNTSelector = jQuery.map(window.TNTOfficiel.carrier.list, function (value, id_carrier) {
            return '.delivery-option input:radio[value^="' + id_carrier + ',"]';
        }).join(', ');

    // Filter to exclude real hidden input (allow masked one with graphics).
    var $objDeliveryOptionRadioTNTList = jQuery(strDeliveryOptionRadioTNTSelector).filter(function (intIndex, element) {
...
function TNTOfficiel_isDeliveryPointValidated() {
    ...
    if ($objDeliveryOptionRadioTNTChecked.length === 1) {
        ...
        if (window.TNTOfficiel.carrier && window.TNTOfficiel.carrier.list[intTNTCarrierIDChecked]) {
...
window.TNTOfficiel_Ready.push(function (jQuery) {
    ...
    // On payment submit.
    jQuery(window.document)
    .off('click.' + window.TNTOfficiel.module.name, '#payment-confirmation :input')
    .on('click.' + window.TNTOfficiel.module.name, '#payment-confirmation :input', TNTOfficiel_XHRcheckPaymentReady);
...
function TNTOfficiel_XHRcheckPaymentReady(objEvent) {
    ...
    if (arrPaymentReadyError.length > 0) {
        ...
        alert(TNTOfficiel_getCodeTranslate(arrPaymentReadyError[0]));
