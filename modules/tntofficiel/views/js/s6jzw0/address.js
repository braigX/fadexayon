        ...
        if (strInputPostCode.length === 5) {
            ...
            if (TNTOfficiel_checkAddressPostcodeCity.cache[strCacheKey] != null) {
                ...
                .done(function (objResponseJSON, strTextStatus, objJqXHR) {
                    ...
                    if (TNTOfficiel_isType(objResponseJSON, 'object') === true) {
...
/**
 * Validate postcode form field.
 *
 * @param boolArgNoSelect true to do not popup city selector to correct the city field.
 *
    ...
    switch (window.TNTOfficiel.link.controller) {
        case 'ordercontroller':
            ...
            if (jQuery(window.strTNTOfficieljQSelectorInputPostCode).length === 0) {
                ...
                jQuery('body#checkout')
                .off('click.' + window.TNTOfficiel.module.name)
                .on('click.' + window.TNTOfficiel.module.name,
                    ':submit[name="confirm-addresses"]',
                    function (objEvent) {
                        var intAddressID = jQuery(':submit[name="confirm-addresses"]')
                        .closest('form').find(':radio[name="id_address_delivery"]:checked')
                        .val() | 0;

                        // If address ID not found, return true (do nothing).
                        if (!(intAddressID > 0)) {
                            return true;
                        }

                        return TNTOfficiel_AddressDeliveryCityDirectValidate(objEvent, {"id_address": intAddressID});
                    }
                );
            }
            ...
            if (TNTOfficiel_isCheckoutTNTCarrierDisplay()) {
                ...
                TNTOfficiel_AddressDeliveryCityDirectValidate(null);
            }
            break;
        case 'adminorderscontroller':
            if (window.TNTOfficiel.order.isDirectAddressCheck) {
                TNTOfficiel_AddressDeliveryCityDirectValidate(null, {"id_order": window.TNTOfficiel.order.intOrderID});
            }
            break;
        default:
            break;
...
function TNTOfficiel_onAddressSubmitValidate(objEvent) {
    ...
    if (intCountryID
        ...
        if (!boolIsPostCodeValid) {
            ...
            jQuery(window.strTNTOfficieljQSelectorInputPostCode).parent()
                .addClass('form-error').removeClass('form-ok');
        }
 ...
 */
function TNTOfficiel_AddressDeliveryCityDirectValidate(objEvent, objData) {
    var objLink = window.TNTOfficiel.link.front;
