 ...
 */
if (\class_exists('JPresta\\Greenlion\\PHP_CodeSniffer_CommentParser_ClassCommentParser', \true) === \false) {
...
class PhOSCo_Sniffs_Commenting_FileCommentSniff implements PHP_CodeSniffer_Sniff
{
     ...
     * @var array
     */
    protected $tags = array('author' => array('required' => \true, 'allow_multiple' => \true, 'order_text' => 'follows @subpackage (if used) or @package'), 'copyright' => array('required' => \false, 'allow_multiple' => \true, 'order_text' => 'follows @author'), 'license' => array('required' => \true, 'allow_multiple' => \false, 'order_text' => 'follows @copyright (if used) or @author'), 'version' => array('required' => \false, 'allow_multiple' => \false, 'order_text' => 'follows @license'), 'see' => array('required' => \false, 'allow_multiple' => \true, 'order_text' => 'follows @link'), 'since' => array('required' => \false, 'allow_multiple' => \false, 'order_text' => 'follows @see (if used) or @link'), 'deprecated' => array('required' => \false, 'allow_multiple' => \false, 'order_text' => 'follows @since (if used) or @see (if used) or @link'));
    /**
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$commentStart]['code'] === \T_DECLARE) {
            $semicolon = $phpcsFile->findNext(\JPresta\Greenlion\T_SEMICOLON, $commentStart + 1);
            $commentStart = $phpcsFile->findNext(\T_WHITESPACE, $semicolon + 1, null, \true);
        }
        // Ignore vim header.
        if ($tokens[$commentStart]['code'] === \T_COMMENT) {
            if (\strstr($tokens[$commentStart]['content'], 'vim:') !== \false) {
                $commentStart = $phpcsFile->findNext(\T_WHITESPACE, $commentStart + 1, null, \true);
            }
        }
        ...
        if ($tokens[$commentStart]['code'] === \T_CLOSE_TAG) {
            ...
            if ($tokens[$commentStart]['code'] === \T_COMMENT) {
                ...
                if ($commentStart === \false || $tokens[$commentStart]['code'] !== \T_DOC_COMMENT) {
                    ...
                    $commentEnd = $phpcsFile->findNext(\T_DOC_COMMENT, $commentStart + 1, null, \true);
                    $commentEnd--;
    ...
    protected function processTags($commentStart, $commentEnd)
    {
        ...
        foreach ($this->tags as $tag => $info) {
            ...
            if ($info['required'] === \true && \in_array($tag, $foundTags) === \false) {
            ...
            $tagName = \ucfirst($tag);
            if ($info['allow_multiple'] === \true) {
                $tagName .= 's';
            }
            $getMethod = 'get' . $tagName;
            $tagElement = $this->commentParser->{$getMethod}();
            if (\is_null($tagElement) === \true || empty($tagElement) === \true) {
                continue;
            }
            ...
            if ($foundIndexes[0] > $orderIndex) {
                ...
                if (\is_array($tagElement) === \true && empty($tagElement) === \false) {
        ...
        foreach ($indentation as $indentInfo) {
            if ($indentInfo['space'] !== 0 && $indentInfo['space'] !== $longestTag + 1) {
                ...
                if ($this->tags[$indentInfo['tag']]['allow_multiple'] === \true) {
