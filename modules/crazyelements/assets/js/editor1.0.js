/*!This is main version of editor.js,elementor - v2.6.2 - 14-07-2019 */
  ...
  function _interopRequireDefault(obj) {
	  ...
	  if (tags.length) {
		...
		getControlValue: function getControlValue() {
		 ...
		 var a= this.elementSettingsModel.get(this.model.get('name'));
			if(this.model.get('name')=='editor' && a===null){
				a="Click edit button to change this text. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut elit tellus, luctus nec ullamcorper mattis, pulvinar dapibus leo."
			}
		return a;
	  ...
	  dir: elementorCommon.config.isRTL ? 'rtl' : 'ltr'
		...
		onReady: function onReady() {
		  ...
		  if (this.ui.select.hasClass((pref + "-" + suff + control).replace('_', '-'))) {
		var item_type = this.ui.select.siblings("#crazy-autocomplte-type").val();
		var auto_values = this.ui.select.siblings("#crazy-autocomplte-values").val();
	  ...
	  results: $.map(data, function(obj) {
		...
		var query = {
	  ajax: true,
	  action: pref + '_ajax_get_items',
	  data: params.term,
	  item_type: item_type,
	  selected_values: auto_values,
		};
		return query
		  }
	  }
	  ...
	  actions: [{
		name: 'ps_copy',
		title: elementor.translate('Cross Copy'),
		shortcut:"",
		callback: this.pscopy.bind(this)
	  }, {
		name: 'ps_paste',
		title: elementor.translate('Cross Paste'),
		shortcut: '',
		callback: this.pspaste.bind(this),
	  }]
		  },{
	  name: 'delete',
	  actions: [{
	  ...
	  elements: [this.model.toJSON({
		...
		pscopy: function pscopy() {
		 
		  var keystore='ps_copy';
		  xdLocalStorage.setItem(keystore, JSON.stringify({
	  type: "copy",
	  elementsType: this.getElementType(),
	  elements: [this.model.toJSON({
		copyHtmlCache: true
		...
		pspaste: function pspaste() {
	  var keystore='ps_copy';
	  var eltype=this.el.dataset.element_type;
	  ...
	  var self=this;
	  xdLocalStorage.getItem(keystore, function(lsdata) {
		 
		  var a = /\.(gif|jpg|jpeg|svg|png)/gi.test(lsdata.value);
		  if(a){
	  jQuery("#elementor-loading").show();
	  jQuery("#elementor-loading").css("background-color", "transparent");
	  elementorCommon.ajax.addRequest('pspaste_image', {
		data: {
		  elements: lsdata.value,
		},
		success: function success(data) {
		   var lsdataval = JSON.parse(data.elements);
	  if (typeof(lsdataval.elementsType)!='undefined') {
		var lsdatavalobj=lsdataval.elements;
	  }else{
		var lsdatavalobj=Array (lsdataval);
	  }
	  lsdatavalobj.forEach(function (item) {
	  ...
	  index++;
	   ...
	   jQuery("#elementor-loading").hide();
		},
		error: function error(data,xhr, ajaxOptions, thrownError) {
		   
		}
	  });
		  }else{
	  var lsdataval = JSON.parse(lsdata.value);
	  if (typeof(lsdataval.elementsType)!='undefined') {
	  ...
	  settings = this.getEditModel().get('settings');
		  ...
		  var extraCSS =  this.getEditModel().get('settings').get('custom_css');
	  ...
	  elementor.helpers.scrollToView(this.$el, 200);
		  ...
		  var eltype = elementor.getElementData(model).elType;
		  
		  if(eltype == 'widget'){
	  var edit = elementor.getElementData(model).editable;
	  if(edit == true){
	  ...
	  InsertTemplateHandler = Marionette.Behavior.extend({
		ui: {
		  ...
		  loginButton: '#elementor-template-library-connect__button'
		},
		events: {
		...
		onInsertButtonClick: function onInsertButtonClick() {
		  ...
		  if ('remote' === args.model.get('source') && !elementor.config.is_connected) {
		jQuery('.elementor-templates-modal .dialog-lightbox-content').html(jQuery('#tmpl-elementor-template-library-connect').html())
		jQuery('#elementor-template-library-connect__button').on('click',function(e){
	  e.preventDefault()
		...
		var eventMethod = window.addEventListener ? "addEventListener" : "attachEvent";
          ...
          eventer(messageEvent,function(e) {
            var ajaxOptions = {
                ...
                success: function success(data) {
            elementor.config.is_connected=true
            jQuery('.elementor-templates-modal__header__logo').click();
            jQuery('#elementor-template-library-header-preview-back').click();
                }
            };
            elementorCommon.ajax.addRequest('library_connect_token', ajaxOptions);
	  ...
	  groups: ['elementor-element']
		...
		onMouseDown: function onMouseDown() {
		  var slug = this.model.get('name').replace('_','-') + '-widget'
	  ...
	  top: '-7',
	  element: this.el,
	  // actionURL: elementor.config.elementPromotionURL.replace('%s', slug),
	  activateUrl: elementor.config.elementActivateURL.replace('%s', slug)
		  });
		}
	  });
	  ...
	  var _promotion = _interopRequireDefault(__webpack_require__(430));
	  ...
	  Autocomplete: __webpack_require__(174),
	  ...
	  if (-1 === location.href.search('CRAZY_TESTS=1')) {
		elementor.start();
	  }
	  ...
	  var state = 'expand' === this.ui.toggleAll.data('elementor-action'),
		  classes = ['ceicon-collapse', 'ceicon-expand'];
	  ...
	  return {
		...
		'pspaste @ui.title': 'onTitlepsPaste',
	  ...
	  jQuery.each(elementor.navigator.indicators, function (indicatorName, indicatorSettings) {
		...
		}).attr('data-section', indicatorSettings.section).html("<i class=\"ceicon-".concat(indicatorSettings.icon, "\"></i>"));
	  ...
	  document.execCommand('insertHTML', false, event.originalEvent.clipboardData.getData('text/plain'));
		  ...
		  key: "onTitlepsPaste",
		  value: function onTitlepsPaste(event) {
	  
	  event.preventDefault();
	  var keystore='ps_copy';
	  xdLocalStorage.getItem(keystore, function(lsdata) {
	  
		var a = /\.(gif|jpg|jpeg|svg|png)/gi.test(lsdata);
		if(a){
		  jQuery("#elementor-loading").show();
		  jQuery("#elementor-loading").css("background-color", "transparent");
		  elementorCommon.ajax.addRequest('pspaste_image', {
	  ...
	  success: function success(data) {
		document.execCommand('insertHTML', false,lsdata);
		jQuery("#elementor-loading").hide();
	  },
	  error: function error(data,xhr, ajaxOptions, thrownError) {
	  ...
	  }, elementor.translate('my_libraries'))
	   );
		  }
		}, {
		  key: "getSearchHTML",
	  ...
	  */
	  ...
	  if (libraryConfig.enqueue && typeof(libraryConfig.enqueue)!='undefined') {
		if(Array.isArray(libraryConfig.enqueue)){
		}else{
		  var a=new Array();
		  a.push(libraryConfig.enqueue)
		  libraryConfig.enqueue=a;
		}
	  ...
	  if(Array.isArray(iconSetting.enqueue)){
	  
		}else{
		  var a=new Array();
		  a.push(iconSetting.enqueue)
		 iconSetting.enqueue=a;
	  ...
	  success: function success() {
		NProgress.done();
		elementor.channels.editor.trigger('status:change');
	  }
		  });
		};
	  ...
	  self.setFilter('type', type, true);
	   self.setFilter('subtype', type.subtype, true)
		  }
	  ...
	  module.exports = Marionette.ItemView.extend({
		...
		onSyncClick: function onSyncClick() {
		  var self = this;
		  self.ui.sync.addClass('ceicon-animation-spin');
		  elementor.templates.requestLibraryData({
	  onUpdate: function onUpdate() {
		self.ui.sync.removeClass('ceicon-animation-spin');
	  ...
	  onConfirm: function onConfirm() {
		toggleMoreIcon.removeClass('ceicon-ellipsis-h').addClass('ceicon-loading ceicon-animation-spin');
	  },
	  ...
	  index++;
	   ...
	   var keystore='ps_copy';
		xdLocalStorage.getItem(keystore, function(lsdata) {
		  var lsdataval = JSON.parse(lsdata.value);
		  if (lsdataval.elements.length >0) {
	  var lsdatavalobj=lsdataval.elements;
		  }else{
		  var lsdatavalobj=Array (lsdataval);
		  }
	  ...
	  lastEditedWrapper: '.elementor-last-edited-wrapper'
		...
		initialize: function initialize() {
		  ...
		  elementor.settings.page.model.on('change', this.onPageSettingsChange.bind(this)); // lets_have_a_look
		...
		onRender: function onRender() {
		  this.setMenuItems(elementor.settings.page.model.get('post_status')); //lets_have_a_look
		  this.addTooltip();
		},
	  ...
	  ControlMediaItemView = ControlBaseDataView.extend({
		ui: function ui() {
		  ...
		  ui.clearGallerysingle='.elementor-control-gallery-clear-single';
		  ui.images = [];
		  return ui;
		},
		events: function events() {
	  ...
	  'click @ui.clearGallerysingle': 'onClearGalleryClickSingle',
	  ...
	  return;
		...
		initFrame: function initFrame(action) {
		  var $this=this;
		  var $imginputid="elementor-control-gallery__preview__input_single_"+this.model.cid
		  var $imginputid="elementor-control-gallery__preview__input_"+this.model.cid
		  var target= baseAdminDir+'filemanager/dialog.php?type=1';
		  ...
		  $.fancybox.open({
			  ...
			  closeClick : true,
			  onUpdate: function(e) {
				  $('.fancybox-iframe').contents().find('a.link').attr('data-field_id', $imginputid);
			  },
			  afterShow: function(e) {
				  $('.fancybox-iframe').contents().find('a.link').attr('data-field_id', $imginputid);
			  },
			  afterClose: function (e) {
				  var imgurl = $('#' + $imginputid).val();
				  var images=[];
				  var a=$this.getControlValue();
				  if(a!='' && $this.ui.images.length==0){
				  ...
				  $this.ui.images.each(function (image) {
				  var a=parseInt(image)
					  if($this.ui.images[image] && !isNaN(a)){
						  images.push({
							  url: $this.ui.images[image]['url']
						  });
					  }
				  });
	  ...
	  return removeDialog;
		...
		onClearGalleryClickSingle: function onClearGalleryClickSingle(){
		  $( "#images_gallary_dialog" ).html('')
		  var $this=this;
		  var a=$this.getControlValue();
		  if(a!='' && $this.ui.images.length==0){
		  a.forEach(function (val, index) {
		  ...
		  var a=parseInt(image)
		  if($this.ui.images[image] && !isNaN(a)){
			  $( "#images_gallary_dialog" ).append("<div class='images_gallary_dialog_img img_index_"+a+"'><img src='"+$this.ui.images[image]['url']+"'><div class='images_gallary_dialog_img_x' data-target='img_index_"+a+"' data-key='"+a+"'>X</div></div>")
		  }
		  });
		$(document).on('click','.images_gallary_dialog_img_x',function(){
		  var targetindex=$(this).data('target');
		  $('.'+targetindex).replaceWith('')
		  var indexkey=$(this).data('key');
		  if($this.ui.images[indexkey]){
			  delete $this.ui.images[indexkey]; 
			  var images = [];
			  $this.ui.images.each(function (image) {
				  var a=parseInt(image)
				  if($this.ui.images[image] && !isNaN(a)){
	  ...
	  // }
	  var $this=this;
		  var $imginputid="elementor-control-media__preview__icon__"+this.model.cid
		  var target= baseAdminDir+'filemanager/dialog.php?type=1';
	  ...
	  this.ui.mediaVideo.attr('src', url);
		...
		openFrame: function openFrame() {
		  var $this=this;
		  var $imginputid="elementor-control-media__preview__input_"+this.model.cid
		  var target= baseAdminDir+'filemanager/dialog.php?type=1';
	  ...
	  */
	  ...
	  'click @ui.popoverToggle': 'onPopoverToggleClick'
		...
		onPopoverToggleClick: function onPopoverToggleClick() {
		  $('.elementor-controls-popover').not(this.$el.next('.elementor-controls-popover')).hide();
	  ...
	  to.splice(afterButtonIndex + 1, 0, button);
		...
		rearrangeButtons: function rearrangeButtons() {
		  ...
		  jQuery(document).on('click','.mce_image_upload_action',function(){
			  var target= baseAdminDir+'filemanager/dialog.php?type=1';
			  var $imginputid=jQuery(this).parent().prev('input').attr('id')
			  $.fancybox.open({
				  href: target,
				  fitToView: false,
				  maxWidth  : 900,
	  ...
	  return;
		...
		if(this.getEditingSettingKey()=='editor' && contentHTML===null){
			contentHTML="Click edit button to change this text. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut elit tellus, luctus nec ullamcorper mattis, pulvinar dapibus leo."
		}
	  ...
	  toolbarIconsPrefix: 'ceicon-editor-',
	  toolbarIconsDictionary: {
		externalLink: {
		  className: 'ceicon-editor-external-link'
		},
		list: {
		  className: 'ceicon-editor-list-ul'
		},
		insertOrderedList: {
		  className: 'ceicon-editor-list-ol'
		},
		insertUnorderedList: {
		  className: 'ceicon-editor-list-ul'
		},
		createlink: {
		  className: 'ceicon-editor-link'
		},
		unlink: {
		  className: 'ceicon-editor-unlink'
		},
		blockquote: {
		  className: 'ceicon-editor-quote'
		},
		p: {
		  className: 'ceicon-editor-paragraph'
		},
		pre: {
		  className: 'ceicon-editor-code'
		}
	  }
		  });
	  ...
	  PanelMenuPageView = Marionette.CompositeView.extend({
		...
		getArrowClass: function getArrowClass() {
		  return 'ceicon-arrow-' + (elementorCommon.config.isRTL ? 'right' : 'left');
		},
		onRender: function onRender() {
		  elementor.getPanelView().getHeaderView().ui.menuIcon.removeClass('ceicon-menu-bar').addClass(this.getArrowClass());
		},
		onDestroy: function onDestroy() {
		  elementor.getPanelView().getHeaderView().ui.menuIcon.removeClass(this.getArrowClass()).addClass('ceicon-menu-bar');
		}
	  }, {
		groups: null,
	  ...
	  menus = [{
		...
		items: [{
		  ...
		  icon: 'ceicon-editor-external-link',
	  ...
	  module.exports = Marionette.ItemView.extend({
		...
		ui: {
		  ...
		  langImporter: '.crazy-lang-content-importer'
		},
		events: {
		  ...
		  'click @ui.langImporter': 'onLangContentImporter'
		},
		behaviors: function behaviors() {
		  var behaviors = {
	  saver: {
	  ...
	  $tool.addClass('elementor-open');
		...
		currentDeviceMode = elementor.channels.deviceMode.request('currentMode');
		  ...
		  this.ui.deviceModeIcon.removeClass('ceicon-device-' + previousDeviceMode).addClass('ceicon-device-' + currentDeviceMode);
		},
		...
		onLangContentImporter: function onLangContentImporter(event) {
			var importLang = this.$(event.target);
			var importLangId = importLang.data('langid');
			this.importLangContent(importLangId)
			console.log(importLangId)
		},
		importLangContent: function importLangContent(langId) {
			var options = {
					data: {
						langid: langId,
						source: 'local'
					...
					success: function success(successData) {
						...
						elementor.getPreviewView().addChildModel(successData.content, {});
					},
					error: function error(errorData) {
			...
			elementorCommon.ajax.addRequest('import_lang_content', options);
		},
	  ...
	  transferData.elements.forEach(function (element) {
		...
		pspaste: function pspaste(atIndex) {
	  ...
	  this.dialog.addButton({
		name: 'activate',
		text: elementor.translate('activate_now_bt'),
		callback: function callback() {
		  open(_this2.activeUrl, '_blank');
		}
	  });
	  // this.dialog.getElements('action').addClass('elementor-button elementor-button-success crazy-element-prodect-btn');
	  this.dialog.getElements('activate').addClass('elementor-button elementor-button-success crazy-element-prodect-btn');
	  ...
	  this.dialog.getElements('header').append($promotionTitle, '', $closeButton);
	  ...
	  this.activeUrl = options.activateUrl;
