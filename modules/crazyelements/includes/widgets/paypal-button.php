...
require_once CRAZY_PATH . 'includes/classes/payment-button.php';

if (!defined('_PS_VERSION_')) {
    exit; // Exit if accessed directly.
}

class Widget_Paypal_Button extends Widget_Base {
	...
	public function get_title() {
		return PrestaHelper::__( 'PayPal Button', 'elementor' );
	}

	public function get_icon() {
		return 'ceicon-paypal-button';
	}
	...
	protected function render_legacy_form() {
		...
		<form action="<?php echo esc_attr( $form_action ); ?>" method="post" target="<?php echo esc_attr( $target ); ?>">
			<input type="hidden" name="cmd" value="<?php echo PrestaHelper::esc_attr( $cmd ); ?>" />
			<input type="hidden" name="business" value="<?php echo PrestaHelper::esc_attr( $email ); ?>" />
			<input type="hidden" name="lc" value="US" />
			<input type="hidden" name="item_name" value="<?php echo PrestaHelper::esc_attr( $settings['product_name'] ); ?>" />
			<input type="hidden" name="item_number" value="<?php echo PrestaHelper::esc_attr( $settings['product_sku'] ); ?>" />
			<input type="hidden" name="currency_code" value="<?php echo PrestaHelper::esc_attr( $settings['currency'] ); ?>" />
			<input type="hidden" name="<?php echo PrestaHelper::esc_attr( $price_field['name'] ); ?>" value="<?php echo PrestaHelper::esc_attr( $price_field['value'] ); ?>" />
			...
			foreach ( $this->get_errors() as $type => $message ) {
				...
				<div class="elementor-message elementor-message-danger elementor-hidden elementor-error-message-<?php Utils::print_unescaped_internal_string( $type ); ?>">
					<?php echo PrestaHelper::esc_html( $message ); ?>
				</div>
				<?php
			}
			?>
	...
	protected function register_account_section() {
		$this->start_controls_section(
			'section_account',
			[
				'label' => PrestaHelper::__( 'Pricing & Payments', 'elementor' ),
			]
		);
		...
		$this->add_control(
			'merchant_account',
			[
				'label' => PrestaHelper::__( 'Merchant Account', 'elementor' ),
				'type' => Controls_Manager::HIDDEN,
				'default' => self::API_TYPE_SIMPLE,
				'options' => [
					self::API_TYPE_SIMPLE => PrestaHelper::__( 'Default (Simple)', 'elementor' ),
					self::API_TYPE_ADVANCED => PrestaHelper::__( 'Custom (Advanced)', 'elementor' ),
				],
		...
		$this->add_control(
			'email',
			[
				'label' => PrestaHelper::__( 'PayPal Account', 'elementor' ),
				'type' => Controls_Manager::TEXT,
				'dynamic' => [
					'active' => true,
				],
				'description' => PrestaHelper::__( 'Transactions made through your PayPal button will be registered under this account.', 'elementor' ),
				'label_block' => true,
				'condition' => [
		...
		$this->add_control(
			'sdk_token',
			[
				'label' => PrestaHelper::__( 'SDK Token', 'elementor' ),
				'type' => Controls_Manager::TEXT,
				'dynamic' => [
					'active' => true,
				],
	...
	protected function register_sandbox_controls() {
		$this->add_control(
			'sandbox_email',
			[
				'label' => PrestaHelper::__( 'Sandbox Email Account', 'elementor' ),
				'type' => Controls_Manager::TEXT,
				'dynamic' => [
					'active' => true,
				],
				'description' => PrestaHelper::__( 'This is the address given to you by PayPal when you set up a sandbox with your developer account. You can use the sandbox to test your purchase flow.', 'elementor' ),
				'label_block' => true,
				'condition' => [
					'sandbox_mode' => 'yes',
				],
