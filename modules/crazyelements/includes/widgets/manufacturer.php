...
class Widget_Manufacturer extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$control = array(
			'id_manufacturer',
			array(
				'label'     => PrestaHelper::__( 'Select Manufacturer', 'elementor' ),
	...
	protected function render() {
		...
		$this->generate_controls_data($settings, 'id_manufacturer');
		...
		// where query params
		$id_manufacturer =  $this->render_autocomplete_result($this->ids);

		$products = \Manufacturer::getProducts( $id_manufacturer, $id_lang, 1, $limit, $orderby, $order );
		...
		if ( $this->layout == 'default' ) {
			...
			$this->show_default_skin($products_for_template, 'products-by-manufacturer', $display_type);
		} else {
			$from_cat_addon = false;
			$this->show_crazy_skins($products_for_template, 'products-by-manufacturer');
		} 
	}
