...
{elseif isset($option.product_list)}
    ...
    {/if}
    <div class="ets_abancart_product_list{$idProductList|escape:'html':'UTF-8'}"{if isset($option.styles) && $option.styles} style="{foreach from=$option.styles key='attr' item='value'}{$attr|escape:'html':'UTF-8'}: {$value|escape:'html':'UTF-8'};{/foreach}"{/if}>
