...
class EtsAbancartCampaign extends ObjectModel
{
    ...
    {
        if ($this->id) {
            $cache_id = 'EtsAbancartCampaign::getCustomerGroups_' . md5($this->id . '|' . ($ids !== true && $ids ? $ids : (int)$ids));
    ...
    public static function getCampaignsFrontEnd($context = null, $exclude_cookie = true)
    {
        ...
        if ($idCustomer) {
            ...
            $dq
                ...
                ->where('IF(ac.is_all_country != 1, a.id_country > 0 AND a.id_country=cc.id_country OR cc.id_country = -1 OR ' . (int)$id_address . '<1 AND cc.id_country=' . (int)$context->country->id . ', 1)');
        } else {
            $dq
                ->where('acg.id_group = ' . (int)$group->id)
                ->where('ac.is_all_country = 1 OR cc.id_country = -1 OR cc.id_country=' . (int)$context->country->id);
        }
    ...
    public static function isSame($type, $obj, $id_object = 0, $context = null)
    {
        if ($context == null)
            $context = Context::getContext();
        $sql = '
