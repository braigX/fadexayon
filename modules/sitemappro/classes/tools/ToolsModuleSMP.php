...
class ToolsModuleSMP
{
     ...
     * @return array
     */
    public static function getLanguageIds($active = true)
    {
        $languages = self::getLanguages($active);
        $ids = array();
        foreach ($languages as $language) {
            $ids[] = (int)$language['id_lang'];
        }
        return $ids;
    }

    public static function createTab($module_name, $class_name, $parent, $name)
    {
        if (!is_array($name)) {
            $name = array('en' => $name);
            ...
            if (array_key_exists($field, $definition_fields)) {
                ...
                if (array_key_exists('lang', $definition_fields[$field]) && $definition_fields[$field]['lang']) {
                    foreach ($languages as $lang) {
                        ...
                        if (!empty($object_field[$lang['id_lang']]) && !forward_static_call_array(
    ...
    {
        if (Tools::file_exists_cache(_PS_THEME_DIR_.'modules/'
     ...
     * @return string
     */
    public static function fetchTemplate($path)
    {
    ...
    public static function registerSmartyFunctions()
    {
        ...
        if (!array_key_exists('get_image_lang_map', $smarty->registered_plugins['function'])) {
            smartyRegisterFunction($smarty, 'function', 'get_image_lang_map', array(__CLASS__, 'getImageLangMap'));
        }
        if (class_exists('TransModSMP')) {
            if (!array_key_exists('ld', $smarty->registered_plugins['modifier'])) {
                smartyRegisterFunction($smarty, 'modifier', 'ld', array(TransModSMP::getInstance(), 'ld'));
            }
        }
    }
    ...
    public static function getImageLangMap($smarty)
    {
        $path = $smarty['path'];
        ...
        if (function_exists('error_get_last')) {
            ...
            if ($error && $error['type'] === E_ERROR) {
                ...
                if (preg_match($memory_regex, $message, $matches)) {
                    ...
                    $message .= ')';
                    LoggerSMP::getInstance()->error($message);
                    LoggerSMP::getInstance()->error(
                ...
                die(json_encode(array(
                    'hasError' => LoggerSMP::getInstance()->hasError(),
                    'log' => LoggerSMP::getInstance()->getMessages()
                )));
            }
        }
        exit;
    ...
    public static function createAjaxApiCall($class)
    {
        ...
        if (method_exists($class, $call_method)) {
            try {
                ...
                LoggerSMP::getInstance()->exception($e);
                die(json_encode(array(
                    'hasError' => LoggerSMP::getInstance()->hasError(),
                    'log' => LoggerSMP::getInstance()->getMessages()
                )));
    ...
    public static function addCSS($css_uri, $css_media_type = 'all', $offset = null, $check_path = true)
    {
        Context::getContext()->controller->addCSS(
            _MODULE_DIR_.self::getModNameForPath(__FILE__)
            .'/views/css/'.$css_uri,
            $css_media_type,
            $offset,
            $check_path
    ...
    public static function properParseStr(
        $queryString,
        $argSeparator = '&',
        $decType = PHP_QUERY_RFC1738
    ) {
        $result = array();
        $parts = explode($argSeparator, $queryString);

        foreach ($parts as $key => $item) {
            if (isset($parts[$key+1]) &&  Tools::substr($item, 0, 10) == 'user_links'
                &&  Tools::substr($parts[$key+1], 0, 10) !== 'user_links') {
                $parts[$key] = $parts[$key] . '&' . $parts[$key+1];
                unset($parts[$key+1]);
            }
        }
        ...
        foreach ($parts as $part) {
            ...
            if (preg_match_all('/\[([^\]]*)\]/m', $paramName, $matches)) {
                $paramName = Tools::substr($paramName, 0, strpos($paramName, '['));
    ...
    {
        return (
            defined('STDIN') || (Tools::strtolower(php_sapi_name()) == 'cli'
