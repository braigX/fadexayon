...
`$request` is an object of `TeamwantRedis\Psr\Http\Message\RequestInterface` and

`$response` is an object implementing `TeamwantRedis\Psr\Http\Message\RequestInterface`
