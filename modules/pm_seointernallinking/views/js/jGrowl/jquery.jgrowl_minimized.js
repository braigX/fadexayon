(function(jQuery) {
	jQuery.jGrowl = function(m, o) {
		if (jQuery('#jGrowl').size() == 0)
			jQuery('<div id="jGrowl"></div>').addClass(
					(o && o.position) ? o.position
							: jQuery.jGrowl.defaults.position).appendTo('body');
		...
		if (jQuery.isFunction(this.each)) {
			...
			return this.each(function() {
				...
				if (jQuery(this).data('jGrowl.instance') == undefined) {
					jQuery(this).data('jGrowl.instance',
							jQuery.extend(new jQuery.fn.jGrowl(), {
								notifications : [],
								element : null,
								interval : null
							}));
				...
				if (jQuery.isFunction(jQuery(this).data('jGrowl.instance')[m])) {
					jQuery(this).data('jGrowl.instance')[m].apply(jQuery(this)
							.data('jGrowl.instance'), jQuery.makeArray(args)
							.slice(1));
				} else {
			...
			.extend(
					jQuery.fn.jGrowl.prototype,
					{
						...
						render : function(notification) {
							...
							if (o.corners != '' && jQuery.fn.corner != undefined)
								jQuery(notification).corner(o.corners);
							if (jQuery('div.jGrowl-notification:parent',
									self.element).size() > 1
									&& jQuery('div.jGrowl-closer', self.element)
											.size() == 0
									&& this.defaults.closer != false) {
								jQuery(this.defaults.closerTemplate)
										.addClass(
						...
						update : function() {
							jQuery(this.element)
									...
									.each(
											function() {
												if (jQuery(this).data("jGrowl") != undefined
														&& jQuery(this).data(
																"jGrowl").created != undefined
														&& (jQuery(this).data(
																"jGrowl").created
																.getTime() + parseInt(jQuery(
																this).data(
						...
						startup : function(e) {
							this.element = jQuery(e).addClass('jGrowl').append(
						...
						shutdown : function() {
							jQuery(this.element).removeClass('jGrowl').find(
									'div.jGrowl-notification').remove();
							clearInterval(this.interval);
						},
						close : function() {
							jQuery(this.element).find('div.jGrowl-notification')
									.each(
											function() {
												jQuery(this).trigger(
														'jGrowl.beforeClose');
