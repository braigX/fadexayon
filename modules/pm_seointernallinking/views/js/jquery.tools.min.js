...
(function(jQuery) {	
	...
	var typeRe = /\[type=([a-z]+)\]/, 
		numRe = /^-?[0-9]*(\.[0-9]+)?$/,
		dateInput = jQuery.tools.dateinput,
	...
	v = jQuery.tools.validator = {
		...
		localizeFn: function(key, messages) {
			v.messages[key] = v.messages[key] || {};
			jQuery.extend(v.messages[key], messages);
		},
	...
	var fns = [], effects = {
		...
		'default' : [
			...
			function(errs) {
				...
				// loop errors
				jQuery.each(errs, function(i, err) {
					...
					if (!msg) { 
						msg = jQuery(conf.message).addClass(conf.messageClass).appendTo(document.body);
					...
					// populate messages
					jQuery.each(err.messages, function(i, m) { 
						jQuery("<p/>").html(m).appendTo(msg);			
					});
			...
			// hide errors function
				...
				inputs.removeClass(conf.errorClass).each(function() {
					var msg = jQuery(this).data("msg.el");
	...
	/* sperial selectors */
	jQuery.each("email,url,number".split(","), function(i, key) {
	...
	/* 
		...
		Usage: jQuery("input:eq(2)").oninvalid(function() { ... });
	*/
	jQuery.fn.oninvalid = function( fn ){
    ...
    form.attr("novalidate", "novalidate");
		...
		function pushMessage(to, matcher, returnValue) {
			...
			if (returnValue === false || jQuery.isArray(returnValue)) {
				...
				if (matches && jQuery.isArray(returnValue)) {
		...
		jQuery.extend(self, {
			...
			invalidate: function(errs, e) {
				...
				if (!e) {
					var errors = [];
					jQuery.each(errs, function(key, val) {
...
//{{{  checkValidity() - flesh and bone of this tool
			...
			checkValidity: function(els, e) {
				...
				els.not(":radio:not(:checked)").each(function() {
					...
					var msgs = [], 
						 el = jQuery(this).data("messages", msgs),
...
//}}} 
		...
		// callbacks	
		jQuery.each("onBeforeValidate,onBeforeFail,onFail,onSuccess".split(","), function(i, name) {
			...
			self[name] = function(fn) {
				if (fn) { jQuery(self).bind(name, fn); }
				return self;
			};
		});	
		...
		if (conf.inputEvent) {
			inputs.bind(conf.inputEvent + ".V", function(e) {
				self.checkValidity(jQuery(this), e);
			});	
		} 
		...
		inputs.filter(":checkbox, select").filter("[required]").bind("change.V", function(e) {
			var el = jQuery(this);
			if (this.checked || (el.is("select") && jQuery(this).val())) {
	...
	jQuery.fn.validator = function(conf) { 
		...
		// configuration
		conf = jQuery.extend(true, {}, v.conf, conf);		
...
':url' 			: 'URL invalide',
'[max]'	 		: 'La valeur doit &ecirc;tre inf&eacute;rieure à $1',
'[min]'	 		: 'La valeur doit &ecirc;tre sup&eacute;rieure à $1',
