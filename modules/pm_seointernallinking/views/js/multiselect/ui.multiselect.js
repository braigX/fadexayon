...
// objectKeys return
jQuery.extend({ objectKeys: function(obj){ if (typeof Object.keys == 'function') return Object.keys(obj); var a = []; jQuery.each(obj, function(k){ a.push(k) }); return a; }});
...
var defaultDataParser = function(data) {
	if ( typeof data == 'string' ) {
		...
		this._messages(jQuery.ui.multiselect.constante.MESSAGE_ERROR, $.ui.multiselect.locale.errorDataFormat);
		data = false;
	}
	return data;
};
  ...
  options: {
	...
	_create: function() {
		...
		if (this.options.locale != undefined) {
			jQuery.ui.multiselect.locale = this.options.locale;
		} else {
			jQuery.ui.multiselect.locale = {
				addAll:'Add all',
		...
		this.selectedContainer = jQuery('<div class="ui-widget-content list-container selected"></div>').appendTo(this.container);
		this.availableContainer = jQuery('<div class="ui-widget-content list-container available"></div>').appendTo(this.container);
		this.selectedActions = jQuery('<div class="actions ui-widget-header ui-helper-clearfix"><span class="count">'+jQuery.tmpl(jQuery.ui.multiselect.locale.itemsCount,{count:0})+'</span><a href="#" class="remove-all">'+jQuery.tmpl(jQuery.ui.multiselect.locale.removeAll)+'</a></div>').appendTo(this.selectedContainer);
		this.availableActions = jQuery('<div class="actions ui-widget-header ui-helper-clearfix"><span class="busy">'+jQuery.tmpl(jQuery.ui.multiselect.locale.busy)+'</span><input type="text" class="search ui-widget-content ui-corner-all"  value="'+jQuery.tmpl(jQuery.ui.multiselect.locale.sInputSearch)+'" onfocus="javascript:if(this.value==\''+jQuery.tmpl(jQuery.ui.multiselect.locale.sInputSearch)+'\')this.value=\'\';" onblur="javascript:if(this.value==\'\')this.value=\''+jQuery.tmpl(jQuery.ui.multiselect.locale.sInputSearch)+'\';" /><a href="#" class="add-all">'+jQuery.tmpl(jQuery.ui.multiselect.locale.addAll)+'</a></div>').appendTo(this.availableContainer);
		this.selectedList = jQuery('<ul class="list selected"><li class="ui-helper-hidden-accessible"></li></ul>').bind('selectstart', function(){return false;}).appendTo(this.selectedContainer);
		this.availableList = jQuery('<ul class="list available"><li class="ui-helper-hidden-accessible"></li></ul>').bind('selectstart', function(){return false;}).appendTo(this.availableContainer);
    ...
    **************************************/
	...
	selectedValues: function() {
		return jQuery.map( this.element.find('option[selected]'), function(item,i) { return jQuery(item).val(); });
	},
			...
			if (data = this.options.dataParser.call(this, data)) {
				for (var key in data) {
					...
					if (this.element.find('option[value="'+key+'"]').size()==0) {
						elements.push( jQuery('<option value="'+key+'"/>').text(data[key].value).appendTo(this.element)[0] );
					}
				}
			}

			if (elements.length>0) {
				this._populateLists(jQuery(elements));
			}
			this._filter(this.availableList.children('li.ui-element'), data);
			
			var availableItemsCount = jQuery.objectKeys(data).length;
			
			if(availableItemsCount >= this.options.remoteLimit && this.options.displayMore == true) {
				if (!jQuery('#multiselectShowMore_'+this.idMultiSelect).length)  {
					var showMoreLink = jQuery('<p id="multiselectShowMore_'+this.idMultiSelect+'"><a href="javascript:void(0);">'+jQuery.tmpl(jQuery.ui.multiselect.locale.sInputShowMore)+'</a></p>');
					this.availableList.after(showMoreLink);
				} else {
					var showMoreLink = jQuery('#multiselectShowMore_'+this.idMultiSelect);
				}
				var that = this;
				...
				jQuery('#multiselectShowMore_'+this.idMultiSelect).fadeOut('fast',function() {jQuery(this).remove();});
			}
			this._setBusy(false);
    ...
    **************************************/
	...
	_setData: function(key, value) {
		switch (key) {
			...
			case 'sortable':
				// readonly options
				this._messages(
					jQuery.ui.multiselect.constants.MESSAGE_WARNING,
					jQuery.ui.multiselect.locale.errorReadonly,
					{option: key}
				);
			default:
				// default behavior
	...
	_messages: function(type, msg, params) {
		this._trigger('messages', null, this._ui('message', type, jQuery.tmpl(msg, params)));
	},
		...
		var _addNode = function() {
			...
			try {
				...
				if ( _addNodeRetry++ < 10 ) {
					...
					that._messages(
						jQuery.ui.multiselect.constants.MESSAGE_EXCEPTION,
						jQuery.ui.multiselect.locale.errorInsertNode,
	...
	_updateCount: function() {
		...
		this.selectedContainer.find('span.count')
			.html(jQuery.tmpl(jQuery.ui.multiselect.locale.itemsCount, {count:count}))
			.attr('title', jQuery.tmpl(jQuery.ui.multiselect.locale.itemsTotal, {count:total}));
	},
	_getOptionNode: function(option) {
		option = jQuery(option);
		var node = jQuery('<li class="ui-state-default ui-element"><span class="ui-icon"/>'+option.text()+'<a href="#" class="ui-state-default action"><span class="ui-corner-all ui-icon"/></a></li>').hide();
	...
	_batchSelect: function(elements, state) {
		...
		setTimeout(function() {
			...
			elements.each(function(i,element) {
				that._setSelected(jQuery(element), state);
			});
			...
			// restore
			jQuery.extend(that.options, _backup);
	...
	_getSuccessorNode: function(item, list) {
		...
		var succ, i = Math.min(item.data('multiselect.idx'),itemsSize-1), direction = comparator(item, jQuery(items[i]));
		...
		if ( direction ) {
			...
			if (0>direction && 0>=i) {
				...
				while (i>=0 && i<items.length) {
					...
					if ( direction != comparator(item, jQuery(items[i])) ) {
   ...
   // @param bool noclone (optional) true only if item should not be cloned on the other list
	_setSelected: function(item, selected, noclone) {
		...
		if (!this.busy) {
			if (this.options.animated) {
				...
				otherItem.fadeTo('fast', 0.3, function() { jQuery(this).fadeTo('fast', 1); });
			}
		}
	...
	_filter: function(elements, data) {
		var input = this.availableActions.children('input.search');
		var term = jQuery.trim( input.val().toLowerCase() );
	...
	_registerHoverEvents: function(elements) {
		elements
			.unbind('mouseover.multiselect').bind('mouseover.multiselect', function() {
				jQuery(this).find('a').andSelf().addClass('ui-state-hover');
			})
			.unbind('mouseout.multiselect').bind('mouseout.multiselect', function() {
				jQuery(this).find('a').andSelf().removeClass('ui-state-hover');
			})
	...
	_registerAddEvents: function(elements) {
		...
		elements.unbind('click.multiselect').bind('click.multiselect', function() {
			// ignore if busy...
			if (!this.busy) {
				that._setSelected(jQuery(this).parent(), true);
			}
			return false;
		});
	...
	_registerSearchEvents: function(input, searchNow) {
		...
		var _searchNow = function(forceUpdate) {
			...
			if ((value != previousValue) || (forceUpdate)) {
				...
				if (that.options.remoteUrl) {
					var params = jQuery.extend({}, that.options.remoteParams);
					try {
						jQuery.ajax({
							url: that.options.remoteUrl,
							data: jQuery.extend(params, {q:escape(value), start:that.options.remoteStart, limit:that.options.remoteLimit}),
							...
							error: function(request,status,e) {
								that._messages(
									jQuery.ui.multiselect.constants.MESSAGE_ERROR,
									jQuery.ui.multiselect.locale.errorRequest,
									{status:status}
								);
								that._setBusy(false);
							}
						...
						that._messages(jQuery.ui.multiselect.constants.MESSAGE_EXCEPTION, e.message);   // error message template ??
						that._setBusy(false);
					}
				} else {
		...
		if (this.options.searchable) {
			...
			.bind('focus.multiselect', function() {
				jQuery(this).addClass('ui-state-active').data('multiselect.hasFocus', true);
			})
			.bind('blur.multiselect', function() {
				jQuery(this).removeClass('ui-state-active').data('multiselect.hasFocus', false);
			})
