 ...
 *   ui.base.js
 *
 * Revision: $Id: jquery.treeview.sortable.js 7674 2011-07-21 09:34:44Z aFolletete $
 */
;(function(jQuery) {
	...
	jQuery.widget("ui.sortableTree", jQuery.extend(jQuery.ui.mouse, {
		init: function() {
		...
		propagate: function(n,e,inst) {
			jQuery.ui.plugin.call(this, n, [e, this.ui(inst)]);
			this.element.triggerHandler(n == "sort" ? n : "sort"+n, [e, this.ui(inst)], this.options[n]);
		},
		serialize: function(o) {
			
			var items = jQuery(this.options.items, this.element).not('.ui-sortableTree-helper'); //Only the items of the sortable itself
			var str = []; o = o || {};
			
			items.each(function() {
				var res = (jQuery(this).attr(o.attribute || 'id') || '').match(o.expression || (/(.+)[-=_](.+)/));
		...
		toArray: function(attr) {
			...
			items.each(function() { ret.push(jQuery(this).attr(attr || 'id')); });
			return ret;
		},
		enable: function() {
		...
		refreshItems: function() {
			...
			var queries = [jQuery(this.options.items, this.element)];
			
			if(this.options.connectWith) {
				for (var i = this.options.connectWith.length - 1; i >= 0; i--){
					var cur = jQuery(this.options.connectWith[i]);
					for (var j = cur.length - 1; j >= 0; j--){
						var inst = jQuery.data(cur[j], 'sortableTree');
						if(inst && !inst.options.disabled) {
							queries.push(jQuery(inst.options.items, inst.element));
			...
			for (var i = queries.length - 1; i >= 0; i--){
				queries[i].each(function() {
					jQuery.data(this, 'sortableTree-item', true); // Data for target checking (mouse manager)
					items.push({
						item: jQuery(this),
						width: 0, height: 0,
						left: 0, top: 0
		...
		mouseStart: function(e,el) {
			...
			var currentItem = null, nodes = jQuery(e.target).parents().each(function() {	
				if(jQuery.data(this, 'sortableTree-item')) {
					currentItem = jQuery(this);
					return false;
				}
			});
			if(jQuery.data(e.target, 'sortableTree-item')) currentItem = jQuery(e.target);
			
			if(!currentItem) return false;	
			if(this.options.handle) {
				var validHandle = false;
				jQuery(this.options.handle, currentItem).each(function() { if(this == e.target) validHandle = true; });
			...
			this.helper = typeof o.helper == 'function' ? jQuery(o.helper.apply(this.element[0], [e, this.currentItem])) : this.currentItem.clone();
			...
			if(jQuery.ui.ddmanager) jQuery.ui.ddmanager.current = this;
			if (jQuery.ui.ddmanager && !o.dropBehaviour) jQuery.ui.ddmanager.prepareOffsets(this, e);
		...
		mouseStop: function(e) {
			...
			var dropped = (jQuery.ui.ddmanager && !this.options.dropBehaviour) ? jQuery.ui.ddmanager.drop(this, e) : false;
		...
		mouseDrag: function(e) {
			...
			if(jQuery.ui.ddmanager) jQuery.ui.ddmanager.drag(this, e);
			var intersectsWithDroppable = false;
			jQuery.each(jQuery.ui.ddmanager.droppables, function() {
	...
	jQuery.extend(jQuery.ui.sortableTree, {
		defaults: {
			items: '> *',
			zIndex: 1000,
			distance: 1
