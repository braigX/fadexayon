	...
	jQuery.extend(jQuery.fn, {
		...
		applyClasses: function(settings, toggler) {
			...
			if (!settings.prerendered) {
				...
				hitarea.removeClass().addClass(CLASSES.hitarea).each(function() {
					var classes = "";
					jQuery.each(jQuery(this).parent().attr("class").split(" "), function() {
		...
		treeview: function(settings) {
			...
			function treeController(tree, control) {
				...
				jQuery("a:eq(0)", control).click( handler(CLASSES.collapsable) );
				// click on second to expand tree
				jQuery("a:eq(1)", control).click( handler(CLASSES.expandable) );
				// click on third to toggle tree
				jQuery("a:eq(2)", control).click( handler() ); 
			}
		
			// handle toggle event
			function toggler() {
				// Added by Prestashop
				if (jQuery(this).parent().is('.static'))
					return;
				jQuery(this)
					.parent()
			...
			function serialize() {
				...
				jQuery.cookie(settings.cookieId, data.join(""), settings.cookieOptions );
			}
