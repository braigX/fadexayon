...
class SeoInternalLinkingCoreClass extends Module
{
            ...
            foreach ($files as $filename) {
                ...
                if (Configuration::get('PS_FORCE_SMARTY_2')) {
                    ...
                    if (!$this->_smarty->clearCompiledTemplate($filename)) {
                        self::clearCompiledTplAlternative($filename, $this->_smarty->getCompileDir());
                    }
                }
            }
        }
    ...
    protected function _postProcess()
    {
        if (Tools::getValue('pm_save_obj')) {
            if (class_exists(Tools::getValue('pm_save_obj'))) {
                ...
                $this->errors = $obj->validateController();
    ...
    protected function _initClassVar()
    {
        ...
        $this->_employee = $this->context->employee;
        $this->_base_config_url = $_SERVER['SCRIPT_NAME'].(($controller = Tools::getValue('controller')) ? '?controller='.$controller: '') . '&configure=' . $this->name . '&token=' . Tools::getValue('token');
    ...
    protected function _headerIframe()
    {
        ...
        foreach ($this->context->controller->js_files as $jsUri) {
            if (!preg_match('#gamification|notifications\.js|helpAccess\.js|help\.js#i', $jsUri)) {
    ...
    public function _initDataTable($id_table, $returnHTML = false, $returnAsScript = false)
    {
        $vars = array(
    ...
    protected function _getModuleConfiguration()
    {
        if (Shop::getContext() == Shop::CONTEXT_SHOP) {
            $allShopConfig = self::getMultiShopValues('PM_' . self::$_module_prefix . '_CONF');
    ...
    protected static function getMultiShopValues($key, $id_lang = null)
    {
        if (method_exists('Configuration', 'getMultiShopValues')) {
            return Configuration::getMultiShopValues($key, $id_lang);
        }
    ...
    protected function registerSmartyObjects()
    {
        ...
        if (!$registered && !empty($this->context->smarty)) {
            ...
            $this->context->smarty->registerPlugin('function', self::$_module_prefix . '_initDataTable', array($this, '_initDataTable'));
    ...
    protected function _getDuration($seconds, $use = null, $zeros = false)
    {
        ...
        foreach ($periods as $period => $value) {
            ...
            $segments[Tools::strtolower($period)] = $count;
            $seconds = $seconds % $value;
        }
        $string = array();
        foreach ($segments as $key => $value) {
            $segment_name = Tools::substr($key, 0, -1);
    ...
    protected static function _isModuleActive($moduleName, $id_shop = false)
    {
        ...
        if (isset(self::$_isModuleActiveCache[$moduleName.(int)$id_shop])) {
            return self::$_isModuleActiveCache[$moduleName.(int)$id_shop];
        }
        $active = false;
