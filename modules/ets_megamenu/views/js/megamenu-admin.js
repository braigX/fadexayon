...
var mm_func = {
                ...
                $.ajax({
                    ...
                    success: function(json)
                    {
                        if (json) 
                        {
                            ...
                            mm_func.search();
                            $('.mm_form .mm_search_product').click();
                        }
    ...
    closeSearch: function () {
        ...
        $('.mm_form .mm_search_product').focus();
    },
...
$(document).ready(function(){
    if($('.adminmodules .mm_menus a').length)
        $('.adminmodules .mm_menus a:not(.mm_export_menu)').removeAttr('href');
   ...
   $(document).on('click','.mm_tab_toggle a',function(e){
        e.stopPropagation();
        e.preventDefault();
        return false;
   });
   ...
   $(document).on('change','input[type="file"]',function(){
        ...
        if ($.inArray($(this).val().split('.').pop().toLowerCase(), fileExtension) == -1) {
            ...
            if($(this).attr('name')=='sliderdata')            
                showErrorMessage(ets_mm_invalid_file);
            else
                showErrorMessage(ets_mm_invalid_image);
    ...
    $(document).on('click','.mm_menu_delete',function(){
            if(!$(this).hasClass('active') && confirm(confirm_delete_menu))
            {
                $(this).addClass('active');
                $.ajax({
                    url: mmBaseAdminUrl,
    ...
    $(document).on('click','.mm_tab_delete',function(){
            if(!$(this).hasClass('active') && confirm(confirm_delete_tab))
            {
                $(this).addClass('active');
                $.ajax({
                    url: mmBaseAdminUrl,
    ...
    $(document).on('click','.mm_column_delete',function(){
            if(!$(this).hasClass('active') && confirm(confirm_delete_column))
            {
                $(this).addClass('active');
                $.ajax({
                    url: mmBaseAdminUrl,
    ...
    $(document).on('click','.mm_block_delete',function(){
        if(!$(this).hasClass('active') && confirm(confirm_delete_block))
        {
            $(this).addClass('active');
            $.ajax({
                url: mmBaseAdminUrl,
        ...
        if(!$('.mm_config_form_content').hasClass('active'))
        {
            ...
            $.ajax({
                ...
                success: function(json){
                    ...
                    $('.mm_config_save').removeClass('loading');
    ...
    $(document).mouseup(function (e)
    {
        ...
        if(container.hasClass('mm_menu_form'))
            return false;
            ...
            $.ajax({
                ...
                success: function(json)
                {
                    if(!json.success)
                        ...
                        showSuccessMessage(Sorted_successfully_text);
                    displayHeightTab();
                },
                error: function()
                {
...
{    
    if(alertmsg)
    {
        if($('.defaultForm.active').parents('.mm_popup_overlay').eq(0).find('.alert').length > 0)
            $('.defaultForm.active').parents('.mm_popup_overlay').eq(0).find('.alert').remove();
        $('.defaultForm.active').parents('.mm_popup_overlay').eq(0).append(alertmsg);
    }    
}
function checkFormFields()
{
    if($('.mm_form select[name="link_type"]').length > 0)
    {
        ...
        {
            $('.mm_form .row_menu_ver_text_color, .mm_form .row_menu_ver_alway_show, .mm_form .row_menu_ver_alway_open_first, .mm_form .row_menu_ver_hidden_border, .mm_form .row_menu_ver_background_color,.mm_form .row_menu_item_width,.mm_form .row_tab_item_width').show();
