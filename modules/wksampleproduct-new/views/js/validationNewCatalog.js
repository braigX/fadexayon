...
*/
    ...
    function showError(elementSelector, errorMessage) {
        var errorHTML = '<div class="text-danger">' + errorMessage + '</div>';
...
function validateOnV2Page(){
    ...
    $(document).on('click', '#product_footer_save', function(e) {
        if($('#form_hooks_wk_follow_global').is(':checked') && isGlobalSampleEnable==0){
                e.preventDefault();
            showError("#globalValidation", globalSettigValidation);
        }

        if($('#form_hooks_max_cart_qty').val() <0){
           e.preventDefault();
           showError("#form_hooks_max_cart_qty", positiveQty);
        }else if ($('#form_hooks_max_cart_qty').val() == ''){
             e.preventDefault();
           showError("#form_hooks_max_cart_qty", invalidQty);
        }
