...
class AdminEcSeoMetaController extends ModuleAdminController
{
    ...
    public function renderForm()
    {
        ...
        $meta = new Meta($id_meta, null, $this->id_shop);
        $languages = Language::getLanguages(false);
       
        
        $this->fields_value = (array)$meta;
        foreach ($languages as $lang) {
            $id_lang = $lang['id_lang'];
            $meta->meta_title[$id_lang] = $meta->title[$id_lang];
            $meta->meta_description[$id_lang] = $meta->description[$id_lang];
            
           /*  if (Tools::strlen($meta->meta_title[$id_lang]) == 0) {
                $meta->meta_title[$id_lang] = $meta->name[$id_lang];
            }
            if (Tools::strlen($meta->meta_description[$id_lang]) == 0) {
                $meta->meta_description[$id_lang] = strip_tags($meta->description[$id_lang]);
            } */
            //$meta->description[$id_lang] = $meta->description[$id_lang].' '.$meta->short_description[$id_lang];
        }
        $meta->link_rewrite = $meta->url_rewrite;
        $this->fields_value['id_meta'] = $id_meta;
        $info_ec_seo_man = Db::getInstance()->getRow('SELECT * FROM '._DB_PREFIX_.'ec_seo_meta WHERE id_meta = '.(int)$id_meta.' AND id_shop = '.(int)$this->id_shop);
        if ($info_ec_seo_man) {
            $info_ec_seo_man_lang = Db::getInstance()->executes('SELECT * FROM '._DB_PREFIX_.'ec_seo_meta_lang WHERE id_seo_meta = '.(int)$info_ec_seo_man['id_seo_meta']);
            foreach ($info_ec_seo_man_lang as $val) {
                $meta->keyword[$val['id_lang']] = $val['keyword'];
                ...
                $meta->h1[$val['id_lang']] = $val['h1'];
                $meta->link_canonic[$val['id_lang']] = $val['link_canonic'];
                $meta->description[$val['id_lang']] = $val['description2'];
            }
        }
        $meta_desc = $mod->checkMeta($meta);

        $href = $this->context->link->getAdminLink('AdminMeta', true);

        if (strpos($href, 'shop') !== false) {
            $href = explode('?_token', $href);
            $href = $href[0].$id_meta.'/edit?_token'.$href[1];
        } else {
            $href.= '&id_meta='.$id_meta.'&updatemeta=1';
        }
        ...
        $br = '';
        foreach ($languages as $lang) {
            if (Tools::strlen($meta->keyword[$lang['id_lang']]) == 0) {
...
//        }
    ...
    public function initContent()
    {
        ...
        $id_meta = Tools::getValue('id_meta');
        $meta = new Meta($id_meta, null, $this->id_shop);
        $id_seo_meta = Db::getInstance()->getValue('SELECT id_seo_meta FROM '._DB_PREFIX_.'ec_seo_meta WHERE id_meta = '.(int)$id_meta.' AND id_shop = '.(int)$this->id_shop);
        if (Tools::getValue('updateKeyWord') == "1") {
            ...
            Db::getinstance()->update(
                ...
                'id_seo_meta = '.(int)$id_seo_meta.' AND id_lang = '.(int)$ec_id_lang.''
            );
        ...
        if (!$id_seo_meta) {
            $id_seo_meta = $mod->initSeo('meta', $id_meta);
            $tab_keyword = array();
            foreach ($languages as $lang) {
                $id_lang = $lang['id_lang'];
                $keyword =  Db::getInstance()->getValue('SELECT keyword FROM '._DB_PREFIX_.'ec_seo_meta_lang WHERE id_seo_meta = '.(int)$id_seo_meta.' AND id_lang = '.(int)$id_lang.'');
        ...
        if (((bool)Tools::isSubmit('submitAddconfiguration')) == true) {
            ...
            foreach ($languages as $lang) {
                $id_lang = $lang['id_lang'];
                //$meta->name[$id_lang] = pSQL($info['name_'.$id_lang]);
                //$meta->short_description[$id_lang] = str_replace('\r\n', '', pSQL($info['short_description_'.$id_lang], true));
                //$meta->description[$id_lang] = str_replace('\r\n', '', pSQL($info['description_'.$id_lang], true));
                $meta->title[$id_lang] = $info['title_'.$id_lang];
                $meta->url_rewrite[$id_lang] = $info['url_rewrite_'.$id_lang];
                    $meta->description[$id_lang] = $info['description_'.$id_lang];
                //$meta->link_rewrite[$id_lang] = pSQL($info['link_rewrite_'.$id_lang], true);
            /*     if (Tools::strlen($info['h1_'.$id_lang]) == 0) {
                    $info['h1_'.$id_lang] = $meta->name[$id_lang];
                } */
                $tab_ec_seo = array(
                    'id_seo_meta' => (int)$id_seo_meta,
        ...
        //$content = $this->context->smarty->fetch(_PS_MODULE_DIR_ . 'ec_seo/views/templates/admin/seo_meta.tpl');
