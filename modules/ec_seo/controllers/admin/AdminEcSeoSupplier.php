...
class AdminEcSeoSupplierController extends ModuleAdminController
{
    ...
    public function renderForm()
    {
        ...
        $sup = new Supplier($id_supplier, null);
        $languages = Language::getLanguages(false);
        foreach ($languages as $lang) {
            $id_lang = $lang['id_lang'];
            if (Tools::strlen($sup->meta_title[$id_lang]) == 0) {
                $sup->meta_title[$id_lang] = $sup->name;
            }
            /* if (Tools::strlen($sup->meta_description[$id_lang]) == 0) {
                $sup->meta_description[$id_lang] = strip_tags($sup->description[$id_lang]);
            } */
            //$sup->description[$id_lang] = $sup->description[$id_lang].' '.$sup->short_description[$id_lang];
        }
        
        $this->fields_value = (array)$sup;
        $this->fields_value['id_supplier'] = $id_supplier;
        $info_ec_seo_sup= Db::getInstance()->getRow('SELECT * FROM '._DB_PREFIX_.'ec_seo_supplier WHERE id_supplier = '.(int)$id_supplier.' AND id_shop = '.(int)$this->id_shop);
        if ($info_ec_seo_sup) {
            $info_ec_seo_sup_lang = Db::getInstance()->executes('SELECT * FROM '._DB_PREFIX_.'ec_seo_supplier_lang WHERE id_seo_supplier = '.(int)$info_ec_seo_sup['id_seo_supplier']);
            foreach ($info_ec_seo_sup_lang as $val) {
                $sup->keyword[$val['id_lang']] = $val['keyword'];
                ...
                $sup->h1[$val['id_lang']] = $val['h1'];
                $sup->link_canonic[$val['id_lang']] = $val['link_canonic'];
            }
        }
       
        
        $meta_desc = $mod->checkMeta($sup);

        $href = $this->context->link->getAdminLink('AdminSuppliers', true);

        if (strpos($href, 'sell') !== false) {
            $href = explode('?_token', $href);
            $href = $href[0].'/'.$id_supplier.'?_token'.$href[1];
        } else {
            $href.= '&id_supplier='.$id_supplier.'&updatesupplier';
        }
        ...
        $br = '';
        foreach ($languages as $lang) {
            if (Tools::strlen($sup->keyword[$lang['id_lang']]) == 0) {
...
//        }
    ...
    public function initContent()
    {
        ...
        $id_supplier = Tools::getValue('id_supplier');
        $sup= new Supplier($id_supplier, null);
        $id_seo_supplier = Db::getInstance()->getValue('SELECT id_seo_supplier FROM '._DB_PREFIX_.'ec_seo_supplier WHERE id_supplier = '.(int)$id_supplier.' AND id_shop = '.(int)$this->id_shop);
        if (Tools::getValue('updateKeyWord') == "1") {
            $ec_id_lang = Tools::getValue('ec_id_lang');
            Db::getinstance()->update(
                'ec_seo_supplier_lang',
                array(
                    'keyword' => pSQL(Tools::getValue('keyword'))
                ),
                'id_seo_supplier = '.(int)$id_seo_supplier.' AND id_lang = '.(int)$ec_id_lang.''
            );
        ...
        if (!$id_seo_supplier) {
            $id_seo_supplier = $mod->initSeo('supplier', $id_supplier);
            $tab_keyword = array();
            foreach ($languages as $lang) {
                $id_lang = $lang['id_lang'];
                $keyword =  Db::getInstance()->getValue('SELECT keyword FROM '._DB_PREFIX_.'ec_seo_supplier_lang WHERE id_seo_supplier = '.(int)$id_seo_supplier.' AND id_lang = '.(int)$id_lang.'');
        ...
        $mod->checkSeoNewLang($sup, 'supplier', $id_seo_supplier);
        if (((bool)Tools::isSubmit('submitAddconfiguration')) == true) {
            ...
            foreach ($languages as $lang) {
                $id_lang = $lang['id_lang'];
                //$sup->name[$id_lang] = pSQL($info['name_'.$id_lang]);
                //$sup->short_description[$id_lang] = str_replace('\r\n', '', pSQL($info['short_description_'.$id_lang], true));
                $sup->description[$id_lang] = str_replace('\r\n', '', $info['description_'.$id_lang]);
                $sup->meta_title[$id_lang] = $info['meta_title_'.$id_lang];
                $sup->meta_description[$id_lang] = $info['meta_description_'.$id_lang];
                //$sup->link_rewrite[$id_lang] = pSQL($info['link_rewrite_'.$id_lang], true);
            /*     if (Tools::strlen($info['h1_'.$id_lang]) == 0) {
                    $info['h1_'.$id_lang] = $sup->name[$id_lang];
                } */
                $tab_ec_seo = array(
        ...
        //$content = $this->context->smarty->fetch(_PS_MODULE_DIR_ . 'ec_seo/views/templates/admin/seo_supplier.tpl');
