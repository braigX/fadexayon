...
class Ec_seoBulkImCMSModuleFrontController extends ModuleFrontController
{
    public $auth = false;

    public $ajax = true;
    protected $ec_token;
    protected $prefix = 'TASK_EC_SEO_IM_CMS_';
    protected $cron = 'bulkImCMS';
...
//        );
    ...
    {
        Db::getinstance()->delete('ec_seo_mi_temp', 'page = "cms"');
    ...
    public function internal_Mesh_CMS()
    {
        ...
        $info_spe = Db::getInstance()->executes(
            'SELECT id, keyword, url, id_lang FROM '._DB_PREFIX_.'ec_seo_cms_mi mi
            LEFT JOIN '._DB_PREFIX_.'ec_seo_cms_mi_lang mil ON (mi.id = mil.id_seo_cms_mi)
        ...
        if ($active_backup) {
            ...
            $backup_file = dirname(__FILE__).'/../../backup/InternalMeshCMS/'.$start_date.'.json';
        }
        ...
        foreach ($cmss as $key => $val) {
            ...
            foreach ($tab_config as $info) {
                $id_lang = $info['id_lang'];
                if (Tools::strlen($cms->content[$id_lang]) > 0) {
                    ...
                    if (!preg_match('/'.$preg_match1.'/', $cms->content[$id_lang]) && !preg_match('/'.$preg_match2.'/', $cms->content[$id_lang]) && !preg_match('/'.$preg_match3.'/', $cms->content[$id_lang])) {
                        $info_desc = $mod->genMiClean($cms->content[$id_lang], $info['keyword'], $replace, 1);
                        $cms->content[$id_lang] = $info_desc['content'];
            ...
            if ($count > 0) {
                ...
                try {
                    ...
                    $this->logger->logInfo('Fail save cat '.$id_cms.' '. $e->getMessage() . ' in line ' . $e->getLine() . ' of file ' . $e->getFile());
                }
            }
