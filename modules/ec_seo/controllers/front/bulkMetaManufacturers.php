...
class Ec_seoBulkMetaManufacturersModuleFrontController extends ModuleFrontController
{
    public $auth = false;

    public $ajax = true;
    protected $ec_token;
    protected $prefix = 'TASK_EC_SEO_META_MANUFACTURERS_';
    protected $cron = 'bulkMetaManufacturers';
...
//        );
    ...
    {
        Db::getinstance()->delete('ec_seo_meta_temp', 'page = "manufacturer"');
    ...
    public function metaManufacturer()
    {
        ...
        $mod = new Ec_seo();
        $errors = array();
        $info_gen = Db::getInstance()->executes('SELECT meta_title, meta_description, id_lang FROM '._DB_PREFIX_.'ec_seo_manufacturer_mg_gen WHERE id_shop = '.(int)$id_shop);
        $tab_config = array();
        ...
        if ($active_backup) {
            ...
            $backup_file = dirname(__FILE__).'/../../backup/MetaManufacturers/'.$start_date.'.json';
        }
        ...
        foreach ($manufacturers as $key => $val) {
            ...
            foreach ($languages as $lang) {
                ...
                $u_lang = false;
                if (Tools::strlen($tab_config['meta_title'][$id_lang]) > 0 && (!$only_empty_metas || Tools::strlen($manufacturer->meta_title[$id_lang]) == 0)) {
                    $meta_title = $mod->genMetaManufacturer($manufacturer, $tab_config['meta_title'][$id_lang], $id_lang, $manufacturer->meta_title[$id_lang]);
                    $manufacturer->meta_title[$id_lang] = $meta_title;
                    $update = true;
                    $u_lang = true;
                }
                if (Tools::strlen($tab_config['meta_description'][$id_lang]) > 0 && (!$only_empty_metas || Tools::strlen($manufacturer->meta_description[$id_lang]) == 0)) {
                    $meta_description = $mod->genMetaManufacturer($manufacturer, $tab_config['meta_description'][$id_lang], $id_lang, $manufacturer->meta_description[$id_lang]);
                    $manufacturer->meta_description[$id_lang] = $meta_description;
                    $update = true;
                    $u_lang = true;
                }
                if ($u_lang) {
            ...
            if ($update) {
                try {
                    ...
                    $this->logger->logInfo('Fail save manufacturer '.$id_manufacturer.' '.$e->getMessage().' !! '.json_encode($manufacturer));
                }
            }
