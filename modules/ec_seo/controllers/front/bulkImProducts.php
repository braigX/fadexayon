...
class Ec_seoBulkImProductsModuleFrontController extends ModuleFrontController
{
    public $auth = false;

    public $ajax = true;
    protected $ec_token;
    protected $prefix = 'TASK_EC_SEO_IM_PRODUCTS_';
    protected $cron = 'bulkImProducts';
...
//        );
    ...
    {
        Db::getinstance()->delete('ec_seo_mi_temp', 'page = "product"');
    ...
    public function internal_Mesh_Product()
    {
        ...
        $info_spe = Db::getInstance()->executes(
            'SELECT id, keyword, url, id_category, id_lang FROM '._DB_PREFIX_.'ec_seo_product_mi mi
            LEFT JOIN '._DB_PREFIX_.'ec_seo_product_mi_lang mil ON (mi.id = mil.id_seo_product_mi)
            LEFT JOIN '._DB_PREFIX_.'ec_seo_product_mi_cat mic ON (mi.id = mic.id_seo_product_mi)
        ...
        if ($active_backup) {
            ...
            $backup_file = dirname(__FILE__).'/../../backup/InternalMeshProducts/'.$start_date.'.json';
        }
        $active_prod = Configuration::get('EC_SEO_MI_ACTIVE_PROD');
        $req_active_prod = '';
        if ($active_prod) {
        ...
        foreach ($prods as $key => $val) {
            ...
            $count = 0;
            foreach ($tab_config[$prod->id_category_default] as $info) {
                $id_lang = $info['id_lang'];
                if (Tools::strlen($prod->description[$id_lang]) > 0) {
                    ...
                    $preg_match3 = 'target="_blank"> '.$info['keyword'].'<\/a>';
                    $preg_match4 = ''.$info['keyword'].'<\/a>';
                    if (!preg_match('/'.$preg_match1.'/', $prod->description[$id_lang]) && !preg_match('/'.$preg_match2.'/', $prod->description[$id_lang]) && !preg_match('/'.$preg_match3.'/', $prod->description[$id_lang]) && !preg_match('/'.$preg_match4.'/', $prod->description[$id_lang])) {
                        $info_desc = $mod->genMiClean($prod->description[$id_lang], $info['keyword'], $replace, 1);
                        $prod->description[$id_lang] = $info_desc['content'];
