...
class Ec_seoBulkImManufacturersModuleFrontController extends ModuleFrontController
{
    public $auth = false;

    public $ajax = true;
    protected $ec_token;
    protected $prefix = 'TASK_EC_SEO_IM_MANUFACTURERS_';
    protected $cron = 'bulkImManufacturers';
...
//        );
    ...
    {
        Db::getinstance()->delete('ec_seo_mi_temp', 'page = "manufacturer"');
    ...
    public function internal_Mesh_Manufacturer()
    {
        ...
        $info_spe = Db::getInstance()->executes(
            'SELECT id, keyword, url, id_lang FROM '._DB_PREFIX_.'ec_seo_manufacturer_mi mi
            LEFT JOIN '._DB_PREFIX_.'ec_seo_manufacturer_mi_lang mil ON (mi.id = mil.id_seo_manufacturer_mi)
        ...
        if ($active_backup) {
            ...
            $backup_file = dirname(__FILE__).'/../../backup/InternalMeshManufacturers/'.$start_date.'.json';
        }
        $manufacturers = Db::getInstance()->executes('SELECT id_manufacturer FROM '._DB_PREFIX_.'manufacturer_shop WHERE id_shop = '.(int)$id_shop);
        $jobLine = 0;
        foreach ($manufacturers as $key => $val) {
            ...
            $manufacturer = new Manufacturer($id_manufacturer, null, $id_shop);
            if (!Validate::isLoadedObject($manufacturer)) {
                $this->logger->logInfo('Manufacturer '.$id_manufacturer.' object invalid');
                continue;
            }
            if ($active_backup) {
                $backup_manufacturer = array(
                    'id' => $manufacturer->id,
                    'description' => $manufacturer->description,
                );
            }
            $count = 0;
            foreach ($tab_config as $info) {
                $id_lang = $info['id_lang'];
                if (Tools::strlen($manufacturer->description[$id_lang]) > 0) {
                    ...
                    if (!preg_match('/'.$preg_match1.'/', $manufacturer->description[$id_lang]) && !preg_match('/'.$preg_match2.'/', $manufacturer->description[$id_lang]) && !preg_match('/'.$preg_match3.'/', $manufacturer->description[$id_lang])) {
                        $info_desc = $mod->genMiClean($manufacturer->description[$id_lang], $info['keyword'], $replace, 1);
                        $manufacturer->description[$id_lang] = $info_desc['content'];
                        $count += $info_desc['total_replace'];
                        if ($info_desc['total_replace'] > 0) {
                            Db::getinstance()->insert(
                                'ec_seo_mi_temp',
                                array(
                                    'id' => (int)$id_manufacturer,
                                    'page' => 'manufacturer',
            ...
            if ($count > 0) {
                if ($active_backup) {
                    $backup[] = $backup_manufacturer;
                }
                try {
                    $manufacturer->save();
                } catch (Exception $e) {
                    $this->logger->logInfo('Fail save cat '.$id_manufacturer.' '.$e->getMessage(). ' in line ' . $e->getLine() . ' of file ' . $e->getFile());
                }
            }
