...
class Ec_seoBulkImCategoriesModuleFrontController extends ModuleFrontController
{
    public $auth = false;

    public $ajax = true;
    protected $ec_token;
    protected $prefix = 'TASK_EC_SEO_IM_CATEGORIES_';
    protected $cron = 'bulkImCategories';
...
//        );
        ...
        if ($this->action === 'next') {
            ...
            if ($nextKey) {
                ...
                if ($this->prg) {
                    ...
                    $nextCron = false;
                    if ($nextCron) {
                        $this->module->followLink($nextCron['link'] . '&prg=' . $this->prg . '&pos=' . $nextCron['position']);
                    }
                }
    ...
    {
        Db::getinstance()->delete('ec_seo_mi_temp', 'page = "category"');
    ...
    public function internal_Mesh_Category()
    {
        ...
        $info_spe = Db::getInstance()->executes(
            'SELECT id, keyword, url, id_category, id_lang FROM '._DB_PREFIX_.'ec_seo_category_mi mi
            LEFT JOIN '._DB_PREFIX_.'ec_seo_category_mi_lang mil ON (mi.id = mil.id_seo_category_mi)
            LEFT JOIN '._DB_PREFIX_.'ec_seo_category_mi_cat mic ON (mi.id = mic.id_seo_category_mi)
        ...
        foreach ($info_spe as $info) {
            if (Tools::strlen($info['keyword']) >0 && Tools::strlen($info['url']) > 0) {
                $tab_config[$info['id_category']][] = array(
        ...
        $filter_cat = '';
        if (count($tab_cat) > 0) {
            $filter_cat = 'AND id_category IN ('.implode(',', $tab_cat).')';
        }
        $active_backup = Configuration::get('EC_SEO_BACKUP');
        if ($active_backup) {
            $backup = array();
            $start_date = str_replace(' ', '-', Configuration::get($this->prefix.'START_TIME'));
            $backup_file = dirname(__FILE__).'/../../backup/InternalMeshCategories/'.$start_date.'.json';
        }
        ...
        foreach ($cats as $key => $val) {
            ...
            $count = 0;
            foreach ($tab_config[$cat->id] as $info) {
                $id_lang = $info['id_lang'];
                if (Tools::strlen($cat->description[$id_lang]) > 0) {
                    $replace = '<a href="'.$info['url'].'" target="_blank">'.$info['keyword'].'</a>';
                    $preg_match1 = 'target="_blank">'.$info['keyword'].'<\/a>';
                    $preg_match2 = 'target="_blank" rel="noreferrer noopener">'.$info['keyword'].'<\/a>';
                    $preg_match3 = ''.$info['keyword'].'<\/a>';
                    if (!preg_match('/'.$preg_match1.'/', $cat->description[$id_lang]) && !preg_match('/'.$preg_match2.'/', $cat->description[$id_lang]) && !preg_match('/'.$preg_match3.'/', $cat->description[$id_lang])) {
                        $info_desc = $mod->genMiClean($cat->description[$id_lang], $info['keyword'], $replace, 1);
                        $cat->description[$id_lang] = $info_desc['content'];
                        $count += $info_desc['total_replace'];
                        if ($info_desc['total_replace'] > 0) {
                            Db::getinstance()->insert(
                                'ec_seo_mi_temp',
                                array(
                                    'id' => (int)$id_category,
                                    'page' => 'category',
                                    'nb_replace' => (int)$info_desc['total_replace'],
                                    'id_lang' => (int)$id_lang,
                                    'id_shop' => (int)$id_shop,
                                )
                            );
