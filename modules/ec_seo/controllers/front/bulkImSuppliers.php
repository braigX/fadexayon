...
class Ec_seoBulkImSuppliersModuleFrontController extends ModuleFrontController
{
    public $auth = false;

    public $ajax = true;
    protected $ec_token;
    protected $prefix = 'TASK_EC_SEO_IM_SUPPLIERS_';
    protected $cron = 'bulkImSuppliers';
...
//        );
    ...
    {
        Db::getinstance()->delete('ec_seo_mi_temp', 'page = "supplier"');
    ...
    public function internal_Mesh_Supplier()
    {
        ...
        $info_spe = Db::getInstance()->executes(
            'SELECT id, keyword, url, id_lang FROM '._DB_PREFIX_.'ec_seo_supplier_mi mi
            LEFT JOIN '._DB_PREFIX_.'ec_seo_supplier_mi_lang mil ON (mi.id = mil.id_seo_supplier_mi)
        ...
        if ($active_backup) {
            ...
            $backup_file = dirname(__FILE__).'/../../backup/InternalMeshSuppliers/'.$start_date.'.json';
        }
        $suppliers = Db::getInstance()->executes('SELECT id_supplier FROM '._DB_PREFIX_.'supplier_shop WHERE id_shop = '.(int)$id_shop);
        $jobLine = 0;
        foreach ($suppliers as $key => $val) {
            ...
            $supplier = new Supplier($id_supplier, null, $id_shop);
            if (!Validate::isLoadedObject($supplier)) {
                $this->logger->logInfo('Supplier '.$id_supplier.' object invalid');
                continue;
            }
            if ($active_backup) {
                $backup_supplier = array(
                    'id' => $supplier->id,
            ...
            foreach ($tab_config as $info) {
                $id_lang = $info['id_lang'];
                if (Tools::strlen($supplier->description[$id_lang]) > 0) {
                    ...
                    if (!preg_match('/'.$preg_match1.'/', $supplier->description[$id_lang]) && !preg_match('/'.$preg_match2.'/', $supplier->description[$id_lang]) && !preg_match('/'.$preg_match3.'/', $supplier->description[$id_lang])) {
                        $info_desc = $mod->genMiClean($supplier->description[$id_lang], $info['keyword'], $replace, 1);
                        $supplier->description[$id_lang] = $info_desc['content'];
            ...
            if ($count > 0) {
                if ($active_backup) {
                    $backup[] = $backup_supplier;
                }
                try {
                    $supplier->save();
                } catch (Exception $e) {
                    $this->logger->logInfo('Fail save cat '.$id_supplier.' '.$e->getMessage(). ' in line ' . $e->getLine() . ' of file ' . $e->getFile());
                }
            }
