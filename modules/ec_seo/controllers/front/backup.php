...
class Ec_seoBackupModuleFrontController extends ModuleFrontController
{
    public $auth = false;

    public $ajax = true;
    protected $ec_token;
    protected $prefix = 'TASK_EC_SEO_BACKUP_';
    protected $cron = 'backup';
    ...
    public function init()
    {
        ...
        $this->object = Tools::getValue('object');
        $this->file = Tools::getValue('file');
        ...
        $this->params = '?ec_token=' . $this->ec_token . '&ts=' . $this->ts . '&jid=' . $this->jid.'&id_shop='.$this->id_shop.'&object='.$this->object.'&file='.$this->file;
...
//        );
    ...
    public function backup()
    {
        ...
        $object = $this->object;
        $object = str_replace('InternalMesh', '', $object);
        $object = str_replace('Meta', '', $object);
        $object = str_replace('Alt', '', $object);
        $class = $tab_class[$object];

        $backups = json_decode(Tools::file_get_contents(dirname(__FILE__).'/../../backup/'.$file), true);
        $jobLine = 0;
        foreach ($backups as $key => $backup) {
            ...
            try {
                ...
                $this->logger->logInfo('Fail save '.$class.' '.$object->id.' '.$e->getMessage());
            }
