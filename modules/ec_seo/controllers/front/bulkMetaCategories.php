...
class Ec_seoBulkMetaCategoriesModuleFrontController extends ModuleFrontController
{
    public $auth = false;

    public $ajax = true;
    protected $ec_token;
    protected $prefix = 'TASK_EC_SEO_META_CATEGORIES_';
    protected $cron = 'bulkMetaCategories';
...
//        );
    ...
    public function postProcess()
    {
        ...
        if ($this->starting) {
            ...
            Configuration::updateGlobalValue($this->prefix . 'PROGRESS', $this->nbCron, $this->logger);
        }
    ...
    {
        Db::getinstance()->delete('ec_seo_meta_temp', 'page = "category"');
    ...
    public function metaCategoryGen()
    {
        //return true;
        ...
        $mod = new Ec_seo();
        $errors = array();
        $info_gen = Db::getInstance()->executes('SELECT meta_title, meta_description, id_lang FROM '._DB_PREFIX_.'ec_seo_category_mg_gen WHERE id_shop = '.(int)$id_shop);
        $tab_config = array();
        ...
        $tab_cat = array();
        $cats = Db::getInstance()->executes('SELECT id_category FROM '._DB_PREFIX_.'ec_seo_category_mg_cat');
        foreach ($cats as $cat) {
            $tab_cat[] = $cat['id_category'];
        }
        $filter_cat = '';
        if (count($tab_cat) > 0) {
            $filter_cat = 'AND id_category NOT IN ('.implode(',', $tab_cat).')';
        }
        $active_backup = Configuration::get('EC_SEO_BACKUP');
        if ($active_backup) {
            $backup = array();
            $start_date = str_replace(' ', '-', Configuration::get($this->prefix.'START_TIME'));
            $backup_file = dirname(__FILE__).'/../../backup/MetaCategories/'.$start_date.'.json';
        }
        $cats = Db::getInstance()->executes('SELECT id_category FROM '._DB_PREFIX_.'category_shop WHERE id_shop = '.(int)$id_shop.' '.$filter_cat);
        ...
        foreach ($cats as $key => $val) {
            ...
            $cat = new Category($id_category, null, $id_shop);
            if (!Validate::isLoadedObject($cat)) {
                $this->logger->logInfo('Category '.$id_category.' object invalid');
                continue;
            }
            if ($active_backup) {
                $backup_cat = array(
            ...
            foreach ($languages as $lang) {
                ...
                $u_lang = false;
                if (Tools::strlen($tab_config['meta_title'][$id_lang]) > 0 && (!$only_empty_metas || Tools::strlen($cat->meta_title[$id_lang]) == 0)) {
                    $meta_title = $mod->genMetaCategory($cat, $tab_config['meta_title'][$id_lang], $id_lang, $cat->meta_title[$id_lang]);
                    if ($meta_title != $cat->meta_title[$id_lang]) {
                        $cat->meta_title[$id_lang] = $meta_title;
                        $update = true;
                        $u_lang = true;
                    }
                }
                if (Tools::strlen($tab_config['meta_description'][$id_lang]) > 0 && (!$only_empty_metas || Tools::strlen($cat->meta_description[$id_lang]) == 0)) {
                    $meta_description = $mod->genMetaCategory($cat, $tab_config['meta_description'][$id_lang], $id_lang, $cat->meta_description[$id_lang]);
                    if ($meta_description != $cat->meta_description[$id_lang]) {
            ...
            if ($update) {
                try {
                    //$this->logger->logInfo('cat id '.$cat->id.' content '.json_encode(($cat));
                    $cat->doNotRegenerateNTree = 1;
                    $cat->save();
                } catch (Exception $e) {
                    $this->logger->logInfo('Fail save cat '.$id_category.' '.$e->getMessage(). ' in line ' . $e->getLine() . ' of file ' . $e->getFile());
                }
            }
    ...
    public function metaCategorySpe()
    {
        //return true;
        $languages = Language::getLanguages(false);
        $id_shop = $this->id_shop;
        ...
        $info_spe = Db::getInstance()->executes(
            'SELECT id, meta_title, meta_description, id_category, id_lang FROM '._DB_PREFIX_.'ec_seo_category_mg mg
            LEFT JOIN '._DB_PREFIX_.'ec_seo_category_mg_lang mgl ON (mg.id = mgl.id_seo_category_mg)
            LEFT JOIN '._DB_PREFIX_.'ec_seo_category_mg_cat mgc ON (mg.id = mgc.id_seo_category_mg)
        ...
        $only_empty_metas = Configuration::get('EC_SEO_ONLY_EMTPY_METAS');
        $jobLine = 0;
        foreach ($tab_config as $id_category => $val) {
            if (($this->stopTime < time()) && ($jobLine > $this->nbCron)) {
                if ($active_backup) {
                    if (count($backup) > 0) {
            ...
            foreach ($languages as $lang) {
                $u_lang = false;
                $id_lang = $lang['id_lang'];
                if (Tools::strlen($val['meta_title'][$id_lang]) > 0 && (!$only_empty_metas || Tools::strlen($cat->meta_title[$id_lang]) == 0)) {
                    $meta_title = $mod->genMetaCategory($cat, $val['meta_title'][$id_lang], $id_lang, $cat->meta_title[$id_lang]);
                    if ($meta_title != $cat->meta_title[$id_lang]) {
                        $cat->meta_title[$id_lang] = $meta_title;
                        $update = true;
                        $u_lang = true;
                ...
                if (Tools::strlen($val['meta_description'][$id_lang]) > 0 && (!$only_empty_metas || Tools::strlen($cat->meta_description[$id_lang]) == 0)) {
                    $meta_description = $mod->genMetaCategory($cat, $val['meta_description'][$id_lang], $id_lang, $cat->meta_description[$id_lang]);
                    if ($meta_description != $cat->meta_description[$id_lang]) {
                        $cat->meta_description[$id_lang] = $meta_description;
                        $update = true;
                        $u_lang = true;
