...
class Ec_seoBulkMetaSuppliersModuleFrontController extends ModuleFrontController
{
    public $auth = false;

    public $ajax = true;
    protected $ec_token;
    protected $prefix = 'TASK_EC_SEO_META_SUPPLIERS_';
    protected $cron = 'bulkMetaSuppliers';
...
//        );
    ...
    public function metaSupplier()
    {
        ...
        $mod = new Ec_seo();
        $errors = array();
        $info_gen = Db::getInstance()->executes('SELECT meta_title, meta_description, id_lang FROM '._DB_PREFIX_.'ec_seo_supplier_mg_gen WHERE id_shop = '.(int)$id_shop);
        $tab_config = array();
        ...
        if ($active_backup) {
            ...
            $backup_file = dirname(__FILE__).'/../../backup/MetaSuppliers/'.$start_date.'.json';
        }
        ...
        foreach ($suppliers as $key => $val) {
            ...
            foreach ($languages as $lang) {
                ...
                $u_lang = false;
                if (Tools::strlen($tab_config['meta_title'][$id_lang]) > 0 && (!$only_empty_metas || Tools::strlen($supplier->meta_title[$id_lang]) == 0)) {
                    $meta_title = $mod->genMetaSupplier($supplier, $tab_config['meta_title'][$id_lang], $id_lang, $supplier->meta_title[$id_lang]);
                    $supplier->meta_title[$id_lang] = $meta_title;
                    $update = true;
                    $u_lang = true;
                }
                if (Tools::strlen($tab_config['meta_description'][$id_lang]) > 0 && (!$only_empty_metas || Tools::strlen($supplier->meta_description[$id_lang]) == 0)) {
                    $meta_description = $mod->genMetaSupplier($supplier, $tab_config['meta_description'][$id_lang], $id_lang, $supplier->meta_description[$id_lang]);
                    $supplier->meta_description[$id_lang] = $meta_description;
                    $update = true;
                    $u_lang = true;
                }
                if ($u_lang) {
            ...
            if ($update) {
                try {
                    ...
                    $this->logger->logInfo('Fail save supplier '.$id_supplier.' '.$e->getMessage(). ' in line ' . $e->getLine() . ' of file ' . $e->getFile().' !! '.json_encode($supplier));
                }
            }
