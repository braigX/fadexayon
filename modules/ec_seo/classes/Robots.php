...
class Robots
{
    ...
    public function __construct($site, $id_shop = '')
    {
        $this->site = $site;
    ...
    public function isOkToCrawl($uri, $ignoreCrawlDelays = false)
    {
        ...
        if (empty($this->robotsTxtLastUpdated) || $this->robotsTxtLastUpdated < $lastRobotsUpdateThreshold) {
            $this->robotsTxt            = $this->_parseRobotsTxt($this->_fetchRobotsTxt());
    ...
    {
        return file_get_contents($this->site . '/robots'.$this->id_shop.'.txt');
    }
