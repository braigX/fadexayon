...
class AdminOrderManagerExportsController extends ModuleAdminController
{
    ...
    public function init()
    {
        ...
        if(Tools::isSubmit('loadInputChangeInline'))
        {
            $this->loadInputChangeInline();
        }
    }
    ...
    public function _searchCustomer()
    {
        ...
        if (!$query OR $query == '' OR Tools::strlen($query) < 1 && Validate::isCleanHtml($query))
            die();
        $items = Ode_dbbase::getCustomerByQuery($query);
        if ($items)
    ...
    public function renderForm()
    {
        ...
        elseif (($id_ets_export_order_rule = (int)Tools::getValue('id_ets_export_order_rule')) && ($exportRule = new Ode_export($id_ets_export_order_rule)) && Validate::isLoadedObject($exportRule) ) {
            $this->context->smarty->assign(array(
                'customers' => $this->module->getCustomers($exportRule->specific_customer),
                'list_fields' => $this->module->getListFields($exportRule->exported_fields),
            ));
        } 
        else
        {
        ...
        $configs = Ode_defines::getInstance($this->module)->getFields('rule',$id_ets_export_order_rule);
        $fields_form = array(
            'form' => array(
                'legend' => array(
    ...
    {
        $datas = Ode_export::getRuleExports();
        if ($datas) {
    ...
    public function loadInputChangeInline()
    {
        $row_order = Tools::getValue('row_order');
        $result = array();
        if($row_order)
        {
            $fields_list =  $this->module->getFieldsListOrder();
            foreach(array_keys($row_order) as $key)
            {
                if(isset($fields_list[$key]) && isset($fields_list[$key]['edit']) && $fields_list[$key]['edit'])
                {
                    $this->context->smarty->assign(
                        array(
                            'tr' => $row_order,
                    ...
                    $result[] = array(
                        'column_name' => $key,
                        'form' => $this->context->smarty->fetch(_PS_MODULE_DIR_.$this->module->name.'/views/templates/hook/input_change_inline.tpl'),
                    );
                }
            }
        }
