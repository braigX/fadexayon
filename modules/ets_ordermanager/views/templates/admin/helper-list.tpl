    ...
    <div class="panel-heading">{$title|escape:'html':'UTF-8'} <span class="badge badge-default">{$field_values|count|intval}</span>
        <span class="panel-heading-action">
            {if !isset($show_add_new) || isset($show_add_new) && $show_add_new}
                <a class="list-toolbar-btn" href="{$currentIndex|cat:'&add'|cat: $table|escape:'html':'UTF-8'}">
                ...
                <a class="list-toolbar-btn" href="{$link->getAdminLink('AdminOrders')|escape:'html':'UTF-8'}">
                    <span data-placement="top" data-html="true" data-original-title="{l s='Manage orders' mod='ets_ordermanager'}" class="label-tooltip hidden_not_15" data-toggle="tooltip" title="">
        				<i class="process-icon-shopping-cart icon icon-shopping-cart"></i> {l s='Manage orders' mod='ets_ordermanager'}
                    </span>
                </a>
                <a class="list-toolbar-btn" href="{$link->getAdminLink('AdminModules')|escape:'html':'UTF-8'}&configure=ets_ordermanager">
                    <span data-placement="top" data-html="true" data-original-title="{l s='General settings' mod='ets_ordermanager'}" class="label-tooltip hidden_not_15" data-toggle="tooltip" title="">
        				<i class="process-icon-cogs icon icon-cogs"></i> {l s='General settings' mod='ets_ordermanager'}
                    </span>
                </a>
            {/if}
            ...
            <form method="post" action="{$currentIndex|escape:'html':'UTF-8'}&amp;list=true">
                <div class="table configuration">
                    <div class="table-row nodrag nodrop">
                        ...
                        {/foreach}
                        {if $show_action}
                            <div class="table-cell" style="text-align: right;">{l s='Action' mod='ets_ordermanager'}</div>
                        {/if}
                    </div>
                    {if $field_values}
                                    ...
                                    <div class="table-cell pointer">
                                        ...
                                        {elseif $key == 'send_file_via_email'}
                                            {if $row.send_file_via_email}<ul class="list_table send_email">
                                                <li class="li_item"><span>{l s='Receiver emails:' mod='ets_ordermanager'} {$row.receivers_mail|escape:'html':'UTF-8'}</span></li>
                                                <li class="li_item"><span>{l s='Frequency:' mod='ets_ordermanager'}
                                                ...
                                                {if $row.send_file_schedule != 'hourly'}
                                                    <li class="li_item"><span>{l s='Send on (at):' mod='ets_ordermanager'}
                                                        {if $row.send_file_schedule == 'daily'}{$hours[$row.send_file_time_hours]|escape:'html':'UTF-8'}{/if}
                                                        {if $row.send_file_schedule == 'weekly'}{$weeks[$row.send_file_time_weeks]|escape:'html':'UTF-8'} - {$hours[$row.send_file_time_hours]|escape:'html':'UTF-8'}{/if}
                                                        {if $row.send_file_schedule == 'monthly'}{foreach from=$months item='item'}{if $item.id_option == $row.send_file_time_months}{$item.label|escape:'html':'UTF-8'}{/if}{/foreach} - {$hours[$row.send_file_time_hours]|escape:'html':'UTF-8'}{/if}
                                                    </span></li>
                                                {/if}
                                            </ul>{else}{l s='--' mod='ets_ordermanager'}{/if}
                                        {elseif $key == 'export_to_server2'}
                                            {if $row.export_to_server2}<ul class="list_table server2">
                                                {if $row.directory_path2}<li class="li_item"><span>{l s='Remote directory path:' mod='ets_ordermanager'} {$row.directory_path2|escape:'html':'UTF-8'}</span></li>{/if}
                                                <li class="li_item"><span>{l s='Frequency:' mod='ets_ordermanager'}
                                                    {assign var="ik" value="0"}
                                                ...
                                                {if $row.server2_schedule != 'hourly'}
                                                    <li class="li_item"><span>{l s='Send on (at):' mod='ets_ordermanager'}
                                                        {if $row.server2_schedule == 'daily'}{$hours[$row.server2_time_hours]|escape:'html':'UTF-8'}{/if}
                                                        {if $row.server2_schedule == 'weekly'}{$weeks[$row.server2_time_weeks]|escape:'html':'UTF-8'} - {$hours[$row.server2_time_hours]|escape:'html':'UTF-8'}{/if}
                                                        {if $row.server2_schedule == 'monthly'}{foreach from=$months item='item'}{if $item.id_option == $row.server2_time_months}{$item.label|escape:'html':'UTF-8'}{/if}{/foreach} - {$hours[$row.server2_time_hours]|escape:'html':'UTF-8'}{/if}
                                                    </span></li>
                                                {/if}
                                            </ul>{else}{l s='--' mod='ets_ordermanager'}{/if}
                                        {elseif $key == 'export_to_server1'}
                                            {if $row.export_to_server1}<ul class="list_table server1">
                                            <li><span class="li_item">{l s='Directory path:' mod='ets_ordermanager'} /{$row.directory_path1|escape:'html':'UTF-8'}</span></li>
                                                ...
                                                {if $row.server1_schedule != 'hourly'}
                                                    <li class="li_item"><span>{l s='Send on (at):' mod='ets_ordermanager'}
                                                        {if $row.server1_schedule == 'daily'}{$hours[$row.server1_time_hours]|escape:'html':'UTF-8'}{/if}
                                                        {if $row.server1_schedule == 'weekly'}{$weeks[$row.server1_time_weeks]|escape:'html':'UTF-8'} - {$hours[$row.server1_time_hours]|escape:'html':'UTF-8'}{/if}
                                                        {if $row.server1_schedule == 'monthly'}{foreach from=$months item='item'}{if $item.id_option == $row.server1_time_months}{$item.label|escape:'html':'UTF-8'}{/if}{/foreach} - {$hours[$row.server2_time_hours]|escape:'html':'UTF-8'}{/if}
                                                    </span></li>
                                                {/if}
                                            </ul>{else}{l s='--' mod='ets_ordermanager'}{/if}
                                            ...
                                            <div class="btn-group pull-right">
                                                ...
                                                <ul class="dropdown-menu">
                                                    ...
                                                    {if $row.send_file_via_email}
                                                        <li><a href="{$currentIndex|cat:'&submitSendMail&'|cat : $identifier|cat :'='|cat: $row.$identifier|escape:'html':'UTF-8'}"><i class="icon-envelope"></i> {l s='Send via email' mod='ets_ordermanager'}</a></li>
                                                    {/if}
                                                    {if $row.export_to_server2}
                                                        <li><a href="{$currentIndex|cat:'&submitSendFTP&'|cat : $identifier|cat :'='|cat: $row.$identifier|escape:'html':'UTF-8'}"><i class="icon-file-o"></i> {l s='Send via FTP' mod='ets_ordermanager'}</a></li>
                                                    {/if}
                                                    {if $row.export_to_server1}
                                                        <li><a href="{$currentIndex|cat:'&submitSaveFile&'|cat : $identifier|cat :'='|cat: $row.$identifier|escape:'html':'UTF-8'}"><i class="icon-save"></i> {l s='Save to web directory' mod='ets_ordermanager'}</a></li>
                                                    {/if}
                                                    <li><a  href="{$currentIndex|cat:'&update'|cat: $table|escape:'html':'UTF-8'}&amp;{$identifier|escape:'html':'UTF-8'}={$row.$identifier|escape:'html':'UTF-8'}"><i class="icon-pencil"></i> {l s='Edit' mod='ets_ordermanager'}</a></li>
                ...
                {if !$field_values}
                <div style="width: 100%; margin-top: 20px;" class="alert alert-info">{l s='No order export rules available. Click on "Add new rule" button to add your first order export rule.' mod='ets_ordermanager'}</div>
                {/if}
            </form>
        </div>
    {/if}
