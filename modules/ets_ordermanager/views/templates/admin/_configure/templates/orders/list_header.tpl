...
<div class="alert alert-warning" id="{$list_id|escape:'html':'UTF-8'}-empty-filters-alert" style="display:none;">{l s='Please fill in at least one field to perform a search in this list.' mod='ets_ordermanager'}</div>
{if isset($sql) && $sql}
...
{block name="startForm"}
	<form method="post" action="{$link->getAdminLink('AdminOrders')|escape:'html':'UTF-8'}{if isset($smarty.get.viewtrash)}&viewtrash=1{/if}" class="form-horizontal clearfix" id="{$list_id|escape:'html':'UTF-8'}_filter_form">
{/block}
		...
		<div class="panel-heading">
			...
			{if isset($toolbar_btn) && count($toolbar_btn) >0}
				...
				<span class="panel-heading-action">
                    {if !(isset($smarty.get.viewtrash) || isset($smarty.post.viewtrash))}
                        <a id="desc-{$table|escape:'html':'UTF-8'}-arrange" class="list-toolbar-btn arrange_order_list" href="{$link->getAdminLink('AdminOrders')|escape:'html':'UTF-8'}&arrangeorder=1">
        					<span title="" data-toggle="tooltip" class="label-tooltip" data-original-title="{l s='Customize order list' mod='ets_ordermanager'}" data-html="true" data-placement="top">
						...
						<a id="desc-{$table|escape:'html':'UTF-8'}-viewtrash" class="list-toolbar-btn" href="{$link->getAdminLink('AdminOrders')|escape:'html':'UTF-8'}&viewtrash=1">
        					<span title="" data-toggle="tooltip" class="label-tooltip" data-original-title="{l s='View trash' mod='ets_ordermanager'}" data-html="true" data-placement="top">
        						<i class="icon icon-trash"></i>
        					</span>
        				</a>
						<a id="desc-{$table|escape:'html':'UTF-8'}-export" class="list-toolbar-btn" href="{$link->getAdminLink('AdminOrderManagerExports')|escape:'html':'UTF-8'}">
							<span title="" data-toggle="tooltip" class="label-tooltip" data-original-title="{l s='Export orders by rule' mod='ets_ordermanager'}" data-html="true" data-placement="top">
        				...
        				{/foreach}
        					<a class="list-toolbar-btn" href="javascript:location.reload();">
        						<span title="" data-toggle="tooltip" class="label-tooltip" data-original-title="{l s='Refresh list' mod='ets_ordermanager'}" data-html="true" data-placement="top">
        				...
        				{if isset($sql) && $sql}
        					...
        					{if $sql_manager.view == 1}
        						<a class="list-toolbar-btn" href="javascript:void(0);" onclick="$('.leadin').first().append('<div class=\'alert alert-info\'>' + $('#sql_query_{$list_id|escape:'html':'UTF-8'}').val() + '</div>'); $(this).attr('onclick', '');">
        							<span class="label-tooltip" data-toggle="tooltip" data-original-title="{l s='Show SQL query' mod='ets_ordermanager'}" data-html="true" data-placement="top" >
        						...
        						<a class="list-toolbar-btn" href="javascript:void(0);" onclick="$('#sql_name_{$list_id|escape:'html':'UTF-8'}').val(createSqlQueryName()); $('#sql_query_{$list_id|escape:'html':'UTF-8'}').val($('#sql_query_{$list_id|escape:'html':'UTF-8'}').val().replace(/\s+limit\s+[0-9,\s]+$/ig, '').trim()); $('#sql_form_{$list_id|escape:'html':'UTF-8'}').submit();">
        							<span class="label-tooltip" data-toggle="tooltip" data-original-title="{l s='Export to SQL Manager' mod='ets_ordermanager'}" data-html="true" data-placement="top" >
					...
					if (btn_save.length > 0 && btn_submit.length > 0) {
						...
						if (btn_save_and_stay.length > 0) {
							...
							if (btn_submit.val().length > 0 && lbl_save_and_stay && !lbl_save_and_stay.hasClass('locked')) {
								lbl_save_and_stay.html(btn_submit.val() + " {l s='and stay' mod='ets_ordermanager'} ");
							}
						}
...
{/if}
	...
	<div class="table-responsive table-responsive-row clearfix{if isset($use_overflow) && $use_overflow} overflow-y{/if}">
					...
					{if isset($multishop_active) && $multishop_active && $shop_link_type}
						<th>
							...
							{if $shop_link_type == 'shop'}
								{l s='Shop' mod='ets_ordermanager'}
							{else}
								{l s='Shop group' mod='ets_ordermanager'}
							{/if}
							</span>
						</th>
					{/if}
					...
					{foreach $fields_display AS $key => $params}
						<th {if isset($params.align)} class="{$params.align|escape:'html':'UTF-8'}" {/if}>
                          ...
                          name="{$list_id|escape:'html':'UTF-8'}Filter_{if isset($params.filter_key)}{$params.filter_key|escape:'html':'UTF-8'}{else}{$key|escape:'html':'UTF-8'}{/if}">
										<option value="">-</option>
										<option value="1" {if $params.value == 1} selected="selected" {/if}>{l s='Yes' mod='ets_ordermanager'}</option>
										<option value="0" {if $params.value == 0 && $params.value != ''} selected="selected" {/if}>{l s='No' mod='ets_ordermanager'}</option>
									</select>
								{elseif $params.type == 'date' || $params.type == 'datetime'}
									<div class="date_range row">
 										<div class="input-group fixed-width-md center">
											<input type="text" class="filter datepicker date-input form-control" id="local_{$params.id_date|escape:'html':'UTF-8'}_0" name="local_{$params.name_date|escape:'html':'UTF-8'}[0]"  placeholder="{l s='From' mod='ets_ordermanager'}" />
 										...
 										<div class="input-group fixed-width-md center">
											<input type="text" class="filter datepicker date-input form-control" id="local_{$params.id_date|escape:'html':'UTF-8'}_1" name="local_{$params.name_date|escape:'html':'UTF-8'}[1]"  placeholder="{l s='To' mod='ets_ordermanager'}" />
						...
						<th class="actions">
							...
							<span class="pull-right">
								...
								<button type="submit" id="submitFilterButton{$list_id|escape:'html':'UTF-8'}" name="submitFilter" class="btn btn-default" data-list-id="{$list_id|escape:'html':'UTF-8'}">
									<i class="icon-search"></i> {l s='Search' mod='ets_ordermanager'}
								</button>
								{if $filters_has_value}
									<button type="submit" name="submitReset{$list_id|escape:'html':'UTF-8'}" class="btn btn-warning">
										<i class="icon-eraser"></i> {l s='Reset' mod='ets_ordermanager'}
									</button>
								{/if}
							</span>
							{/if}
