...
<div class="wk-sample-block" style="padding-top: 25px;clear: both;">
    <div class="alert alert-danger" id="wk_sp_standard_product_error" {if !isset($standardAdded) || !$standardAdded}
    ...
    <span class="control-label">
        {if isset($samplePrice) && isset($sampleOrgPrice)}
            {l s='Sample price' mod='wksampleproduct'} : <span class="product-price">{if $sampleOrgPrice == 0}
                    {l s='Free' mod='wksampleproduct'}
                {else}{$samplePrice|escape:'htmlall':'UTF-8'}
                    {if (($sample.price_type == 4) && ($sample.price_tax == 0)) || ($isTaxExclDisplay|escape:'htmlall':'UTF-8')}({l s='Tax excluded' mod='wksampleproduct'}){else}({l s='Tax included' mod='wksampleproduct'}){/if}{/if}
                </span>
            {/if}
        </span>
        ...
        <div class="product-quantity clearfix" {if isset($standardAdded) && $standardAdded} style="display:none" {/if}
            style="display:block">
            {if $wkShowQtySpin}
            <div class="qty {if isset($isHummingBirdThemeEnable) && $isHummingBirdThemeEnable}wk-qty-button{/if}">
                    {if isset($isHummingBirdThemeEnable) && $isHummingBirdThemeEnable}
                        <div class="product-actions__quantity quantity-button js-quantity-button col-md-auto">
                            <div class="input-group flex-nowrap mb-3">
                                <button role="button" aria-label="decrement"
                                    class="btn decrement js-decrement-button wkjs-touchspin" type="button">
                                    <i class="material-icons" aria-hidden="true"></i>
                                    <i class="material-icons confirmation d-none"></i>
                                    <div class="spinner-border spinner-border-sm align-middle d-none" role="status"></div>
                                </button>

                                <input type="text" name="wkqty" id="wkquantity_wanted" min="1" value="1"
                                    class="input-group form-control" style="display: block;">

                                <button role="button" aria-label="increment"
                                    class="btn increment js-increment-button wkjs-touchspin" type="button">
                                    <i class="material-icons" aria-hidden="true"></i>
                                    <i class="material-icons confirmation d-none"></i>
                                    <div class="spinner-border spinner-border-sm align-middle d-none" role="status"></div>
                                </button>
            ...
            {/if}
            <button class="btn btn-primary add-to-cart" id="wksamplebuybtn"
                ...
                data-cart-url="{$cartPageURL|escape:'htmlall':'UTF-8'}" {if $sampleFullInCart}disabled{/if}
                ...
                {else}{$sample.button_label}
                {/if}
            </button>
        </div>
        <span id="wksampleproductqty_spinerror" class="wksampleproduct-lineerror w-100"
            {if !$sampleQtyWarning}style="display:none;" {/if}>
            <i class="material-icons wkproduct-unavailable"></i>
            {l s='You can buy maximum ' mod='wksampleproduct'}{$sample.max_cart_qty|escape:'htmlall':'UTF-8'}
            {l s='samples.' mod='wksampleproduct'}
        </span>
