...
class WkSampleProduct extends Module
{
        ...
        if (Tools::isSubmit('submitGlobalSample')) {
            ...
            if (Tools::getValue('WK_GLOBAL_SAMPLE')) {
                ...
                if ($priceType == 2) {
                    ...
                    } elseif (!Validate::isPercentage((float) $samplePercent)) {
                        $this->context->controller->errors[] =
                ...
                if (($sampleWeight && !Validate::isFloat((float) $sampleWeight)) || ((float) $sampleWeight < 0)) {
                    $this->context->controller->errors[] =
    ...
    public function renderForm()
    {
        ...
        $carriers = json_decode(stripslashes(Configuration::get('WK_GLOBAL_SAMPLE_CARRIERS')));
    ...
    public function displaySampleButton($product, $isQuickView = false)
    {
        ...
        if ($sample && $sample['active']) {
            ...
            if ($samplePrice || ((string) $samplePrice == '0')) {
                ...
                $this->context->smarty->assign('samplePrice', WkSampleProduct::displayPrice($samplePrice));
            }
            ...
            if ($sampleCart) {
                ...
                if ($sample['price_type'] == 1) {
                    ...
                    $this->context->smarty->assign('samplePrice', WkSampleProduct::displayPrice($productPrice));
                }
            ...
            $this->context->smarty->assign([
                ...
                'isHummingBirdThemeEnable' => $this->isHummingBirdThemeEnable(),
            ]);
    ...
    public function hookActionOrderGridDefinitionModifier(array $params)
    {
        ...
        // Decide which location class to use based on PrestaShop version
        if (class_exists(\PrestaShop\PrestaShop\Core\Grid\Column\Type\Common\DataColumn::class)) {
            $columnClass = \PrestaShop\PrestaShop\Core\Grid\Column\Type\Common\DataColumn::class;
        } else {
            $columnClass = \PrestaShop\PrestaShop\Core\Grid\Column\Type\DataColumn::class;
        }
     ...
     *
     * @return string HTML output of the template
     */
        ...
        if ($idProduct) {
            ...
            if ($isNotglobalSample) {
                $this->context->smarty->assign([
                    ...
                    'isSample' => $isSample,
                    'productPrice' => WkSampleProduct::displayPrice($product->getPrice(false)),
                ]);
            }
            $sampleExists = false;
            ...
            $id_product_download = $product_download->getIdFromIdProduct($idProduct);
    ...
    public function validationBeforeProductSave()
    {
        ...
        if ($useGlobal == 1) {
            ...
            if (($weight && !Validate::isFloat((float) $weight)) || ((float) $weight < 0)) {
    ...
    public function hookActionSampleProductAddInCart($params)
    {
        ...
        if (version_compare(_PS_VERSION_, '1.7.5', '<')) {
            ...
            return (
                $sampleCart
                || (
                    isset($cookie->sampleProductId, $cookie->sampleProductIdAttr)
                    && $cookie->sampleProductId == $idProduct
                    && $cookie->sampleProductIdAttr == $idAttr
                )
            ) ? true : false;
        }
    }
    ...
    public function getSampleCartInformations($idCart, $idProducts = [])
    {
        ...
        $cartSamples = WkSampleCart::getCartProduct($idProducts, $idCart);
        if (!empty($cartSamples)) {
            ...
            $sampleWeights = WkSampleCart::getSampleWeight($idProductSamples);
        ...
        if (Tools::isSubmit('submitReorder') && ($idOrder = Tools::getValue('id_order'))) {
            ...
            if ($objSampleMap->getSampleProduct($product->id)) {
                ...
                if ($sampleOrder) {
                    ...
                    $this->context->cookie->__set('sampleProductId', $product->id);
                    // $this->context->cookie->sampleProductIdAttr = (int) $params['id_product_attribute'];
                    $this->context->cookie->__set('sampleProductIdAttr', (int) $params['id_product_attribute']);
                }
            }
        }
            ...
            if ($params['operator'] == 'up') {
                ...
                && !$isSampleProduct
                    ...
                    if ($sampleProductUpdate) {
                        ...
                        $objSampleCart = new WkSampleCart();
                        $sampleCarts = SpecificPrice::getIdsByProductId($product->id, $params['id_product_attribute'], $this->context->cart->id);
                        if (!empty($sampleCarts)) {
                            foreach ($sampleCarts as $sampleCart) {
                                ...
                                if ($sampleProductUpdate['price_type'] == 1) {
                                    $specificPrice->price = $productPrice;
                                } elseif ($sampleProductUpdate['price_type'] == 2) {
                                    $specificPrice->price = (float) $productPrice - (float) $sampleProductUpdate['amount'];
                                } elseif ($sampleProductUpdate['price_type'] == 3) {
                                    $specificPrice->price = (float) $productPrice - (float) $productPrice * (float) $sampleProductUpdate['amount'] / 100;
                                } elseif ($sampleProductUpdate['price_type'] == 4) {
                                    $specificPrice->price = (float) $sampleProductUpdate['price'];
    ...
    public function hookDisplayOrderDetail($params)
    {
        ...
        if (!empty($sample)) {
            foreach ($sample as &$product) {
                $product['sample_price'] = WkSampleProduct::displayPrice($product['total_price_tax_incl']);
            }
            $this->context->smarty->assign([
                'sample' => $sample,
        ...
        if ($this->context->controller->php_self == 'cart') {
            ...
            if ($isSampleProduct) {
                return $this->context->smarty->fetch(
                    ...
                    ['showNotice' => $this->isHummingBirdThemeEnable()]
                );
            }
        }
    }
    ...
    public function installTab($className, $tabName, $tabParentName = false)
    {
        ...
        if ($tabParentName) {
            if (version_compare(_PS_VERSION_, '9.0.0', '>=')) {
                ...
                $tabId = Tab::getIdFromClassName($tabParentName);
            }
    ...
    public function isHummingBirdThemeEnable()
    {
        $context = Context::getContext();
        $foTheme = $context->shop->theme->getName();

        if ($foTheme == 'hummingbird') {
            return true;
        }
    ...
    public function installOverrideFiles()
    {
        ...
        if (file_exists($wkDestinationPath)) {
            WkSampleProduct::deleteFolderContents($wkDestinationPath);
        } else {
            mkdir($wkDestinationPath, 0755, true);
        }

        WkSampleProduct::copyFolderContents($wkSourcePath, $wkDestinationPath);
