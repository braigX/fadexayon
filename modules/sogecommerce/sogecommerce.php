...
require_once(_PS_MODULE_DIR_ . 'sogecommerce/autoload.php');
require_once(_PS_MODULE_DIR_ . 'sogecommerce/classes/sdk-autoload.php');
...
class Sogecommerce extends PaymentModule
{
    ...
    public function install()
    {
        ...
        $installError = false;
        if (! defined('SOGECOMMERCE_MODULE_UPGRADE')) {
        ...
        $admin_config_params = SogecommerceTools::getAdminParameters();

        if (defined('SOGECOMMERCE_MODULE_UPGRADE')) {
            $results = Db::getInstance()->executeS(
                'SELECT `name` FROM `' . _DB_PREFIX_ . "configuration` WHERE `name` LIKE 'SOGECOMMERCE_%' AND `name` NOT LIKE 'SOGECOMMERCE_OS_%'"
            );
        ...
        foreach ($admin_config_params as $param) {
            ...
            if (! defined('SOGECOMMERCE_MODULE_UPGRADE') || ! in_array($key, $already_installed_params)) {
                if (in_array($key, SogecommerceTools::$multi_lang_fields)) {
                    $default = SogecommerceTools::convertIsoArrayToIdArray($param['default']);
                } elseif (is_array($param['default'])) {
                    $default = serialize($param['default']);
                } elseif (defined('SOGECOMMERCE_TRANSIENT_DEFAULT')) {
                    $defaults = unserialize(SOGECOMMERCE_TRANSIENT_DEFAULT);
        ...
        if (defined('SOGECOMMERCE_MODULE_UPGRADE') && ! empty($already_installed_params)) {
        ...
        if (SogecommerceTools::$plugin_features['oney'] && ! Configuration::get('SOGECOMMERCE_OS_ONEY_PENDING')) {
            ...
            $name = array(
                ...
                'br' => 'Solicitação de financiamento em andamento',
                'pt' => 'Solicitação de financiamento em andamento'
            );

            $oney_state = new OrderState();
            $oney_state->name = SogecommerceTools::convertIsoArrayToIdArray($name);
            ...
            if (! $oney_state->save() || ! Configuration::updateGlobalValue('SOGECOMMERCE_OS_ONEY_PENDING', $oney_state->id)) {
            ...
            @copy(
                _PS_MODULE_DIR_ . 'sogecommerce/views/img/os_oney.gif',
                _PS_IMG_DIR_ . 'os/' . Configuration::get('SOGECOMMERCE_OS_ONEY_PENDING') . '.gif'
            );
        }

        if (! Configuration::get('SOGECOMMERCE_OS_TO_VALIDATE')) {
            ...
            $name = array(
                ...
                'br' => 'Pagamento para validar',
                'pt' => 'Pagamento para validar'
            );

            $tvp_state = new OrderState();
            $tvp_state->name = SogecommerceTools::convertIsoArrayToIdArray($name);
            ...
            if (! $tvp_state->save() || ! Configuration::updateGlobalValue('SOGECOMMERCE_OS_TO_VALIDATE', $tvp_state->id)) {
            ...
            // Add small icon to state.
            @copy(
                _PS_MODULE_DIR_ . 'sogecommerce/views/img/os_tvp.gif',
                _PS_IMG_DIR_ . 'os/' . Configuration::get('SOGECOMMERCE_OS_TO_VALIDATE') . '.gif'
            );
        }

        if (! Configuration::get('PS_OS_OUTOFSTOCK_PAID') && ! Configuration::get('SOGECOMMERCE_OS_PAYMENT_OUTOFSTOCK')) {
            ...
            $name = array(
                ...
                'br' => 'Reposição em andamento (pagamento aceito)',
                'pt' => 'Reposição em andamento (pagamento aceito)'
            );

            $oos_state = new OrderState();
            $oos_state->name = SogecommerceTools::convertIsoArrayToIdArray($name);
            ...
            if (! $oos_state->save() || ! Configuration::updateGlobalValue('SOGECOMMERCE_OS_PAYMENT_OUTOFSTOCK', $oos_state->id)) {
            ...
            // Add small icon to state.
            @copy(
                _PS_MODULE_DIR_ . 'sogecommerce/views/img/os_oos.gif',
                _PS_IMG_DIR_ . 'os/' . Configuration::get('SOGECOMMERCE_OS_PAYMENT_OUTOFSTOCK') . '.gif'
            );
        }

        if (! Configuration::get('SOGECOMMERCE_OS_AUTH_PENDING')) {
            ...
            $name = array(
                ...
                'br' => 'Autorização em andamento',
                'pt' => 'Autorização em andamento'
            );

            $auth_state = new OrderState();
            $auth_state->name = SogecommerceTools::convertIsoArrayToIdArray($name);
            ...
            if (! $auth_state->save() || ! Configuration::updateGlobalValue('SOGECOMMERCE_OS_AUTH_PENDING', $auth_state->id)) {
            ...
            // Add small icon to state.
            @copy(
                _PS_MODULE_DIR_ . 'sogecommerce/views/img/os_auth.gif',
                _PS_IMG_DIR_ . 'os/' . Configuration::get('SOGECOMMERCE_OS_AUTH_PENDING') . '.gif'
            );
        }

        if ((SogecommerceTools::$plugin_features['sofort'] || SogecommerceTools::$plugin_features['sepa'])
            && ! Configuration::get('SOGECOMMERCE_OS_TRANS_PENDING')) {
            ...
            $name = array(
                ...
                'br' => 'Transferência pendente',
                'pt' => 'Transferência pendente'
            );

            $sofort_state = new OrderState();
            $sofort_state->name = SogecommerceTools::convertIsoArrayToIdArray($name);
            ...
            if (! $sofort_state->save() || ! Configuration::updateGlobalValue('SOGECOMMERCE_OS_TRANS_PENDING', $sofort_state->id)) {
            ...
            // Add small icon to state.
            @copy(
                _PS_MODULE_DIR_ . 'sogecommerce/views/img/os_trans.gif',
                _PS_IMG_DIR_ . 'os/' . Configuration::get('SOGECOMMERCE_OS_TRANS_PENDING') . '.gif'
            );
        }

        if (! Configuration::get('SOGECOMMERCE_OS_REFUNDED')) {
            // Create to validate payment order state.
            $name = array(
                'en' => 'Refunded with Sogecommerce',
                'fr' => 'Remboursé avec Sogecommerce',
                'de' => 'Rückerstattet mit Sogecommerce',
                'es' => 'Reembolsado con Sogecommerce',
                'br' => 'Reembolsado com Sogecommerce',
                'pt' => 'Reembolsado com Sogecommerce'
            );

            $refund_state = new OrderState();
            $refund_state->name = SogecommerceTools::convertIsoArrayToIdArray($name);
            ...
            if (! $refund_state->save() || ! Configuration::updateGlobalValue('SOGECOMMERCE_OS_REFUNDED', $refund_state->id)) {
                $this->logger->logWarning('Error while creating customized order status «Refunded with Sogecommerce».');

                $this->_errors[] = sprintf($this->l('Error while creating customized order status « %s ».'), sprintf($this->l('Refunded with %s'), 'Sogecommerce'));
                $installError = true;
            }

            // Add small icon to state.
            @copy(
                _PS_MODULE_DIR_ . 'sogecommerce/views/img/os_refund.gif',
                _PS_IMG_DIR_ . 'os/' . Configuration::get('SOGECOMMERCE_OS_REFUNDED') . '.gif'
            );
        }
    ...
    {
        $result = true;
        foreach (SogecommerceTools::getAdminParameters() as $param) {
            $result &= Configuration::deleteByName($param['key']);
        }

        // Delete all obsolete gateway params but not custom order states.
        $result &= Db::getInstance()->execute(
            'DELETE FROM `' . _DB_PREFIX_ . "configuration` WHERE `name` LIKE 'SOGECOMMERCE_%' AND `name` NOT LIKE 'SOGECOMMERCE_OS_%'"
        );
    ...
    public function getContent()
    {
        ...
        if (isset($this->context->cookie->sogecommerceEmailSendMsg)) {
            $msg = $this->context->cookie->sogecommerceEmailSendMsg;
            unset($this->context->cookie->sogecommerceEmailSendMsg);
        }

        if (Tools::isSubmit('sogecommerce_submit_admin_form')) {
            $this->postProcess();
            ...
            $msg .= '<br />';
        } else if (Tools::getValue('submitter') === 'sogecommerce_send_support') {
            if (Tools::getValue('sender') && Tools::getValue('subject') && Tools::getValue('message')) {
                ...
                if ($this->sendEmail($email)) {
                   if (Tools::getValue('sogecommerce_mail_origine') === 'order') {
                       // Display success message in Order details page.
                       $this->context->cookie->sogecommerceMessageSuccessSent = $this->l('Thank you for contacting us. Your email has been successfully sent.');
                       die();
                   } else {
                    ...
                    if (Tools::getValue('sogecommerce_mail_origine') === 'order') {
                        ...
                        $this->context->cookie->sogecommerceMessageErrorSent = $this->l('An error has occurred. Your email was not sent.');
                        die();
                    } else {
                ...
                $this->context->cookie->sogecommerceEmailSendMsg = $msg;
            } else {
                if (Tools::getValue('sogecommerce_mail_origine') === 'order') {
                    // Display error message in Order details page.
                    $this->context->cookie->sogecommerceMessageErrorSent = $this->l('Please make sure to configure all required fields.');
                    die();
                } else {
                    $this->context->cookie->sogecommerceEmailSendMsg = $this->displayError($this->l('Please make sure to configure all required fields.'));
                }
            }
        }
    ...
    private function sendEmail($email)
    {
        return Mail::Send(
            ...
            'sogecommerce', // Email template file to be use.
            $email['subject'], // Email subject.
            array(
                '{email}' => $email['sender'], // Sender email address.
                '{message}' => $email['message'] // Email content.
            ),
            SogecommerceTools::getDefault('SUPPORT_EMAIL'), // Receiver email address.
            NULL, // Receiver name.
    ...
    {
        $request = new Lyranetwork\Sogecommerce\Sdk\Form\Request(); // New instance of Lyranetwork\Sogecommerce\Sdk\Form\Request for parameters validation.
        ...
        foreach (SogecommerceTools::getAdminParameters() as $param) {
            ...
            if (in_array($key, SogecommerceTools::$multi_lang_fields)) {
            ...
            } elseif (in_array($key, SogecommerceTools::$group_amount_fields)) {
                if (! is_array($value) || empty($value)) {
                    $value = array();
                } else {
                    $error = false;
                    foreach ($value as $id => $option) {
                        if (($key === 'SOGECOMMERCE_CHOOZEO_OPTIONS') && ! isset($option['enabled'])) {
            ...
            } elseif ($key === 'SOGECOMMERCE_MULTI_OPTIONS') {
                if (! is_array($value) || empty($value)) {
                    $value = array();
                } else {
                    $error = false;
                    foreach ($value as $id => $option) {
                        if (($option['min_amount'] && (! is_numeric($option['min_amount']) || $option['min_amount'] < 0))
                            ...
                            $default = is_string($option['label']) && $option['label'] ? $option['label'] : $option['count'] . ' x';
                ...
                $value = serialize($value);
            } elseif ($key === 'SOGECOMMERCE_AVAILABLE_LANGUAGES') {
                $value = (is_array($value) && ! empty($value)) ? implode(';', $value) : '';
            } elseif ($key === 'SOGECOMMERCE_STD_PAYMENT_CARDS' || $key === 'SOGECOMMERCE_MULTI_PAYMENT_CARDS') {
                ...
                $value = implode(';', $value);
            } elseif ($key === 'SOGECOMMERCE_ONEY_SHIP_OPTIONS') {
                if (! is_array($value) || empty($value)) {
                    $value = array();
                } else {
                    foreach ($value as $id => $option) {
                ...
                $value = serialize($value);
            } elseif ($key === 'SOGECOMMERCE_CATEGORY_MAPPING') {
                if (Tools::getValue('SOGECOMMERCE_COMMON_CATEGORY', null) !== 'CUSTOM_MAPPING') {
                    continue;
                }
                ...
                $value = serialize($value);
            } elseif (($key === 'SOGECOMMERCE_ONEY34_ENABLED') && ($value === 'True')) {
            ...
            } elseif (in_array($key, SogecommerceTools::$amount_fields)) {
                if (! empty($value) && (! is_numeric($value) || $value < 0)) {
                    $this->_errors[] = sprintf($this->l('Invalid value « %1$s » for field « %2$s ».'), $value, $label);
                    continue;
                }
            } elseif ($key === 'SOGECOMMERCE_ONEY34_OPTIONS') {
                if (! is_array($value) || empty($value)) {
                    $value = array();
                } else {
                    $error = false;
                    foreach ($value as $id => $option) {
                        if ((isset($option['count']) && ! is_numeric($option['count'])) && ($option['card_type'] !== 'ONEY_PAYLATER') || ! is_numeric($option['rate']) || empty($option['code'])) {
                            ...
                            if ($option['card_type'] === 'ONEY_PAYLATER') {
                                $default = is_string($option['label']) && $option['label'] ? $option['label'] : 'Pay Later Oney';
                            } else {
                                $default = is_string($option['label']) && $option['label'] ? $option['label'] : $option['count'] . ' x';
                            }
                ...
                $value = serialize($value);
            } elseif ($key === 'SOGECOMMERCE_FFIN_OPTIONS') {
                if (! is_array($value) || empty($value)) {
                    $value = array();
                } else {
                    $error = false;
                ...
                $value = serialize($value);
            } elseif (($key === 'SOGECOMMERCE_FULLCB_OPTIONS') && (Tools::getValue('SOGECOMMERCE_FULLCB_ENABLE_OPTS', 'False') === 'True')) {
                if (! is_array($value) || empty($value)) {
                    $value = array();
                } else {
                    $error = false;
                ...
                $value = serialize($value);
            } elseif ($key === 'SOGECOMMERCE_OTHER_PAYMENT_MEANS') {
                if (! is_array($value) || empty($value)) {
                    $value = array();
                } else {
                    $error = false;
                    ...
                    $cards = Lyranetwork\Sogecommerce\Sdk\Form\Api::getSupportedCardTypes();

                    // Add extra means of payment to supported payment means.
                    $extra_cards = @unserialize(Configuration::get('SOGECOMMERCE_EXTRA_PAYMENT_MEANS'));
                    ...
                    foreach ($value as $id => $option) {
                        ...
                        if ($option['min_amount'] && (! is_numeric($option['min_amount']) || $option['min_amount'] < 0)
                            || $option['max_amount'] && (! is_numeric($option['max_amount']) || $option['max_amount'] < 0)
                            || $option['min_amount'] && $option['max_amount'] && ($option['min_amount'] > $option['max_amount'])
                            || ! isset($option['embedded']) && $option['capture'] && ! is_numeric($option['capture'])) {
                            unset($value[$id]); // Error, do not save this option.
                            $error = true;
                        } else {
                ...
                $value = serialize($value);
            } elseif ($key === 'SOGECOMMERCE_EXTRA_PAYMENT_MEANS') {
                $used_cards = array_keys(Lyranetwork\Sogecommerce\Sdk\Form\Api::getSupportedCardTypes());
                if (! is_array($value) || empty($value)) {
                    $value = array();
                } else {
                    $error = false;
                ...
                // Update SOGECOMMERCE_OTHER_PAYMENT_MEANS (options containing deleted payment means should not appear).
                $other_payment_means = @unserialize(Configuration::get('SOGECOMMERCE_OTHER_PAYMENT_MEANS'));
                ...
                Configuration::updateValue('SOGECOMMERCE_OTHER_PAYMENT_MEANS', serialize($other_payment_means));

                $value = serialize($value);
            } elseif ($key === 'SOGECOMMERCE_STD_REST_PAYMENT_MEANS_GROUPING_THRESHOLD') {
                if ($value && (! is_numeric($value) || $value < 1)) {
                    $this->_errors[] = sprintf($this->l('Invalid value « %1$s » for field « %2$s ».'), $value, $label);
                    continue;
                }
            } elseif ($key === 'SOGECOMMERCE_STD_REST_PLACEHLDR') {
                $value = serialize($value);
            } elseif ($key === 'SOGECOMMERCE_STD_REST_ATTEMPTS') {
                if ($value && (! is_numeric($value) || $value < 0 || $value > 2)) {
                    $this->_errors[] = sprintf($this->l('Invalid value « %1$s » for field « %2$s ».'), $value, $label);
                    continue;
                }
            } elseif ($key === 'SOGECOMMERCE_REST_SERVER_URL' || $key === 'SOGECOMMERCE_REST_JS_CLIENT_URL') {
            ...
            // Validate with Lyranetwork\Sogecommerce\Sdk\Form\Request.
    ...
    {
        $label = $this->l('Oney payment', 'sogecommerceoney34payment');
        ...
        if (! $inside) {
            $key = 'SOGECOMMERCE_ONEY34_AMOUNTS';
            ...
            if (! Tools::getValue('SOGECOMMERCE_ONEY34_OPTIONS')) {
                return sprintf($this->l('The field « %s » is mandatory.'), $this->l('Oney payment - Payment options', 'back_office'));
            }
        }
    ...
    {
        $this->addJS('sogecommerce.js');

        if (SogecommerceTools::$plugin_features['support']) {
        ...
        $this->context->smarty->assign(SogecommerceHelperForm::getAdminFormContext());
        $form = $this->context->smarty->fetch(_PS_MODULE_DIR_ . 'sogecommerce/views/templates/admin/back_office.tpl');

        $prefered_post_vars = 0;
        $prefered_post_vars += substr_count($form, 'name="SOGECOMMERCE_');
    ...
    public function hookHeader($params)
    {
        ...
        if ($controller instanceof OrderController || $controller instanceof OrderOpcController || $controller instanceof SogecommerceWalletModuleFrontController) {
            if (isset($this->context->cookie->sogecommercePayErrors)) {
                // Process errors from other pages.
                $controller->errors = array_merge(
                    $controller->errors,
                    explode("\n", $this->context->cookie->sogecommercePayErrors)
                );
                unset($this->context->cookie->sogecommercePayErrors);
            ...
            if ($standard->isAvailable($this->context->cart) && $embedded) {
                $test_mode = Configuration::get('SOGECOMMERCE_MODE') === 'TEST';
                $pub_key = $test_mode ? Configuration::get('SOGECOMMERCE_PUBKEY_TEST') : Configuration::get('SOGECOMMERCE_PUBKEY_PROD');

                // URL where to redirect after payment.
                $return_url = $this->context->link->getModuleLink('sogecommerce', 'rest', array(), true);
                ...
                if (! Lyranetwork\Sogecommerce\Sdk\Form\Api::isSupportedLanguage($language_iso_code)) {
                    $language_iso_code = Configuration::get('SOGECOMMERCE_DEFAULT_LANGUAGE');
                }

                $html .= '<script>
                            var SOGECOMMERCE_LANGUAGE = "' . $language_iso_code . '";
                          </script>';

                $html .= '<script src="' . Configuration::get('SOGECOMMERCE_REST_JS_CLIENT_URL') . 'js/krypton-client/V4.0/stable/kr-payment-form.min.js"
                                  kr-public-key="' . $pub_key . '"
                                  kr-post-url-success="' . $return_url . '"
                                  kr-post-url-refused="' . $return_url . '"
                                  kr-language="' . $language_iso_code . '"
                                  kr-label-do-register="' . Configuration::get('SOGECOMMERCE_STD_REST_LBL_REGIST', $language['id_lang']) . '"';

                $rest_placeholders = @unserialize(Configuration::get('SOGECOMMERCE_STD_REST_PLACEHLDR'));
                ...
                $rest_theme = Configuration::get('SOGECOMMERCE_STD_REST_THEME');
                $html .= '<link rel="stylesheet" href="' . Configuration::get('SOGECOMMERCE_REST_JS_CLIENT_URL') . 'js/krypton-client/V4.0/ext/' . $rest_theme . '-reset.css">
                          <script src="' . Configuration::get('SOGECOMMERCE_REST_JS_CLIENT_URL') . 'js/krypton-client/V4.0/ext/' . $rest_theme . '.js"></script>';

                $this->context->smarty->assign('sogecommerce_rest_theme', $rest_theme);

                if (version_compare(_PS_VERSION_, '1.6', '>=')) {
                    $page = Configuration::get('PS_ORDER_PROCESS_TYPE') ? 'order-opc' : 'order';
                    Media::addJsDef(array('sogecommerce' => array('restUrl' => $return_url, 'pageType' => $page)));
                }
            ...
            if (version_compare(_PS_VERSION_, '1.7', '<')) {
                $this->addCss('sogecommerce_bc.css');
    ...
    public function hookDisplayCustomerAccount($params)
    {
        ...
        return $this->context->smarty->fetch(_PS_MODULE_DIR_ . 'sogecommerce/views/templates/hook/customer_account.tpl');
    }
    ...
    private function addCss($css_file)
    {
        ...
        if (method_exists($controller, 'registerStylesheet')) { // PrestaShop 1.7.
            $controller->registerStylesheet(
                'module-sogecommerce-' . basename($css_file, '.png'),
    ...
    public function hookPayment($params)
    {
        ...
        $customersConfig = @unserialize(Configuration::get('SOGECOMMERCE_CUSTOMERS_CONFIG'));

        // Version tag for specific styles.
        $tag = version_compare(_PS_VERSION_, '1.6', '<') ? 'sogecommerce15' : 'sogecommerce16';
        $this->context->smarty->assign('sogecommerce_tag', $tag);
        ...
        $oney34 = new SogecommerceOney34Payment();
        ...
        $franfinance = new SogecommerceFranfinancePayment();
        ...
        $other_payments = SogecommerceOtherPayment::getAvailablePaymentMeans($cart);
        if (Configuration::get('SOGECOMMERCE_OTHER_GROUPED_VIEW') === 'True' && count($other_payments) > 1) {
            $grouped = new SogecommerceGroupedOtherPayment();
    ...
    public function hookPaymentOptions($params)
    {
        ...
        // Version tag for specific styles.
        $this->context->smarty->assign('sogecommerce_tag', 'sogecommerce17');

        /**
         * AbstractSogecommercePayment::getPaymentOption() returns a payment option of type
        ...
        if ($standard->isAvailable($cart)) {
            ...
            $useCustomerWallet = $isRestPayment && (Configuration::get('SOGECOMMERCE_STD_USE_WALLET') == 'True');
            ...
            if ($standard->isOneClickActive() && ! $useCustomerWallet) {
                ...
                if ($isStandardValidAlias) {
                    ...
                    $additionalForm = $this->fetch('module:sogecommerce/views/templates/hook/payment_std_oneclick.tpl');
                    $option->setAdditionalInformation($additionalForm);
                }
            }

            if ($standard->hasForm() || $oneClickPayment) {
                $form = $this->fetch('module:sogecommerce/views/templates/hook/' . $standard->getTplName());

                if (($standard->isEmbedded() && $isRestPayment)) {
                    // REST mode.
                    $option->setAdditionalInformation($form . $additionalForm);
                    if ($oneClickPayment) {
                        $option->setForm('<form id="sogecommerce_standard" onsubmit="javascript: sogecommerceSubmit(event);"><input id="sogecommerce_payment_by_identifier" type="hidden" name="sogecommerce_payment_by_identifier" value="1" /></form>');
                    } else {
                        $option->setForm('<form id="sogecommerce_standard" onsubmit="javascript: sogecommerceSubmit(event);"></form>');
                    }
                } else {
                    $option->setForm($form);
                }
            } else if (Configuration::get('SOGECOMMERCE_STD_CARD_DATA_MODE') === '1' && Configuration::get('SOGECOMMERCE_STD_SELECT_BY_DEFAULT') == 'True') {
                    $additionalForm = $this->fetch('module:sogecommerce/views/templates/hook/payment_std_select_as_default.tpl');
                    $option->setAdditionalInformation($additionalForm);
            }
        ...
        if ($multi->isAvailable($cart)) {
            ...
            if ($multi->hasForm()) {
                $this->context->smarty->assign($multi->getTplVars($cart));
                $form = $this->fetch('module:sogecommerce/views/templates/hook/' . $multi->getTplName());
                $option->setForm($form);
            }
        ...
        if ($choozeo->isAvailable($cart)) {
            ...
            if ($choozeo->hasForm()) {
                $this->context->smarty->assign($choozeo->getTplVars($cart));
                $form = $this->fetch('module:sogecommerce/views/templates/hook/' . $choozeo->getTplName());
                $option->setForm($form);
            }
        ...
        if ($oney34->isAvailable($cart)) {
            ...
            if ($oney34->hasForm()) {
                $this->context->smarty->assign($oney34->getTplVars($cart));
                $form = $this->fetch('module:sogecommerce/views/templates/hook/' . $oney34->getTplName());
                $option->setForm($form);
            }
        ...
        if ($franfinance->isAvailable($cart)) {
            ...
            if ($franfinance->hasForm()) {
                $this->context->smarty->assign($franfinance->getTplVars($cart));
                $form = $this->fetch('module:sogecommerce/views/templates/hook/' . $franfinance->getTplName());
                $option->setForm($form);
            }
        ...
        if ($fullcb->isAvailable($cart)) {
            ...
            if ($fullcb->hasForm()) {
                $this->context->smarty->assign($fullcb->getTplVars($cart));
                $form = $this->fetch('module:sogecommerce/views/templates/hook/' . $fullcb->getTplName());
                $option->setForm($form);
            }
        ...
        if ($sepa->isAvailable($cart)) {
            ...
            if ($sepa->isOneClickActive()) {
                ...
                if ($isSepaValidAlias) {
                    $this->context->smarty->assign($sepa->getTplVars($cart));
                    $additionalForm = $this->fetch('module:sogecommerce/views/templates/hook/payment_sepa_oneclick.tpl');
                    $option->setAdditionalInformation($additionalForm);
                    $option->setForm(
                        '<form action="' . $this->context->link->getModuleLink('sogecommerce', 'redirect', array(), true) . '" method="post">
        ...
        if (Configuration::get('SOGECOMMERCE_OTHER_GROUPED_VIEW') === 'True' && count($other_payments) > 1) {
            ...
            if ($grouped->isAvailable($cart)) {
                ...
                if ($grouped->hasForm()) {
                    $this->context->smarty->assign($grouped->getTplVars($cart));
                    $form = $this->fetch('module:sogecommerce/views/templates/hook/' . $grouped->getTplName());
                    $option->setForm($form);
                }
    ...
    private function checkCurrency()
    {
        ...
        foreach ($currencies as $currency) {
            if ($cart_currency->id == $currency['id_currency']) {
                ...
                return Lyranetwork\Sogecommerce\Sdk\Form\Api::findCurrencyByAlphaCode($cart_currency->iso_code) != null;
            }
        }
    ...
    public function hookPaymentReturn($params)
    {
        ...
        if (! $error) {
            $array['total_to_pay'] = SogecommerceTools::formatPrice(
    ...
    {
        if (isset($this->context->cookie->sogecommerceRefundWarn)) {
            $this->context->controller->warnings[] = $this->context->cookie->sogecommerceRefundWarn;
            unset($this->context->cookie->sogecommerceRefundWarn);
        }
    }
    ...
    public function hookDisplayAdminOrderTop($params)
    {
        ...
        if (isset($this->context->cookie->sogecommerceMessageErrorSent)) {
            $this->get('session')->getFlashBag()->set('error', $this->context->cookie->sogecommerceMessageErrorSent);
            unset($this->context->cookie->sogecommerceMessageErrorSent);
            $this->context->cookie->write();

            Tools::redirectAdmin($url_admin_order);
        }

        if (isset($this->context->cookie->sogecommerceMessageSuccessSent)) {
            $this->get('session')->getFlashBag()->set('success', $this->context->cookie->sogecommerceMessageSuccessSent);
            unset($this->context->cookie->sogecommerceMessageSuccessSent);
            $this->context->cookie->write();
    ...
    public function hookDisplayAdminOrder($params)
    {
        ...
        if (isset($this->context->cookie->sogecommerceMessageErrorSent)) {
            $this->context->controller->errors[] = $this->context->cookie->sogecommerceMessageErrorSent;
            unset($this->context->cookie->sogecommerceMessageErrorSent);
        }

        if (isset($this->context->cookie->sogecommerceMessageSuccessSent)) {
            $this->context->controller->confirmations[] = $this->context->cookie->sogecommerceMessageSuccessSent;
            unset($this->context->cookie->sogecommerceMessageSuccessSent);
        }
    ...
    public function hookActionEmailSendBefore(array $params)
    {
        ...
        if (! isset($this->context->cookie->sogecommerceActionEmailSend)
            && isset($this->context->cookie->sogecommerceNewOrder)
            && $params['template'] === 'payment') {
            $this->logger->logInfo("Stop Order #{$order->id} payment email from being sent.");

            $this->context->cookie->sogecommerceActionEmailSend = true;
            unset($this->context->cookie->sogecommerceNewOrder);

            return false;
        }

        if ($params['template'] === 'payment') {
            unset($this->context->cookie->sogecommerceActionEmailSend);
    ...
    {
        $template = _PS_MODULE_DIR_ . 'sogecommerce/views/templates/admin/';
    ...
    private function displaySupportContactFromOrderDetails($order)
    {
        ...
        $tpl_vars = SogecommerceHelperForm::getAdminFormContext();
        $tpl_vars['trans_id_title'] = $this->l('Transaction UUID : ');
        $tpl_vars['sogecommerce_site_id'] = Configuration::get('SOGECOMMERCE_SITE_ID', null, $order->id_shop_group, $order->id_shop);
        $tpl_vars['sogecommerce_mode'] = Configuration::get('SOGECOMMERCE_MODE', null, $order->id_shop_group, $order->id_shop);
        $tpl_vars['sogecommerce_sign_algo'] = Configuration::get('SOGECOMMERCE_SIGN_ALGO', null, $order->id_shop_group, $order->id_shop);

        $card_data_mode = Configuration::get('SOGECOMMERCE_STD_CARD_DATA_MODE', null, $order->id_shop_group, $order->id_shop);
        $tpl_vars['sogecommerce_std_card_data_mode'] = $card_data_mode && $card_data_mode != '4' ? $card_data_mode : '1';
        ...
        $tpl_vars['total_paid'] = SogecommerceTools::formatPrice($order->total_paid, $order->id_currency, $this->context);
        $tpl_vars['total_products_wt'] = SogecommerceTools::formatPrice($order->total_products_wt, $order->id_currency, $this->context);
        $tpl_vars['total_shipping'] = SogecommerceTools::formatPrice($order->total_shipping, $order->id_currency, $this->context);
        $tpl_vars['total_discounts'] = SogecommerceTools::formatPrice($order->total_discounts, $order->id_currency, $this->context);
        ...
        // Recover POST uri.
        $sogecommerce_request_uri = $this->context->link->getAdminLink('AdminModules');
        $sogecommerce_request_uri = str_replace('&token=', '&configure=sogecommerce&token=', $sogecommerce_request_uri);
        $tpl_vars['sogecommerce_request_uri'] = $sogecommerce_request_uri;
        $this->context->smarty->assign($tpl_vars);

        return $this->context->smarty->fetch(_PS_MODULE_DIR_ . 'sogecommerce/views/templates/admin/support_contact_from_order_details.tpl');
    }
    ...
    public function hookActionOrderStatusUpdate($params)
    {
        ...
        if (SogecommerceTools::checkFormIpnValidity() || SogecommerceTools::checkRestIpnValidity()) {
            return;
        }

        $managedRefundStatuses = array((int) Configuration::get('SOGECOMMERCE_OS_REFUNDED'), (int) Configuration::get('PS_OS_CANCELED'));
        if (! in_array($params['newOrderStatus']->id, $managedRefundStatuses)) {
            return;
        }

        if ($order->total_paid_real <= 0 || $this->context->cookie->sogecommerceSplitPaymentUpdateRefundStatus === "True") {
            // Order already cancelled or refunded.
            $this->logger->logInfo("Order #{$order->id} was already cancelled or refunded.");
            unset($this->context->cookie->sogecommerceSplitPaymentUpdateRefundStatus);
            return;
        }

        // Update order status is manually changed to "Refunded with Sogecommerce or Canceled" (not by refund function).
        $this->context->cookie->sogecommerceManualUpdateToManagedRefundStatuses = 'True';

        $refund_processor = new SogecommerceRefundProcessor();

        $client = new Lyranetwork\Sogecommerce\Sdk\Refund\Api(
            $refund_processor,
            $this->getPrivateKey(),
            Configuration::get('SOGECOMMERCE_REST_SERVER_URL'),
            Configuration::get('SOGECOMMERCE_SITE_ID'),
            'PrestaShop'
        );

        $orderCurrency = new Currency((int) $order->id_currency);

        $order_refund_bean = new Lyranetwork\Sogecommerce\Sdk\Refund\OrderInfo();
        $order_refund_bean->setOrderRemoteId($order->id_cart);
        $order_refund_bean->setOrderId($order->id_cart);
        $order_refund_bean->setOrderReference($order->reference);
        $order_refund_bean->setOrderCurrencyIsoCode($orderCurrency->iso_code);
        $order_refund_bean->setOrderCurrencySign($orderCurrency->sign);
        $order_refund_bean->setOrderUserInfo($this->getUserInfo());

        // If any error during WS refund/cancel redirect to order details to avoid display success message.
        if (! $client->refund($order_refund_bean, $order->total_paid_real)) {
            if (version_compare(_PS_VERSION_, '1.7.7', '<')) {
                ...
                if (isset($this->context->cookie->sogecommerceRefundWarn)) {
                    $this->get('session')->getFlashBag()->set('warning', $this->context->cookie->sogecommerceRefundWarn);
                    unset($this->context->cookie->sogecommerceRefundWarn);
                }
        ...
        } elseif (version_compare(_PS_VERSION_, '1.7.7', '>=') && isset($this->context->cookie->sogecommerceRefundWarn)) {
            // Display warning to customer if any for Prestashop versions >= 1.7.7.
            $this->get('session')->getFlashBag()->set('warning', $this->context->cookie->sogecommerceRefundWarn);
            unset($this->context->cookie->sogecommerceRefundWarn);
        }
    ...
    {
        if (! Tools::isSubmit('doPartialRefundSogecommerce')
            && ! Tools::isSubmit('doStandardRefundSogecommerce')) {
            return;
        }
    ...
    private function getPrivateKey()
    {
        ...
        return $test_mode ? Configuration::get('SOGECOMMERCE_PRIVKEY_TEST') : Configuration::get('SOGECOMMERCE_PRIVKEY_PROD');
    }
    ...
    {
        if ((Configuration::get('SOGECOMMERCE_SEND_SHIP_DATA') === 'True') || (Configuration::get('SOGECOMMERCE_FFIN_ENABLED') === 'True') || (Configuration::get('SOGECOMMERCE_ONEY34_ENABLED') === 'True')) {
            ...
            $this->context->cookie->sogecommerceShippingOptionsWarn = $msg;
        }
    }
    ...
    {
        if (isset($this->context->cookie->sogecommerceShippingOptionsWarn)) {
            $this->context->controller->warnings[] = $this->context->cookie->sogecommerceShippingOptionsWarn;
            unset($this->context->cookie->sogecommerceShippingOptionsWarn);
        }
    }
     ...
     * @param Cart $cart
     * @param int $state
     * @param Lyranetwork\Sogecommerce\Sdk\Form\Response $response
     * @return Order
     */
    public function saveOrder($cart, $state, $response)
    {
        ...
        $currency = Lyranetwork\Sogecommerce\Sdk\Form\Api::findCurrency($response->get('currency'));
        ...
        $class_name = 'Sogecommerce' . SogecommerceTools::ucClassName($module_id) . 'Payment';
        if (! $module_id || ! class_exists($class_name)) {
            ...
            $class_name = 'SogecommerceStandardPayment';
        }

        $payment = new $class_name();

        // Specific case of "Other payment means" submodule.
        if (is_a($payment, 'SogecommerceOtherPayment')) {
            $method = SogecommerceOtherPayment::getMethodByCode($response->getExtInfo('payment_method'));
            $payment->init($method['code'], $method['title']);
        }

        if (is_a($payment, 'SogecommerceStandardPayment') && (Configuration::get('SOGECOMMERCE_STD_USE_PAYMENT_MEAN_AS_TITLE') === 'True')) {
        ...
        // To block first payment mail send only for Sogecommerce new orders.
        $this->context->cookie->sogecommerceNewOrder = true;
    ...
    public function createMessage($order, $response)
    {
        ...
        if ($authorized_amount = $response->get('authorized_amount')) {
            $currency = Lyranetwork\Sogecommerce\Sdk\Form\Api::findCurrencyByNumCode($response->get('currency'));
        ...
        if ((Configuration::get('SOGECOMMERCE_ENABLE_CUST_MSG') === 'True') && version_compare(_PS_VERSION_, '1.7.1.2', '>=')) {
    ...
    public function savePayment($order, $response, $force_stop_payment_creation = false)
    {
        ...
        if ($response->get('card_brand') === 'MULTI') {
            ...
            $transactions = array_filter($sequences->transactions, 'Sogecommerce::filterTransactions');
            ...
            $multi_options = SogecommerceMultiPayment::getAvailableOptions();
            $option = $multi_options[$option_id];
            ...
            if ($response->get('operation_type') === 'CREDIT') {
                if (version_compare(_PS_VERSION_, '1.7.7', '>=')) {
                    ...
                    if (! Tools::isSubmit('doPartialRefundSogecommerce') && ! Tools::isSubmit('doStandardRefundSogecommerce')) {
                        $orderCurrency = new Currency((int) $order->id_currency);
                        ...
                        try {
                            if ($this->sogecommerceCreateOrderSlip($order, $amount)) {
    ...
    public function saveIdentifier($customer, $response)
    {
        ...
        if ($response->get('identifier') && in_array($response->get('identifier_status'), array('CREATED', 'UPDATED'))) {
            ...
            $customers_config = @unserialize(Configuration::get('SOGECOMMERCE_CUSTOMERS_CONFIG'));
            ...
            Configuration::updateValue('SOGECOMMERCE_CUSTOMERS_CONFIG', serialize($customers_config));
    ...
    public function checkIdentifier($identifier, $customerEmail)
    {
        try {
            ...
            $client = new Lyranetwork\Sogecommerce\Sdk\Rest\Api(
                Configuration::get('SOGECOMMERCE_REST_SERVER_URL'),
                Configuration::get('SOGECOMMERCE_SITE_ID'),
                $this->getPrivateKey()
            );
            ...
            SogecommerceTools::checkRestResult($checkIdentifierResponse);

            $cancellationDate = SogecommerceTools::getProperty($checkIdentifierResponse['answer'], 'cancellationDate');
    ...
    public static function filterTransactions($trs)
    {
        $successful_states = array_merge(
            Lyranetwork\Sogecommerce\Sdk\Form\Api::getSuccessStatuses(),
            Lyranetwork\Sogecommerce\Sdk\Form\Api::getPendingStatuses()
        );
    ...
    {
        $final_states = array(
            Configuration::get('SOGECOMMERCE_OS_REFUNDED'),
        ...
        if ($response->isAcceptedPayment()) {
            ...
            if ($outofstock) {
                if ($valid) {
                    $new_state = Configuration::get('PS_OS_OUTOFSTOCK_PAID') ? 'PS_OS_OUTOFSTOCK_PAID' : 'SOGECOMMERCE_OS_PAYMENT_OUTOFSTOCK';
                } else {
        ...
        if ($new_state_id && Tools::substr($new_state, 0, Tools::strlen('SOGECOMMERCE_OS')) === 'SOGECOMMERCE_OS' && ! self::isSogecommerceCustomizedState($new_state_id)) {
            $new_state = 'PS_OS_ERROR';
        }
    ...
    public static function isSuccessState($order)
    {
        ...
        $s_states = array(
            'PS_OS_PAYMENT',
            'SOGECOMMERCE_OS_TRANS_PENDING',
            'SOGECOMMERCE_OS_TO_VALIDATE',
            'SOGECOMMERCE_OS_ONEY_PENDING',
            'SOGECOMMERCE_OS_AUTH_PENDING'
        );
    ...
    public static function isOutOfStock($order)
    {
        ...
        $oos_states = array(
            ...
            'SOGECOMMERCE_OS_PAYMENT_OUTOFSTOCK' // Paid state for PrestaShop < 1.6.1.
        );
    ...
    public static function isStateInArray($state_id, $state_names)
    {
        ...
        foreach ($state_names as $state_name) {
            ...
            if ((int) $state_id === (int) Configuration::get($state_name)) {
                if (Tools::substr($state_name, 0, Tools::strlen('SOGECOMMERCE_OS')) === 'SOGECOMMERCE_OS') {
                    return self::isSogecommerceCustomizedState($state_id);
                }
    ...
    public static function isSogecommerceCustomizedState($state_id)
    {
        ...
        foreach ($all_available_order_states as $available_order_state) {
            if ((int) $state_id === (int) $available_order_state['id_order_state']) {
                return $available_order_state['module_name'] === 'sogecommerce';
            }
        }
    ...
    {
        return in_array($response->get('card_brand'), SogecommerceTools::$oney_cards);
    }
    ...
    {
        return in_array($response->get('card_brand'), SogecommerceTools::$credit_cards);
    }
    ...
    private static function sogecommerceCreateOrderSlip($order, $amount)
    {
