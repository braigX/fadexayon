...
class SogecommerceRestModuleFrontController extends ModuleFrontController
{
    public $ssl = true;
    private $logger;
    ...
    public function getToken()
    {
        ...
        $last_cart_update = isset($this->context->cookie->sogecommerce_last_cart_update) ? $this->context->cookie->sogecommerce_last_cart_update : null;
        if ($last_cart_update && ($last_cart_update === $cart->date_upd)) {
            ...
            SogecommerceTools::getLogger()->logInfo("Cart data did not change since last payment attempt, use last created token for cart #{$cart->id}.");
            ...
            if ($token) {
                ...
                $this->context->cookie->sogecommerce_last_cart_update = $cart->date_upd;
    ...
    public function postProcess()
    {
        if (Tools::getValue('refreshToken')) {
            ...
            Tools::redirect(Context::getContext()->link->getModuleLink('sogecommerce', 'wallet', array(), true));
        ...
        $sha_key = $test_mode ? Configuration::get('SOGECOMMERCE_RETKEY_TEST') : Configuration::get('SOGECOMMERCE_RETKEY_PROD');

        // Use direct post content to avoid stipslashes from json data.
        $data = $_POST;

        // Check the authenticity of the request.
        if (! SogecommerceTools::checkHash($data, $sha_key)) {
            $ip = Tools::getRemoteAddr();
            $this->logger->logError("{$ip} tries to access module/sogecommerce/rest page without valid signature with parameters: " . print_r($data, true));
        ...
        if (! $order_id) {
            if ($response->getExtInfo('from_account')) {
                if ($response->get('identifier') || ($response->get('identifier_status') == 'CREATED')) {
                    $this->context->cookie->sogecommerceIdentifierOperationSuccess = $this->module->l('Payment means successfully added.', 'wallet');
                } else {
                    $this->context->cookie->sogecommerceCreateIdentifierError = $this->module->l('Unable to add payment means to your account.', 'wallet');
                }
            ...
            if ($response->isAcceptedPayment()) {
                ...
                if ($save_on_failure) {
                    ...
                    if (! $response->isCancelledPayment()) {
                        ...
                        $this->context->cookie->sogecommercePayErrors = $this->module->l('Your payment was not accepted. Please, try to re-order.', 'submit');
                    }
    ...
    private function deleteIdentifier($identifier)
    {
        ...
        Configuration::updateValue('SOGECOMMERCE_CUSTOMERS_CONFIG', serialize($customersConfig));

        $this->logger->logInfo("Payment identifier deleted successfully for user {$customerEmail} for standard submodule.");
        $this->context->cookie->sogecommerceIdentifierOperationSuccess = $this->module->l('The stored means of payment was successfully deleted.', 'wallet');
    }
