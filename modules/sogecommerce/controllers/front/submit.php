...
class SogecommerceSubmitModuleFrontController extends ModuleFrontController
{
    public $ssl = true;
    ...
    private function processPaymentReturn()
    {
        ...
        if (! $response->isAuthentified()) {
            ...
            $this->logger->logError("{$ip} tries to access module/sogecommerce/submit page without valid signature with parameters: " . print_r($_REQUEST, true));
        ...
        // Rebuild context.
        try {
            SogecommerceTools::rebuildContext($this->currentCart);
        } catch (Exception $e) {
        ...
        if (! $order_id) {
            ...
            if ($response->isAcceptedPayment()) {
                ...
                if ($save_on_failure || Sogecommerce::isOney($response)) {
                    ...
                    $this->sogecommerceRedirect('index.php?controller=history');
                } else {
            ...
            $new_state = (int) Sogecommerce::nextOrderState($response, $outofstock);
            ...
            if (! Configuration::get('PS_OS_OUTOFSTOCK_PAID')) {
                $bc_os_state_valid &= Sogecommerce::isStateInArray($new_state, array('PS_OS_OUTOFSTOCK', 'SOGECOMMERCE_OS_PAYMENT_OUTOFSTOCK'));
            }
            ...
            } elseif (Sogecommerce::isStateInArray($old_state, Sogecommerce::getManagedStates())) {
                if (($old_state == Configuration::get('PS_OS_ERROR')) && $response->isAcceptedPayment() && Sogecommerce::hasAmountError($order)) {
    ...
    private function redirectSuccess($order, $check = false)
    {
        ...
        $this->sogecommerceRedirect($link);
    }
