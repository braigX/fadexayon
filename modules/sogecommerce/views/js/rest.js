...
$(function() {
    $('#total_price').on('DOMSubtreeModified', function() {
        if (typeof (sogecommerce) === 'undefined') {
            return;
        }

        // If it's one-page checkout, do nothing.
        if (sogecommerce.pageType === 'order-opc') {
            return;
        }

        sogecommerceRefreshToken(false);
    });

    setTimeout(function() {
        if ($('#cgv').length) {
            if ($('#cgv').is(':checked')) {
                $('.sogecommerce .kr-payment-button').removeAttr('disabled');
            } else {
                // Unchecked CVG, disable payment button.
                $('.sogecommerce .kr-payment-button').attr('disabled', 'disabled');
            }
        }

        $('.sogecommerce .kr-payment-button').click(function(e) {
            $('.sogecommerce .kr-form-error').html('');
        });

        sogecommerceInitRestEvents();
    }, 0);
});

// Use default messages for these errors.
const SOGECOMMERCE_DEFAULT_MESSAGES = [
    'CLIENT_300', 'CLIENT_304', 'CLIENT_502', 'PSP_539'
];

// Errors requiring page reloading.
const SOGECOMMERCE_EXPIRY_ERRORS = [
    'PSP_108', 'PSP_136', 'PSP_649'
];

var SOGECOMMERCE_LAST_CART = false;

var sogecommerceRestoreCart = function() {
    ...
    $.ajax({
        type: 'POST',
        url: decodeURIComponent(sogecommerce.restUrl),
        async: false,
        cache: false,
        data: "restoreCart=" + SOGECOMMERCE_LAST_CART,
        success: function() {
            SOGECOMMERCE_LAST_CART = false;
        }
    });
};

var sogecommerceEmptyCart = function() {
    $.ajax({
        type: 'POST',
        url: decodeURIComponent(sogecommerce.restUrl),
        async: false,
        ...
        success: function(json) {
            ...
            if (response.lastCart) {
                SOGECOMMERCE_LAST_CART = response.lastCart;
            }
        }
    });
};

var sogecommerceRefreshToken = async function(emptyCart) {
    var refreshData = 'refreshToken=1';

    if ($('#sogecommerce_payment_by_identifier').length > 0) {
        ...
        $.ajax({
            ...
            success: function(json) {
                ...
                if (response.cartUnchanged) {
                    if (emptyCart) {
                        sogecommerceEmptyCart();
                    }

                    resolve(response);
                } else if (response.token) {
                    var token = response.token;
                    sessionStorage.setItem('sogecommerceToken', response.token);
                    ...
                    if (response.identifierToken) {
                        sessionStorage.setItem('sogecommerceIdentifierToken', response.identifierToken);
                    ...
                    KR.setFormConfig({ formToken: token, language: SOGECOMMERCE_LANGUAGE }).then(
                        function(v) {
                            KR = v.KR;
                            resolve(response);
                        }
...
var sogecommerceInitRestEvents = function() {
    KR.onError(function(e) {
        sogecommerceRestoreCart();

        $('button.kr-method-label').each(function() {
            $(this).removeAttr('disabled');
        });

        $('.sogecommerce .processing').css('display', 'none');
        $('#sogecommerce_oneclick_payment_description').show();

        if ($('#sogecommerce_standard').length && $('#sogecommerce_standard').data('submitted')) {
            // PrestaShop 1.7 template.
            $('#payment-confirmation button').removeAttr('disabled');
            $('#sogecommerce_standard').data('submitted', false);
        }
        ...
        var msg = '';
        if (SOGECOMMERCE_DEFAULT_MESSAGES.indexOf(e.errorCode) > -1) {
            msg = e.errorMessage;
        ...
        if (SOGECOMMERCE_EXPIRY_ERRORS.indexOf(e.errorCode) > -1) {
            msg += ' <a href="#" onclick="window.location.reload(); return false;">' + sogecommerceTranslate('RELOAD_LINK') + '</a>';
        }

        $('.sogecommerce .kr-form-error').html('<span style="color: red;"><span>' + msg + '</span></span>');
    });
    ...
    KR.smartForm.onClick(function({paymentMethod, action, formId}) {
        ...
        if ($('#sogecommerce_oneclick_payment_description').length && ! $('.sogecommerce .kr-popin-button').length && ! $('.sogecommerce .kr-type-popin').length) {
            $('#sogecommerce_oneclick_payment_description').hide();
        }

        return sogecommerceCheckTermsAndConditions(paymentMethod, action);
    });

    KR.onPopinClosed(function() {
        // Reset PrestaShop confirmation button style on popin close.
        if (sogecommerceCanProceed()) {
            $("#payment-confirmation button.btn").removeClass('disabled');
        }
    });
...
var sogecommerceCheckTermsAndConditions = async function(paymentMethod, action) {
    if (! sogecommerceCanProceed()) {
        KR.throwCustomError(sogecommerceTranslate("CLIENT_312"), paymentMethod);
        return false;
    }
...
var sogecommerceCanProceed = function() {
    var approvedConditions = ($('input[name*="conditions_to_approve\"]:checked').length == $('input[name*="conditions_to_approve\"]').length);
