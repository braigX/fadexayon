...
<form action="{$link->getModuleLink('sogecommerce', 'redirect', array(), true)|escape:'html':'UTF-8'}" method="post" style="margin-left: 2.875rem; margin-top: 1.25rem; margin-bottom: 1rem;">
  ...
  {if $sogecommerce_multi_card_mode == 2}
    <p style="margin-bottom: .4rem;">{if $sogecommerce_avail_cards|@count == 1}{l s='Payment Mean' mod='sogecommerce'}{else}{l s='Choose your payment mean' mod='sogecommerce'}{/if}</p>
  ...
  <p style="margin-bottom: .4rem;">{if $sogecommerce_multi_options|@count == 1}{l s='Payment option' mod='sogecommerce'}{else}{l s='Choose your payment option' mod='sogecommerce'}{/if}</p>
  {assign var=first value=true}
  {foreach from=$sogecommerce_multi_options key="key" item="option"}
    {if {$sogecommerce_multi_options|@count} == 1}
      <input type="hidden" name="sogecommerce_opt" value="{$key|escape:'html':'UTF-8'}" id="sogecommerce_opt_{$key|escape:'html':'UTF-8'}">
    {else}
      <input type="radio" name="sogecommerce_opt" value="{$key|escape:'html':'UTF-8'}" id="sogecommerce_opt_{$key|escape:'html':'UTF-8'}" {if $first == true} checked="checked"{/if}>
      &nbsp;
    {/if}

    <label for="sogecommerce_opt_{$key|escape:'html':'UTF-8'}" style="font-weight: bold; display: inline;">{$option.localized_label|escape:'html':'UTF-8'}</label>
    <br />
