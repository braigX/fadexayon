...
{if {$sogecommerce_ffin_options|@count} == 0}
  <div class="payment_module sogecommerce {$sogecommerce_tag|escape:'html':'UTF-8'}">
    <a href="javascript: $('#sogecommerce_franfinance').submit();" title="{l s='Click here to pay with FranFinance' mod='sogecommerce'}">
      <img class="logo" src="{$sogecommerce_logo|escape:'html':'UTF-8'}" />{$sogecommerce_title|escape:'html':'UTF-8'}

      <form action="{$link->getModuleLink('sogecommerce', 'redirect', array(), true)|escape:'html':'UTF-8'}" method="post" id="sogecommerce_franfinance">
