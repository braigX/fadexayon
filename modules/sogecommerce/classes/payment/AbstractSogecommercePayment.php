...
abstract class AbstractSogecommercePayment
{
    const SOGECOMMERCE_CART_MAX_NB_PRODUCTS = 85;
    ...
    {
        $this->module = Module::getInstanceByName('sogecommerce');
    ...
    public function accountCustomerWallet()
    {
        ...
        return ($controller instanceof SogecommerceWalletModuleFrontController);
    }
    ...
    {
        return _MODULE_DIR_ . 'sogecommerce/views/img/' . $this->logo;
    }

    public function getTplVars($cart)
    {
        return array(
            'sogecommerce_title' => $this->getTitle((int) $cart->id_lang),
            'sogecommerce_logo' => $this->getLogo()
        );
    }
    ...
    public function getPaymentOption($cart)
    {
        ...
        $option->setCallToActionText($this->getTitle((int) $cart->id_lang))
            ->setModuleName('sogecommerce');

        if (file_exists(_PS_MODULE_DIR_ . 'sogecommerce/views/img/' . $this->logo)) {
            $option->setLogo($this->getLogo());
        }

        if (! $this->hasForm()) {
            $option->setAction($this->context->link->getModuleLink('sogecommerce', 'redirect', array(), true));

            $inputs = array(
                array('type' => 'hidden', 'name' => 'sogecommerce_payment_type', 'value' => $this->name)
            );
    ...
    public function prepareRequest($cart, $data = array())
    {
        ...
        $colissimo_address = SogecommerceTools::getColissimoDeliveryAddress($cart); // Get SoColissimo delivery address.
        ...
        if (! empty($cart->id)) {
            SogecommerceTools::getLogger()->logInfo("Form data generation for cart #{$cart->id} with {$this->name} submodule.");
        }

        /* @var $request SogecommerceRequest */
        ...
        foreach (SogecommerceTools::getAdminParameters() as $param) {
            if (isset($param['name'])) {
                $id_lang = null;
                if (in_array($param['key'], SogecommerceTools::$multi_lang_fields)) {
        ...
        $cart_currency = new Currency((int) $cart->id_currency);
        $currency = SogecommerceApi::findCurrencyByAlphaCode($cart_currency->iso_code);
        ...
        if(SogecommerceTools::$plugin_features['brazil']) {
            $customer_address = array_merge((array) $cust, (array) $billing_address);

            $address_number = '0';
            $config_number = Configuration::get('SOGECOMMERCE_NUMBER');
            ...
            $document = '';
            $config_document = Configuration::get('SOGECOMMERCE_DOCUMENT');
            ...
            $document = SogecommerceTools::formatDocument($document);

            $neighborhood = '-';
            $config_neighborhood = Configuration::get('SOGECOMMERCE_NEIGHBORHOOD');
        ...
        // Activate 3DS?
        $threeds_mpi = null;
        $threeds_min_amount_options = @unserialize(Configuration::get('SOGECOMMERCE_3DS_MIN_AMOUNT'));
        ...
        // Return URL.
        $request->set('url_return', $this->context->link->getModuleLink('sogecommerce', 'submit', array(), true));
    ...
    public function isFromBackend()
    {
        if (isset($this->context->cookie->sogecommerceBackendPayment)) {
            return $this->context->cookie->sogecommerceBackendPayment;
        }
        ...
        if (isset($adminCookie->id_employee) && ! empty($adminCookie->id_employee) && Tools::getIsset('recover_cart')) {
            $this->context->cookie->sogecommerceBackendPayment = true;
            $return = true;
        }
    ...
    private function setCartData($cart, $currency, &$sogecommerce_request)
    {
        $products = $cart->getProducts(true);

        if (! $this->needs_cart_data && (count($products) > self::SOGECOMMERCE_CART_MAX_NB_PRODUCTS)) {
            return;
        }

        if (! $this->needs_cart_data && ($this->force_local_cart_data || (Configuration::get('SOGECOMMERCE_SEND_CART_DETAIL') !== 'True'))) {
            return;
        }

        // Prepare cart data to send to gateway.
        if (Configuration::get('SOGECOMMERCE_COMMON_CATEGORY') !== 'CUSTOM_MAPPING') {
            $category = Configuration::get('SOGECOMMERCE_COMMON_CATEGORY');
        } else {
            $oney_categories = @unserialize(Configuration::get('SOGECOMMERCE_CATEGORY_MAPPING'));
        }
        ...
        foreach ($products as $product) {
            ...
            $sogecommerce_request->addProduct(
    ...
    private function setAdvancedShippingData($cart, $delivery_address, &$sogecommerce_request)
    {
        if (Configuration::get('SOGECOMMERCE_SEND_SHIP_DATA') !== 'True' && ! $this->needs_shipping_method_data) {
            return;
        }

        // For Oney, some parameters must contains different data.
        $isOney34 = $this instanceof SogecommerceOney34Payment;

        // Oney delivery options defined in admin panel.
        $shipping_options = @unserialize(Configuration::get('SOGECOMMERCE_ONEY_SHIP_OPTIONS'));
        ...
        if ($cart->isVirtualCart() || ! isset($carrier_id) || ! is_array($shipping_options) || empty($shipping_options)) {
            ...
            $sogecommerce_request->set('ship_to_type', 'ETICKET');
            $sogecommerce_request->set('ship_to_speed', 'EXPRESS');
        } elseif (self::isSupportedRelayPoint($carrier_id)) {
            // Specific supported relay point carrier.
            $sogecommerce_request->set('ship_to_type', 'RELAY_POINT');
            $sogecommerce_request->set('ship_to_speed', 'STANDARD');
            ...
            switch (true) {
                ...
                case (self::isSoColissimoLiberteRelay($carrier_id)):
                    ...
                    $country = $sogecommerce_request->get('ship_to_country');
                ...
                case (self::isColissimoRelay($carrier_id) && $delivery_address->company):
                    ...
                    // Already set address.
                    $city = $sogecommerce_request->get('ship_to_city');
                    $zipcode = $sogecommerce_request->get('ship_to_zip');
                    $country = $sogecommerce_request->get('ship_to_country');
            ...
            $sogecommerce_request->set('ship_to_street', preg_replace($address_not_allowed_chars, ' ', $address));
            $sogecommerce_request->set('ship_to_street2', null);
            $sogecommerce_request->set('ship_to_zip', $zipcode);
            $sogecommerce_request->set('ship_to_city', preg_replace($not_allowed_chars, ' ', $city));
            $sogecommerce_request->set('ship_to_state', null);
            $sogecommerce_request->set('ship_to_country', $country);

            $delivery_company = preg_replace($not_allowed_chars, ' ', $address . ' ' . $zipcode . ' ' . $city);
            $sogecommerce_request->set('ship_to_delivery_company_name', $delivery_company);
        } else {
            // Other cases.
            ...
            $sogecommerce_request->set('ship_to_type', $delivery_type);
            $sogecommerce_request->set('ship_to_speed', $delivery_speed);
            ...
            if ($delivery_type === 'RECLAIM_IN_SHOP') {
                ...
                $sogecommerce_request->set('ship_to_street', $shop_name . ' ' . $shipping_options[$carrier_id]['address']);
                $sogecommerce_request->set('ship_to_street2', null);
                $sogecommerce_request->set('ship_to_zip', $shipping_options[$carrier_id]['zip']);
                $sogecommerce_request->set('ship_to_city', $shipping_options[$carrier_id]['city']);
                $sogecommerce_request->set('ship_to_country', 'FR');
            ...
            if ($shipping_options[$carrier_id]['speed'] === 'PRIORITY') {
                $sogecommerce_request->set('ship_to_delay', $shipping_options[$carrier_id]['delay']);
            }

            $sogecommerce_request->set('ship_to_delivery_company_name', preg_replace($not_allowed_chars, ' ', $company_name));
        }

        if ($isOney34) {
            // Modify address to send it to Oney.
            if ($sogecommerce_request->get('ship_to_street')) { // If there is a delivery address.
                $sogecommerce_request->set('ship_to_status', 'PRIVATE'); // By default PrestaShop doesn't manage customer type.

                $address = $sogecommerce_request->get('ship_to_street') . ' ' . $sogecommerce_request->get('ship_to_street2');

                $sogecommerce_request->set('ship_to_street', preg_replace($address_not_allowed_chars, ' ', $address));
                $sogecommerce_request->set('ship_to_street2', $relay_point_name);
            ...
            $sogecommerce_request->set('cust_status', 'PRIVATE');

            // Send FR even address is in DOM-TOM unless form is rejected.
            $sogecommerce_request->set('cust_country', 'FR');
        }
    }
    ...
    protected function l($string)
    {
        ...
        return $sogecommerce->l($string, Tools::strtolower(get_class($this)));
    }
    ...
    public function setCookieValidPaymentByAlias($identifier, $customer)
    {
        ...
        if ($identifier && $customer->id) {
            try {
                ...
                SogecommerceTools::getLogger()->logError(
    ...
    {
        return SogecommerceTools::getDefault('LOGO_URL') . strtolower($card) . '.png';
    }
}
