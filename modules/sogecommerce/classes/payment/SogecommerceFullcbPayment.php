...
class SogecommerceFullcbPayment extends AbstractSogecommercePayment
{
    const FULLCB_THREE_TIMES_MAX_FEES = 9;
    const FULLCB_FOUR_TIMES_MAX_FEES = 12;

    protected $prefix = 'SOGECOMMERCE_FULLCB_';
    ...
    public function validate($cart, $data = array())
    {
        ...
        $errors = SogecommerceTools::checkAddress($billing_address, 'billing', $this->name);

        if (empty($errors)) {
            // Billing address is valid, check delivery address.
            $delivery_address = new Address((int) $cart->id_address_delivery);

            $errors = SogecommerceTools::checkAddress($delivery_address, 'delivery', $this->name);
        }
    ...
    public static function getAvailableOptions($cart)
    {
        ...
        $options = @unserialize(Configuration::get('SOGECOMMERCE_FULLCB_OPTIONS'));
        ...
        foreach ($options as $key => $option) {
            ...
            if ((empty($min) || $amount >= $min) && (empty($max) || $amount <= $max)) {
                ...
                $option['order_amount'] = SogecommerceTools::formatPrice($amount, $cart->id_currency, $context);
                $option['first_payment'] = SogecommerceTools::formatPrice($first, $cart->id_currency, $context);
                $option['monthly_payment'] = SogecommerceTools::formatPrice($payment, $cart->id_currency, $context);
                $option['total_amount'] = SogecommerceTools::formatPrice($amount + $fees, $cart->id_currency, $context);
                $option['fees'] = SogecommerceTools::formatPrice($fees, $cart->id_currency, $context);
    ...
    public function getTplVars($cart)
    {
        ...
        $vars['sogecommerce_fullcb_options'] = $options;
