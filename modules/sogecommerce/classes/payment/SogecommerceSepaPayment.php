...
class SogecommerceSepaPayment extends AbstractSogecommercePayment
{
    protected $prefix = 'SOGECOMMERCE_SEPA_';
    ...
    public function getTplVars($cart)
    {
        ...
        $vars['sogecommerce_is_valid_sepa_identifier'] = false;
        $vars['sogecommerce_sepa_saved_payment_mean'] = '';

        if ($this->isValidSavedAlias()) {
            $vars['sogecommerce_is_valid_sepa_identifier'] = true;
            $customers_config = @unserialize(Configuration::get('SOGECOMMERCE_CUSTOMERS_CONFIG'));
            $vars['sogecommerce_sepa_saved_payment_mean'] = isset($customers_config[$cart->id_customer][$this->name]['m']) ?
