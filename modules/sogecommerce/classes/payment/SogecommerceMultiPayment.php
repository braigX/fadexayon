...
class SogecommerceMultiPayment extends AbstractSogecommercePayment
{
    protected $prefix = 'SOGECOMMERCE_MULTI_';
    ...
    public static function getAvailableOptions($cart = null)
    {
        ...
        $options = @unserialize(Configuration::get('SOGECOMMERCE_MULTI_OPTIONS'));
    ...
    public function getTplVars($cart)
    {
        ...
        $vars['sogecommerce_multi_options'] = self::getAvailableOptions($cart);

        if ($this->isFromBackend()) {
            $vars['sogecommerce_multi_card_mode'] = '1';
            return $vars;
        }

        $entry_mode = Configuration::get($this->prefix . 'CARD_MODE');
        $vars['sogecommerce_multi_card_mode'] = $entry_mode;

        if ($entry_mode === SogecommerceTools::MODE_LOCAL_TYPE) {
    ...
    {
        $all_cards = SogecommerceTools::getSupportedMultiCardTypes();
