...
class SogecommerceStandardPayment extends AbstractSogecommercePayment
{
    protected $prefix = 'SOGECOMMERCE_STD_';
    ...
    public function getTplVars($cart, $force_customer_wallet = false)
    {
        ...
        if ($embedded && version_compare(_PS_VERSION_, '1.7', '<') && (bool) Configuration::get('PS_JS_DEFER')) {
            ...
            if ($pub_key = $test_mode ? Configuration::get('SOGECOMMERCE_PUBKEY_TEST') : Configuration::get('SOGECOMMERCE_PUBKEY_PROD')) {
                $vars['sogecommerce_set_std_rest_kr_public_key'] = $pub_key;
            }

            // Return url.
            if ($return_url = $this->context->link->getModuleLink('sogecommerce', 'rest', array(), true)) {
                $vars['sogecommerce_set_std_rest_return_url'] = $return_url;
            }
            ...
            if ($pan_label = $rest_placeholders['pan'][$language['id_lang']]) {
                $vars['sogecommerce_set_std_rest_kr_placeholder_pan'] = $pan_label;
            }

            // kr-placeholder-expiry.
            if ($expiry_label = $rest_placeholders['expiry'][$language['id_lang']]) {
                $vars['sogecommerce_set_std_rest_kr_placeholder_expiry'] = $expiry_label;
            }

            // kr-placeholder-security-code.
            if ($cvv_label = $rest_placeholders['cvv'][$language['id_lang']]) {
                $vars['sogecommerce_set_std_rest_kr_placeholder_security_code'] = $cvv_label;
            }

            // kr-label-do-register.
            if ($register_card_label = Configuration::get('SOGECOMMERCE_STD_REST_LBL_REGIST', $language['id_lang'])) {
                $vars['sogecommerce_set_std_rest_kr_label_do_register'] = $register_card_label;
            }
        }
        ...
        if (! SogecommerceApi::isSupportedLanguage($language_iso_code)) {
            $language_iso_code = Configuration::get('SOGECOMMERCE_DEFAULT_LANGUAGE');
        }

        $vars['sogecommerce_set_std_rest_language'] = $language_iso_code;

        if ($this->isFromBackend()) {
            $vars['sogecommerce_std_card_data_mode'] = '1';
            return $vars;
        }

        $controller = $this->context->controller;

        $entry_mode = ($controller instanceof SogecommerceWalletModuleFrontController) ? SogecommerceTools::MODE_SMARTFORM : $this->getEntryMode();
        $vars['sogecommerce_std_card_data_mode'] = $entry_mode;
        $vars['sogecommerce_std_rest_popin_mode'] = ($controller instanceof SogecommerceWalletModuleFrontController) ? false : Configuration::get('SOGECOMMERCE_STD_REST_POPIN_MODE');
        $vars['sogecommerce_std_rest_theme'] = Configuration::get('SOGECOMMERCE_STD_REST_THEME');
        $vars['sogecommerce_std_smartform_compact_mode'] = Configuration::get('SOGECOMMERCE_STD_SF_COMPACT_MODE');
        $vars['sogecommerce_std_smartform_payment_means_grouping_threshold'] = Configuration::get('SOGECOMMERCE_STD_SF_THRESHOLD') ?
            Configuration::get('SOGECOMMERCE_STD_SMARTFORM_PAYMENT_MEANS_GROUPING_THRESHOLD') : 'False';
        $vars['sogecommerce_std_display_title'] = $this->displayTitle();

        // Payment by identifier.
        $vars['sogecommerce_is_valid_std_identifier'] = false;
        $vars['sogecommerce_saved_payment_mean'] = '';

        $vars['sogecommerce_rest_form_token'] = '';
        $vars['sogecommerce_rest_identifier_token'] = '';

        $vars['sogecommerce_std_select_by_default'] = (Configuration::get('SOGECOMMERCE_STD_SELECT_BY_DEFAULT') == 'True') ? 'True' : 'False';

        if ($this->isValidSavedAlias()) {
            $vars['sogecommerce_is_valid_std_identifier'] = true;
            $customers_config = @unserialize(Configuration::get('SOGECOMMERCE_CUSTOMERS_CONFIG'));
            $vars['sogecommerce_saved_payment_mean'] = isset($customers_config[$cart->id_customer][$this->name]['m']) ?
                $customers_config[$cart->id_customer][$this->name]['m'] : '';
        }

        if ($entry_mode === SogecommerceTools::MODE_LOCAL_TYPE /* Card type on website. */) {
            $vars['sogecommerce_avail_cards'] = $this->getPaymentCards();
        } elseif ($embedded /* REST API. */) {
            $form_token = $this->getFormToken($cart);

            if ($form_token) {
                // REST API params.
                $vars['sogecommerce_rest_form_token'] = $form_token;
                $vars['sogecommerce_rest_identifier_token'] = $form_token;

                if (Configuration::get('SOGECOMMERCE_STD_USE_WALLET') == 'True') {
                    $vars['sogecommerce_is_valid_std_identifier'] = false;
                } else {
                    // Customer has an identifier and it is valid.
                    $identifier_token = $this->getFormToken($cart, true);
                    if ($identifier_token) {
                        $vars['sogecommerce_rest_identifier_token'] = $identifier_token;
                    }
                }
    ...
    private function getPaymentCards()
    {
        ...
        if (! empty($cards)) {
            ...
            $cards = array_keys(SogecommerceTools::getSupportedCardTypes());
        }
    ...
    {
        return SogecommerceTools::getEntryMode($this->prefix);
    }
    ...
    private function displayTitle()
    {
        ...
        if ($this->isEmbedded() && Configuration::get('SOGECOMMERCE_STD_REST_POPIN_MODE') === 'True') {
            return 'True';
        }

        return Configuration::get('SOGECOMMERCE_STD_SF_DISPLAY_TITLE');
    }
    ...
    /**
     * {@inheritDoc}
     * @see AbstractSogecommercePayment::prepareRequest()
     */
    public function prepareRequest($cart, $data = array())
    {
        ...
        if (isset($this->context->cookie->sogecommerceBackendPayment)) {
            $request->set('payment_src', 'MOTO');
            unset($this->context->cookie->sogecommerceBackendPayment);
        ...
        if ($this->isOneClickActive() && $customer->id) {
            $isCustomerWallet = $this->isEmbedded() && (Configuration::get('SOGECOMMERCE_STD_USE_WALLET') == 'True') || $this->accountCustomerwallet();
            if ($isCustomerWallet) {
                $controller = $this->context->controller;
                if ($controller instanceof SogecommerceWalletModuleFrontController) {
                ...
                SogecommerceTools::getLogger()->logInfo('Customer ' . $request->get('cust_email') . ' will be asked for card data registration on payment page.');
    ...
    public function getFormToken($cart, $use_identifier = false)
    {
        ...
        $currency = SogecommerceApi::findCurrencyByNumCode($this->getEscapedVar($request, 'currency'));
        ...
        $validationMode = Configuration::get('SOGECOMMERCE_STD_VALIDATION');
        if ($validationMode !== "") {
            $validationMode = ($validationMode === '-1') ? Configuration::get('SOGECOMMERCE_VALIDATION_MODE') : $validationMode;
        ...
        if ($use_identifier) {
            if ($saved_identifier = $this->getEscapedVar($request, 'identifier')) {
                SogecommerceTools::getLogger()->logInfo("Customer {$cust_mail} has an identifier. Use it for payment of cart #{$cart_id}.");
        ...
        $test_mode = Configuration::get('SOGECOMMERCE_MODE') === 'TEST';
        $key = $test_mode ? Configuration::get('SOGECOMMERCE_PRIVKEY_TEST') : Configuration::get('SOGECOMMERCE_PRIVKEY_PROD');
        $site_id = Configuration::get('SOGECOMMERCE_SITE_ID');
        ...
        SogecommerceTools::getLogger()->logInfo("Creating form token for cart #{$cart_id} with parameters: {$data}");

        try {
            $client = new SogecommerceRest(Configuration::get('SOGECOMMERCE_REST_SERVER_URL'), $site_id, $key);
            $result = $client->post('V4/Charge/'. $webservice, $data);

            if ($result['status'] !== 'SUCCESS') {
                SogecommerceTools::getLogger()->logError("Error while creating payment form token for {$metadata}: " . $result['answer']['errorMessage']
                    . ' (' . $result['answer']['errorCode'] . ').');

                if (isset($result['answer']['detailedErrorMessage']) && ! empty($result['answer']['detailedErrorMessage'])) {
                    SogecommerceTools::getLogger()->logError('Detailed message: ' . $result['answer']['detailedErrorMessage']
                        . ' (' . $result['answer']['detailedErrorCode'] . ').');
                }
            } else {
                // Payment form token created successfully.
                SogecommerceTools::getLogger()->logInfo("Form token created successfully for {$metadata}");
                $return = $result['answer']['formToken'];
            }
        } catch (Exception $e) {
            SogecommerceTools::getLogger()->logError("{$e->getMessage() }" . ($e->getCode() > 0 ? ' (' . $e->getCode() . ').' : ''));
        }
    ...
    {
        if ($this->getEntryMode() === SogecommerceTools::MODE_FORM) {
            return false;
        }
    ...
    public function isEmbedded()
    {
        ...
        $embedded = array(
            SogecommerceTools::MODE_SMARTFORM,
            SogecommerceTools::MODE_SMARTFORM_EXT_WITH_LOGOS,
            SogecommerceTools::MODE_SMARTFORM_EXT_WITHOUT_LOGOS
        );
    ...
    {
        $paymentCards = Configuration::get('SOGECOMMERCE_STD_PAYMENT_CARDS');
        ...
        if (Configuration::get('SOGECOMMERCE_OTHER_ENABLED') === 'True') {
            $otherPaymentMeans = @unserialize(Configuration::get('SOGECOMMERCE_OTHER_PAYMENT_MEANS'));
