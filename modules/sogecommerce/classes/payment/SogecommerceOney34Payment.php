...
class SogecommerceOney34Payment extends AbstractSogecommercePayment
{
    protected $prefix = 'SOGECOMMERCE_ONEY34_';
    ...
    public function isAvailable($cart)
    {
        ...
        if (! SogecommerceTools::checkOneyRequirements($cart, $this->label)) {
            return false;
        }
    ...
    public function validate($cart, $data = array())
    {
        ...
        $errors = SogecommerceTools::checkAddress($billing_address, 'billing', 'oney34');

        if (empty($errors)) {
            // Billing address is valid, check delivery address.
            $delivery_address = new Address((int) $cart->id_address_delivery);

            $errors = SogecommerceTools::checkAddress($delivery_address, 'delivery', 'oney34');
        }
    ...
    public static function getAvailableOptions($cart)
    {
        ...
        $options = @unserialize(Configuration::get('SOGECOMMERCE_ONEY34_OPTIONS'));
    ...
    public function getTplVars($cart)
    {
        ...
        $vars['sogecommerce_oney_options'] = self::getAvailableOptions($cart);
