...
require_once dirname(__FILE__) . '/sogecommerce.php';

// Module logger object.
$logger = SogecommerceTools::getLogger();

$save_on_failure = true;

if (SogecommerceTools::checkRestIpnValidity()) {
    ...
    $data = SogecommerceTools::convertRestResult($answer);
    ...
    // Rebuild context.
    try {
        SogecommerceTools::rebuildContext($cart);
    } catch (Exception $e) {
    ...
    $sha_key = $test_mode ? Configuration::get('SOGECOMMERCE_PRIVKEY_TEST') : Configuration::get('SOGECOMMERCE_PRIVKEY_PROD');

    if (! SogecommerceTools::checkHash($_POST, $sha_key)) {
    ...
    /** @var SogecommerceResponse $response */
    $response = new SogecommerceResponse($data, null, null, null);
} elseif (SogecommerceTools::checkFormIpnValidity()) {
    ...
    /** @var SogecommerceResponse $response */
    $response = new SogecommerceResponse(
        $_POST,
        Configuration::get('SOGECOMMERCE_MODE'),
        Configuration::get('SOGECOMMERCE_KEY_TEST'),
        Configuration::get('SOGECOMMERCE_KEY_PROD'),
        Configuration::get('SOGECOMMERCE_SIGN_ALGO')
    );
...
if (! $order_id) {
    ...
    $new_state = (int) Sogecommerce::nextOrderState($response);

    if ($response->isAcceptedPayment()) {
        $logger->logInfo("Payment accepted for cart #$cart_id. New order state is $new_state.");

        $order = $sogecommerce->saveOrder($cart, $new_state, $response);

        if (Sogecommerce::hasAmountError($order)) {
        ...
        $save_on_failure &= (Configuration::get('SOGECOMMERCE_FAILURE_MANAGEMENT') === SogecommerceTools::ON_FAILURE_SAVE);
        if ($save_on_failure || Sogecommerce::isOney($response)) {
            // Save on failure option is selected or Oney payment.
            $msg = Sogecommerce::isOney($response) ? 'Oney payment' : 'Save on failure option is selected';
    ...
    $currency = SogecommerceApi::findCurrency($response->get('currency'));
    ...
    $outofstock = Sogecommerce::isOutOfStock($order);
    $new_state = (int) Sogecommerce::nextOrderState($response, $outofstock, $old_state, $is_partial_payment);
    ...
    $first_payment = ($response->get('sequence_number') === '1')
        || Sogecommerce::isFirstSequenceInOrderPayments($order, $response->get('trans_id'), $response->get('sequence_number'));
    ...
    if (($old_state === $new_state) || ! $first_payment) {
        ...
        // Do not create payment if it is cancelled partial debit payment OR order is in final status SOGECOMMERCE_OS_REFUNDED.
        $force_stop_payment_creation = (Configuration::get('SOGECOMMERCE_OS_REFUNDED') == $old_state) ||
                                       (($response->isCancelledPayment() || ($response->getTransStatus() === 'CANCELLED'))
                                       && ($response->get('operation_type') === 'DEBIT') && $is_partial_payment);

        $sogecommerce->savePayment($order, $response, $force_stop_payment_creation);
        $sogecommerce->createMessage($order, $response);
    ...
    } elseif (Sogecommerce::isPaidOrder($order) &&
        (! Sogecommerce::isStateInArray($new_state, $consistent_states))) {
    ...
    } elseif (! $old_state || Sogecommerce::isStateInArray($old_state, Sogecommerce::getManagedStates())) {
        if (($old_state === Configuration::get('PS_OS_ERROR')) && $response->isAcceptedPayment() &&
            Sogecommerce::hasAmountError($order)) {
        ...
        $sogecommerce->setOrderState($order, $new_state, $response);
