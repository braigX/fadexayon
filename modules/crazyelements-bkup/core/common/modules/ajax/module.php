...
class Module extends BaseModule {
	...
	public function __construct() {
		...
		PrestaHelper::add_action( 'wp_ajax_mailchimpsubscribe', array( $this, 'function_mch_subscribe' ) );
	...
	public function get_dynamic_link(){
		...
		$front_controllers_links = $this->get_front_controllers($search, $context);

		$results = array_merge($results, $front_controllers_links);
	...
	private function get_front_controllers($search, $context){
        ...
        (
			...
			'pdf-order-return' => 'Pdf Order Return',
			'pdf-order-slip' => 'Pdf Order Slip',
        ...
        foreach($controllers as $key => $controller){
			$haystack = strtolower($controller);
			$search = strtolower($search);
			if(strstr( $haystack, $search )){
				$link = $context->link->getPageLink($key, true);
				$ret_arr[] = array(
					'ID'        => $key,
					'title'     => $controller,
					'permalink' => $link,
	...
	private function mailchimp_subscriber_status( $listidhidden, $email, $status, $list_id, $api_key, $merge_fields = array(
		'FNAME' => '',
		'LNAME' => '',
		'PHONE' => '',
	) ) {
	...
	public function function_mch_subscribe() {
		$list_id       = $_POST['listidhidden'];
		$api_key       = PrestaHelper::get_option( 'mailchimp_data' );
		$results       = $this->mailchimp_subscriber_status(
			$_POST['listidhidden'],
			$_POST['email'],
			'subscribed',
			$list_id,
		...
		$result_expand = json_decode( $results );
		$result_expand = json_decode( $results, true );
		if ( $result_expand['status'] == 'subscribed' ) {
			echo 'thank you for being with us';

		} else {
			echo $result_expand['title'];
		}
		die();
	}
	public function ajax_get_items() {
		...
		} elseif ( $item_type == 'saved_templates' ) {
			$rs = $this->getSavedTemplatesByName();
		}
		echo json_encode( $rs );
		die();
	}

	public function getSavedTemplatesByName(){
		...
		if ( isset( $value_search ) && $value_search != '' ) {
			$exSql = '';
			if ( $selected_values != '' ) {
				$exSql .= ' AND crlr.`id_crazy_library` NOT IN(';
				$exSql .= $selected_values;
				$exSql .= ') ';
			}
			$query = 'SELECT crlr.`id_crazy_library` as `key`, crlr.`title`  FROM ' . _DB_PREFIX_ . 'crazy_library as crlr WHERE crlr.`title` LIKE "%' . pSQL( $value_search ) . '%" ' . $exSql;
			$templates = \Db::getInstance()->executeS( $query );
			$newarray=[];
			foreach($templates as $key=> $res){
				$reskey=$res['key']."_".$res['title'];
...
public function getProductNameImage() {
	...
	if ( isset( $value_search ) && $value_search != '' ) {
		$data_settings = \Tools::getValue( 'is_current' );
		$data_settings = json_decode($data_settings, true);

		// 🔒 BONUS : validation JSON
		if (!is_array($data_settings) || !isset($data_settings['is_current'])) {
			die(json_encode(['error' => 'Invalid input']));
		}

		$current_category = $data_settings['is_current'];

		// 🔒 Protection anti-SQLi
		if ($current_category !== null && $current_category !== 'no' && is_numeric($current_category)) {
			$current_category = ' AND cp.`id_category` = ' . (int)$current_category;
		} else {
			$current_category = ' AND cp.`id_category` = p.`id_category_default`';
		}

		if (isset($data_settings['number_of_results'])) {
			$limit = (int) $data_settings['number_of_results'];
			$limit = ' LIMIT ' . $limit;
		} else {
			$limit = '';
		}
		...
		$sql = 'SELECT p.`id_product` as `key`, pl.`name`
			...
			' . ( $front ? ' AND product_shop.`visibility` IN ("both", "catalog")' : '' ) . '
			AND pl.`name` LIKE "%' . $value_search . '%" ' .
			$current_category . '
			ORDER BY pl.`name` ' . $limit . ';';
