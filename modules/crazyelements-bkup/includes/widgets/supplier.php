...
class Widget_Supplier extends Widget_Base {
	...
	public function get_title() {
		return PrestaHelper::__( 'Product by Supplier', 'elementor' );
	}
	...
	public function get_icon() {
		return 'ceicon-hotel-supplier-widget';
	}
	...
	protected function _register_controls() {
		...
		$control = array(
			'id_supplier',
			array(
				'label'     => PrestaHelper::__( 'Select Supplier', 'elementor' ),
		...
		$this->general_controls($control, true, false);
	...
	protected function render() {
		...
		$this->generate_controls_data($settings, 'id_supplier');
		...
		// where query params
		$id_supplier =  $this->render_autocomplete_result($this->ids);
		
		if(isset($id_supplier) && $id_supplier != ''){
			$products = \Supplier::getProducts( (int) $id_supplier, $id_lang, 1, $limit, $orderby, $order );
			if ( ! $products ) {
				return false;
			}
	
			// Query and get data for template
			$products_for_template = $this->get_products_for_template($id_lang, $products);
			...
			if ( $this->layout == 'default' ) {
				...
				$this->show_default_skin($products_for_template, 'products-by-supplier', $display_type);
			} else {
				$from_cat_addon = false;
				$this->show_crazy_skins($products_for_template, 'products-by-supplier');
			} 
		}else{
			echo 'Please Select a Supplier to Show Products Of That Supllier';
		}
	}
