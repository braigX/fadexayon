...
class WkSampleProduct extends Module
{
        ...
        if (Tools::isSubmit('submitGlobalSample')) {
            ...
            if (Tools::getValue('WK_GLOBAL_SAMPLE')) {
                ...
                foreach ($allLanguages as $lang) {
                    ...
                    // if ($sampleDesc && Tools::strlen(trim($sampleDesc)) && $objProductMap->checkParaInHtml($sampleDesc)) {
    ...
    public function hookDisplayProductAdditionalInfo($params)
    {
        ...
        // Vérifie si l'affichage rapide est en cours
        if ((Tools::getValue('action') != 'quickview') && !Tools::getValue('quickview')) {
            if (Configuration::get('WK_SAMPLE_LOGGED_ONLY')) {
                if (!isset($this->context->customer->id)) {
                    return ''; // Si connexion requise et l'utilisateur n'est pas connecté, on ne montre rien
                }
            }
    
            // Vérifie si l'échantillon est disponible via hookDisplayFooterProduct
        ...
        return ''; // Ne rien afficher si les conditions ne sont pas remplies
    }
    ...
    public function hookActionOrderGridQueryBuilderModifier(array $params)
    {
        ...
        $searchQueryBuilder->addSelect(
            'coalesce(wsc.sample, 0) as sample, (CASE WHEN coalesce(wsc.sample, 0)=1 THEN "Yes"
            WHEN coalesce(wsc.sample, 0)=0 THEN "No" END) AS wk_has_sample'
        );
    ...
    public function hookDisplayAdminProductsExtra($params)
    {
        ...
        if ($idProduct) {
            if (Module::isEnabled('marketplace') && WkMpSellerProduct::getSellerProductByPsIdProduct($idProduct)) {
                ...
                if ($sampleFileName = $objSampleProductMap->getSampleFileName($idProduct)) {
                    ...
                    if (
                        Tools::strlen(trim($sampleFileName['sample_file']))
    ...
    public function hookActionProductSave($params)
    {
        if ($params['id_product']) {
            ...
            foreach ($allLanguages as $lang) {
                ...
                $sampleDesc = trim(Tools::getValue('wk_sample_desc_' . $lang['id_lang']));
                // if ($sampleDesc && Tools::strlen(trim($sampleDesc)) && $objProductMap->checkScriptInHtml($sampleDesc)) {
                //     $this->context->controller->errors['hooks_wk_sample_desc_' . $lang['id_lang']] =
    ...
    public function hookActionCartUpdateQuantityBefore($params)
    {
        ...
        if (isset($params['operator'])) {
            if ($params['operator'] == 'up') {
                ...
                // I removed this condition because it was causing the sample product to be added to the cart with the wrong price
                // To reverse it back replace the if condition with the one below
                //
                ...
                if (
                    isset($this->context->cookie->sampleProductId, $this->context->cookie->sampleProductIdAttr)
                    && ($this->context->cookie->sampleProductId == $product->id)
                    && !$isSampleProduct
                ) {
                ...
                if (
                    $objSampleCart->checkProductQtyInCart(
                        $cart,
                        $product->id,
                        $params['quantity'],
                        $params['id_product_attribute']
