...
var etsSeo = {
  ...
  getTextContentFromInputs: (idLang, controller) => {
    ...
    if (['AdminProducts', 'AdminManufacturers'].indexOf(controller) !== -1) {
      return $(short_desc + idLang).val() + $(content + idLang).val();
    }
  ...
  readability: {
    ...
    sentenceLength: function(id_lang, content) {
      ...
      if (percent_bad <= 25) {
        ETS_SEO_DEFINED.readability_rules.sentence_length.error.short_code['[link_js]'].show = false;
        etsSeo.getAnalysisMessage(
            '#analysis-result--list-readablity-' + id_lang,
            'sentence_length',
        ...
        if (ETS_SEO_DEFINED.readability_rules.sentence_length.error.short_code['[number]']) {
          ...
          ETS_SEO_DEFINED.readability_rules.sentence_length.error.short_code['[link_js]'].link = `etsSeoAdmin.highlightBadSentences(${id_lang})`;
          ETS_SEO_DEFINED.readability_rules.sentence_length.error.short_code['[link_js]'].show = false;
        }
        etsSeo.getAnalysisMessage(
            '#analysis-result--list-readablity-' + id_lang,
  ...
  initTabSeo: function(id_lang) {
    const runAnalysis = Boolean(!ETS_SEO_SCORE_DATA || !ETS_SEO_SCORE_DATA[id_lang] || !ETS_SEO_SCORE_DATA[id_lang].score_analysis || !ETS_SEO_SCORE_DATA[id_lang].content_analysis);
    ...
    if (ETS_SEO_ENABLE_AUTO_ANALYSIS && runAnalysis) {
      ...
      if (id_lang == etsSeoBo.currentActiveLangId) {
      ...
      if (ETS_SEO_SCORE_DATA) {
        ...
        if (typeof ETS_SEO_SCORE_DATA[id_lang] !== 'undefined' && ETS_SEO_SCORE_DATA[id_lang].score_analysis) {
          if (typeof ETS_SEO_SCORE_DATA[id_lang].score_analysis === 'string') {
            scoreAnalysis = JSON.parse(ETS_SEO_SCORE_DATA[id_lang].score_analysis);
          } else {
            scoreAnalysis = ETS_SEO_SCORE_DATA[id_lang].score_analysis;
          }
        }
        ...
        if (typeof ETS_SEO_SCORE_DATA[id_lang] !== 'undefined' && ETS_SEO_SCORE_DATA[id_lang].content_analysis) {
          let contentAnalysis = ETS_SEO_SCORE_DATA[id_lang].content_analysis;
          ...
          if (contentAnalysis) {
            ...
            if (input.length) {
              ...
              currentData[id_lang] = contentAnalysis;
            ...
            [ssTypes, raTypes].forEach((v, i) => {
              for (const [key, val] of Object.entries(v)) {
                if (!val) {
                  $(`${i === 0 ? '#box-seo-analysis' : '.box-seo-readability'} .lang-${id_lang} .analysis-result--${key}`).hide();
                }
              }
            });
          }
  ...
  detectMissingAnalyzeByLang: function() {
    ...
    if (ETS_SEO_SCORE_DATA && this.missingAnalyzeByLang['length'] > 1) {
      Object.keys(ETS_SEO_SCORE_DATA).forEach((value) => this.missingAnalyzeByLang.remove(String(value)));
    }
  },
  timerSaveScore: null,
    ...
    this.idPreviewTimer = window.setTimeout(() => {
      ...
      Object.keys(ETS_SEO_LANGUAGES).forEach(function(isocode) {
        ...
        Object.keys(seo_analysis.seo_score).forEach(function(k) {
          if (!seo_analysis.seo_score[k].hasOwnProperty(curIdLang)) {
            skipProcess[curIdLang] = 1;
            return;
          }
          seo_score += parseInt(seo_analysis.seo_score[k][curIdLang]);
        });
        let readability_score = 0;
        Object.keys(seo_analysis.readability_score).forEach(function(k) {
          if (!seo_analysis.readability_score[k].hasOwnProperty(curIdLang)) {
            skipProcess[curIdLang] = 1;
            return;
          }
          readability_score += parseInt(seo_analysis.readability_score[k][curIdLang]);
        });
        if (Object.keys(skipProcess).indexOf(curIdLang) !== -1) {
          return;
        }
        const indexInput = $(`#ets_seo_allow_search_engine_show_post-${curIdLang}`);
        const allowSearchInput = $(`#ets_seo_allow_search_engine_show_post-${curIdLang}`);
        ...
        if (indexData == '1' || indexData == '0') {
          ...
          const configValue = allowSearchInput.attr('data-value');
        ...
        etsSeo.setDataPreviewAnalysis('seo-processing', curIdLang, seo_score, isIndex, keyPhrase, minorKeyPhrase);
        etsSeo.setDataPreviewAnalysis('readability-processing', curIdLang, readability_score, isIndex, keyPhrase, minorKeyPhrase);
      });
      const input = $('#ets_seo_content_analysis');
      let saveScore = false;
      if (input.length && input.val()) {
    ...
    if (etsSeo.activeControllers.indexOf(etsSeoBo.currentController) !== -1) {
      ...
      setTimeout(function() {
        if (!ETS_SEO_ENABLE_AUTO_ANALYSIS || ETS_SEO_SCORE_DATA) {
          if (ETS_SEO_SCORE_DATA && typeof missingAnalyzeByLang !== 'undefined') {
            Object.keys(ETS_SEO_SCORE_DATA).forEach((v) => {
              // noinspection EqualityComparisonWithCoercionJS
              if (v != etsSeoBo.currentActiveLangId) {
                etsSeo.initTabSeo(v);
              }
              etsSeo.missingAnalyzeByLang.remove(v);
            });
          }
