...
$(document).ready(function(){
   ...
   $(document).on('click','.checkbox input',function(){
    ...
    if(ybc_current_tab && $('.config_tab_'+ybc_current_tab).length)
    {
        $('.ybc-blog-tab-'+ybc_current_tab).addClass('active');
        $('.config_tab_'+ybc_current_tab).addClass('active');
    }
    else
    {
    ...
    $('.confi_tab').click(function(){
        if($(this).hasClass('config_tab_author'))
            return true;
    ...
    $('#exclude_product_autocomplete_input').autocomplete(ybc_blog_ajax_url,{
        minChars: 1,
        autoFill: true,
        max:20,
        matchContains: true,
    ...
    }).result(ybcAddExcludeProduct);
    $('#exclude_product_autocomplete_input').setOptions({
        extraParams: {
            excludeIds : ybcGetAccessoriesIds()
        }
    });
...
{
    if ($('#inputAccessories').val() === undefined && $('#exclude_products').val() === undefined)
			return '';
		return ($('#inputAccessories').val() !== undefined ? $('#inputAccessories').val().replace(/\-/g,','):'')+','+($('#exclude_products').val() !== undefined ? $('#exclude_products').val().replace(/\-/g,','):'');
}
...
var ybcAddAccessory = function(event, data, formatted)
{
    ...
    var productLink = data[3];
	var $divAccessories = $('#divAccessories');
	var $inputAccessories = $('#inputAccessories');
	var $nameAccessories = $('#nameAccessories');

	/* delete product from select + add product line to the div, input_name, input_ids elements */
	$divAccessories.html($divAccessories.html() + '<div class="form-control-static form-control-static_'+productId+'"><button type="button" onclick="ybcDelAccessory('+productId+');" class="btn btn-default remove_button" name="' + productId + '"><i class="icon-remove text-danger"></i></button>&nbsp;<img src="'+data[2]+'" style="width:32px;">&nbsp;<a href="'+productLink+'" target="_blank">'+ productName +'</a></div>');
...
var ybcAddExcludeProduct = function(event, data, formatted)
{
    if (data == null)
        return false;
    var productId = data[1];
    var productName = data[0];
    var productLink = data[3];
    var $divAccessories = $('#divexcludeproduct');
    var $inputAccessories = $('#exclude_products');
    var $nameAccessories = $('#name_exclude_products');

    /* delete product from select + add product line to the div, input_name, input_ids elements */
    $divAccessories.html($divAccessories.html() + '<div class="form-control-static form-control-static_'+productId+'"><button type="button" onclick="ybcDelExcludeProduct('+productId+');" class="btn btn-default remove_button" name="' + productId + '"><i class="icon-remove text-danger"></i></button>&nbsp;<img src="'+data[2]+'" style="width:32px;">&nbsp; <a href="'+productLink+'" target="_blank">'+ productName +'</a></div>');
    $nameAccessories.val($nameAccessories.val() + productName + '¤');
    $inputAccessories.val($inputAccessories.val() + productId + '-');
    $('#exclude_product_autocomplete_input').val('');
    $('#product_autocomplete_input').setOptions({
        extraParams: {excludeIds : ybcGetAccessoriesIds()}
    });
    $('#exclude_product_autocomplete_input').setOptions({
...
function ybcDelExcludeProduct(id)
{
    var div = getE('divexcludeproduct');
    var input = getE('exclude_products');
    var name = getE('name_exclude_products');
...
function displayFormNext()
{
    ...
    if($('input[name="YBC_BLOG_ENABLED_GPT"]').length)
    {
        if($('input[name="YBC_BLOG_ENABLED_GPT"]:checked').val()==1)
            $('.form-group.chatgpt').show();
        else
            $('.form-group.chatgpt').hide();
    }
