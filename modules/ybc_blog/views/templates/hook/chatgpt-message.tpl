...
*}
<li id="chatgpt-message-{$chatgpt_message->id|intval}" class="chatgpt-message{if $chatgpt_message->is_chatgpt} is_chatgpt{else} is_customer{/if}" data-id="{$chatgpt_message->id|intval}">
    <div class="chatgpt-content">
        {if $chatgpt_message->is_chatgpt}
            <i class="svg_icon">
                <svg width="20" height="20" xmlns="http://www.w3.org/2000/svg" shape-rendering="geometricPrecision" text-rendering="geometricPrecision" image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd" viewBox="0 0 512 512">
        ...
        {else}
            <i class="svg_icon" title="{l s='You' mod='ybc_blog'}">
        ...
        {/if}
        <p class="message-content" style="margin-bottom: 0;">{$chatgpt_message->message|nl2br nofilter}</p>
    </div>
    {if $chatgpt_message->is_chatgpt}
        <div class="chatgpt-button-append row">
            <div class="chatgpt-button-append-content">
                <label class="form-control-label col-lg-3">{l s='Apply content for' mod='ybc_blog'}</label>
                <div class="col-lg-6 chatgpt-button-select-lang">
                    <select name="content-apply-chatgpt" class="form-control">
                        <option value="title"{if $chatgpt_message->field=='title'} selected="selected"{/if}>{l s='Post title' mod='ybc_blog'}</option>
                        <option value="short_description"{if $chatgpt_message->field=='short_description'} selected="selected"{/if}>{l s='Short description' mod='ybc_blog'}</option>
                        <option value="description"{if $chatgpt_message->field=='description'} selected="selected"{/if}>{l s='Post content' mod='ybc_blog'}</option>
                        <option value="meta_title"{if $chatgpt_message->field=='meta_title'} selected="selected"{/if}>{l s='Meta title' mod='ybc_blog'}</option>
                        <option value="meta_description"{if $chatgpt_message->field=='meta_description'} selected="selected"{/if}>{l s='Meta description' mod='ybc_blog'}</option>
                    </select>
