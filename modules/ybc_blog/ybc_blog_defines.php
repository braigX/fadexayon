...
class Ybc_blog_defines extends Module
{
    ...
    protected static $subTabs;
    protected static $configs_global;
    protected static $configs_seo;
    protected static $configs_socials;
    protected static $configs_email;
    protected static $configs_homepage;
    protected static $configs_sidebar;
    protected static $configs_sitemap;
    protected static $configs_image;
    protected static $rss;
    protected static $customer_settings;
    protected static $configs_postlistpage;
    public $alias;
    public $friendly;
    public function getCustomerSettings()
    {
        if(!isset(self::$customer_settings))
        {
            self::$customer_settings = array(
        ...
        return self::$customer_settings;
    }
    public function getSubTabs(){
        if(!isset(self::$subTabs))
        {
            self::$subTabs = array(
                array(
                ...
                array(
                    'class_name' => 'AdminYbcBlogBackUp',
                    'tab_name' => $this->l('Import/Export','ybc_blog_defines'),
    ...
    public function getConfigSiteMap()
    {
        if(!isset(self::$configs_sitemap))
        {
            self::$configs_sitemap = array(
        ...
        return self::$configs_sitemap;
    }
    public function getConfigsImage()
    {
        if(!isset(self::$configs_image))
        {
            self::$configs_image = array(
        ...
        return self::$configs_image;
    }
    public function getConfigsGlobal(){
        if(!isset(self::$configs_global))
        {
            self::$configs_global = array(
                ...
                'YBC_BLOG_AUTO_LOAD' => array(
                    'label' => $this->l('Enable infinite scroll for blog listing pages','ybc_blog_defines'),
                    'type' => 'switch',
                    'default' => 1,
                    'values' => array(
                        array(
                ...
                'YBC_BLOG_DISABLE_SLICK_LIBRARY' =>array(
                    'label'=> $this->l('Disable Slick library','ybc_blog_defines'),
                    'type'=>'switch',
                    'tab' => 'general',
                    'desc' => $this->l('If your current theme already has the Slick library, you can disable the Slick library of BLOG module to avoid calling this library multiple times, therefore making page loading time increase. If your current theme does not have a Slick library, activating this option will cause an error.','ybc_blog_defines'),
                    'values' => array(
                        array(
                            'id' => 'YBC_BLOG_DISABLE_SLICK_LIBRARY_on',
                            'value' => 1,
                            'label' => $this->l('Yes','ybc_blog_defines')
                        ),
                        array(
                            'id' => 'YBC_BLOG_DISABLE_SLICK_LIBRARY_off',
                            'value' => 0,
                            'label' => $this->l('No','ybc_blog_defines')
                        )
                    ),
                ),
                'YBC_BLOG_ENABLED_GPT' =>array(
                    'label'=> $this->l('Enable ChatGPT for blog post','ybc_blog_defines'),
                    'type'=>'switch',
                    'tab' => 'chatgpt',
                    'default' => 0,
                    'values' => array(
                        array(
                            'id' => 'YBC_BLOG_ENABLED_GPT_on',
                            'value' => 1,
                            'label' => $this->l('Yes','ybc_blog_defines')
                        ),
                        array(
                            'id' => 'YBC_BLOG_ENABLED_GPT_off',
                            'value' => 0,
                            'label' => $this->l('No','ybc_blog_defines')
                        )
                    ),
                ),
                'YBC_BLOG_API_GPT' =>array(
                    'label'=> $this->l('API key','ybc_blog_defines'),
                    'type'=>'text',
                    'tab' => 'chatgpt',
                    'required' => (int)Tools::getValue('YBC_BLOG_ENABLED_GPT'),
                    'required2' => true,
                    'form_group_class' => 'chatgpt',
                    'validate' => 'isCleanHtml',
                    'desc' => Module::getInstanceByName('ybc_blog')->display(Module::getInstanceByName('ybc_blog')->getLocalPath(),'apichatgpt.tpl'),
                ),

            );
        }
        return self::$configs_global;
    }
    public function getConfigsRss()
    {
        if(!isset(self::$rss))
        {
            self::$rss = array(
    ...
    public function getConfigsHome()
    {
        if(!isset(self::$configs_homepage))
        {
            self::$configs_homepage = array(
        ...
        return self::$configs_homepage;
    }
    public function getConfigsPostListPage()
    {
        if(!isset(self::$configs_postlistpage))
        {
            self::$configs_postlistpage = array(
                ...
                'YBC_BLOG_DISPLAY_SORT_BY' => array(
                    'label' => $this->l("Display Sort by menu on the post listing page",'ybc_blog_defines'),
                    'type' => 'switch',
                    'default' => 1,
                    'values' => array(
                        array(
        ...
        return self::$configs_postlistpage;
    }
    protected static $configs_postpage;
    public function getConfigsPostPage()
    {
        if(!isset(self::$configs_postpage))
        {
            self::$configs_postpage = array(
        ...
        return self::$configs_postpage;
    }
    protected static $configs_categorypage;
    public function getConfigsCategoryPage()
    {
        if(!isset(self::$configs_categorypage))
        {
            self::$configs_categorypage = array(
        ...
        return self::$configs_categorypage;
    }
    protected static $configs_productpage;
    public function getConfigsProductPage()
    {
        if(!isset(self::$configs_productpage))
        {
            self::$configs_productpage = array(
        ...
        return self::$configs_productpage;
    }
    public function getConfigsSidebar()
    {
        if(!isset(self::$configs_sidebar))
        {
            self::$configs_sidebar = array(
        ...
        return self::$configs_sidebar;
    }
    public function getConfigsEmail()
    {
        if(!isset(self::$configs_email))
        {
            self::$configs_email = array(
        ...
        return self::$configs_email;
    }
    public function getConfigsSeo()
    {
        if(!isset(self::$configs_seo))
        {
            self::$configs_seo = array(
                ...
                'YBC_BLOG_ALIAS' => array(
                    ...
                    'type' => 'text',
                    'default' => 'blog',
                    'desc' => $this->l('Your blog main page:','ybc_blog_defines'). ' <a  href="'.$this->getLink().'" class="ybc-link-desc" target="_blank">'.$this->getLink().'</a><br/>'.$this->l('Copy this link and paste it to your top menu or somewhere in order to link the blog area with your website','ybc_blog_defines'),
                    'required' => true,
                    'lang'=>true,
                ),
        ...
        return self::$configs_seo;
    }
    public function getConfigsSocials()
    {
        if(!isset(self::$configs_socials))
        {
            self::$configs_socials = array(
    ...
    public static function getMaxId($tbl, $primaryKey)
    {
    ...
    public static function changeStatus($tbl, $field, $id , $status)
    {
    ...
    public static function getProducts($query,$excludeIds=false,$excludeVirtuals=false,$exclude_packs=false)
    {
    ...
    {
        $tbls = array('post', 'post_lang','post_shop','post_category', 'category','category_shop', 'category_lang', 'comment', 'gallery','gallery_shop', 'gallery_lang', 'tag', 'slide', 'slide_shop','slide_lang','employee','employee_lang','log_view','log_like','post_related_categories','reply','polls','email_template','chatgpt_message');
        foreach($tbls as $tbl)
        {
            Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'ybc_blog_'.pSQL($tbl).'`');
        }
        Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'ybc_chatgpt_template`');
        Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'ybc_chatgpt_template_lang`');
        return true;
    }
    ...
    public static function getInstance()
    {
        if (!isset(self::$instance)) {
            self::$instance = new Ybc_blog_defines();
        }
    ...
    public static function getGalleryField()
    {
        return array(
            array(
                'name' => 'id_gallery',
    ...
    public static function getSlideField()
    {
        return array(
            array(
                'name' => 'id_slide',
