...
class Ybc_Blog_ImportExport extends Module
{
    ...
    {
        $sql='SELECT c.* FROM `'._DB_PREFIX_.'ybc_blog_category` c,`'._DB_PREFIX_.'ybc_blog_post_category` pc 
	...
	{		
        ...
        $xml_output .= $this->exportXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsGlobal());
        $xml_output .= $this->exportXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsSeo());
        $xml_output .= $this->exportXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigSiteMap());
        $xml_output .= $this->exportXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsHome());
        $xml_output .= $this->exportXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsCategoryPage());
        $xml_output .= $this->exportXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsProductPage());
        $xml_output .= $this->exportXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsSidebar());
        $xml_output .= $this->exportXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsEmail());
        $xml_output .= $this->exportXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsSocials());
        $xml_output .= $this->exportXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsRss());
        $xml_output .= $this->exportXmlConfiguration(Ybc_blog_defines::getInstance()->getCustomerSettings());
        $xml_output .= $this->exportXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsImage());
        $xml_output .='</configuration>'."\n";
		$xml_output .= '</entity_profile>'."\n";
    ...
    public function processImport($zipfile = false,$params = array())
    {
        ...
        if(isset($params['data_import']) && $params['data_import'])
        {
            ...
            if(!$errors)
            {
                if(!Tools::ZipExtract($extractUrl, _YBC_BLOG_CACHE_DIR_))
		...
		{	
            ...
            if(isset($params['data_import']) &&  ($data_imports= $params['data_import']) && is_array($data_imports))
            {
                ...
                if(in_array('posts_categories',$data_imports))
                {
                    ...
                    if(isset($xml->post) && $xml->post)
                    {
                        foreach ($xml->post as $post_xml)
                		{
                            ...
                            $post->datetime_added = Validate::isDate((string)$post_xml['datetime_added']) ? (string)$post_xml['datetime_added'] :date('Y-m-d H:i:s');
                            $post->datetime_modified = Validate::isDate((string)$post_xml['datetime_modified']) ? (string)$post_xml['datetime_modified']: date('Y-m-d H:i:s');
                            if($keepauthorid)
                            {
                                $ok=false;
                            ...
                            $post->datetime_active = Validate::isDate((string)$post_xml['datetime_active']) ? (string)$post_xml['datetime_active'] : date('Y-m-d H:i:s');
                                ...
                                if(isset($post_xml->tags)&& $post_xml->tags)
                                {
                                    ...
                                    foreach($post_xml->tags as $tag_xml)
                                    {
                                        if((string)$tag_xml['iso_code'])
                                        {
                                            ...
                                            $tag = Validate::isCleanHtml((string)$tag_xml['tag']) ? (string)$tag_xml['tag'] :'';
                                            ...
                                            if($comment->subject && $comment->name && $comment->comment)
                                            {
                                                ...
                                                if($comment->id && isset($comment_xml->reply) && $comment_xml->reply)
                                                {
                                                    foreach($comment_xml->reply as $reply_xml)
                                                    {
                                                        ...
                                                        $sql= "INSERT INTO `"._DB_PREFIX_."ybc_blog_reply`(id_comment,id_user,name,email,reply,id_employee,approved,datetime_added,datetime_updated) values('".(int)$comment->id."', '".(int)$reply_xml['id_user']."','".pSQL($name)."','".pSQL($email)."','".pSQL($reply_text)."','".(int)$reply_xml['id_employee']."','".(int)$reply_xml['approved']."','".pSQL($date_add)."','".pSQL($date_upd)."')";
                                ...
                                if(in_array('posts_polls',$data_imports))
                                {
                                    if(isset($post_xml->post_polls) && $post_xml->post_polls)
                                    {
                                        ...
                                        foreach($post_xml->post_polls as $post_polls_xml)
                                        {
                                            ...
                                            $polls_class->name = Validate::isDate((string)$post_polls_xml['name']) ? (string)$post_polls_xml['name']:'';
                                            $polls_class->email = Validate::isEmail((string)$post_polls_xml['email']) ? (string)$post_polls_xml['email'] :'';
                                            $polls_class->id_post = (int)$post->id;
                                            $polls_class->feedback = Validate::isCleanHtml((string)$post_polls_xml->post_polls_text) ? (string)$post_polls_xml->post_polls_text :'';
                                            $polls_class->dateadd = Validate::isDate((string)$post_polls_xml['dateadd']) ? (string)$post_polls_xml['dateadd'] : date('Y-m-d H:i:s');
                        ...
                        foreach ($xml->slide as $slide_xml)
                		{
                            ...
                            if($slide_xml->slidelanguage)
                                foreach($slide_xml->slidelanguage as $slidelanguage)
                                {
                                    if((string)$slidelanguage['iso_code'])
                                    {
                                        ...
                                        if($id_lang)
                                        {
                                            $slide->caption[$id_lang] = Validate::isCleanHtml((string)$slidelanguage->caption) ? (string)$slidelanguage->caption :'';
                                            $slide->url[$id_lang] = Validate::isCleanHtml((string)$slidelanguage->url) ? (string)$slidelanguage->url :'';
                                            if(isset($slidelanguage->image) && self::isImage((string)$slidelanguage->image))
                                            {
                            ...
                            if(isset($languageSlideDefault))
                            {
                                foreach(Language::getLanguages(false) as $lang)
                                {
                                    ...
                                    {
                                        $slide->caption[$lang['id_lang']] = Validate::isCleanHtml((string)$languageSlideDefault->caption) ? (string)$languageSlideDefault->caption :'';
                                        $slide->url[$lang['id_lang']] = Validate::isCleanHtml((string)$languageSlideDefault->url) ? (string)$languageSlideDefault->url :'';
                                        if(isset($languageSlideDefault->image) && self::isImage((string)$languageSlideDefault->image))
                ...
                if(in_array('gallery_images',$data_imports))
                {
                    if(isset($xml->gallery)&& $xml->gallery)
                    {
                        foreach($xml->gallery as $gallery_xml)
                        {
                            ...
                            if(isset($gallery_xml['thumb']) && self::isImage((string)$gallery_xml['thumb']))
                            {
                                foreach($languages as $language)
                            ...
                            if($gallery_xml->gallerylanguage)
                            {
                                foreach($gallery_xml->gallerylanguage as $gallerylanguage)
                                {
                                    if((string)$gallerylanguage['iso_code'])
                                    {
                                        ...
                                        if($id_lang)
                                        {
                                            $gallery->title[$id_lang] = Validate::isCleanHtml((string)$gallerylanguage->title) ? (string)$gallerylanguage->title :'';
                                            $gallery->description[$id_lang] = Validate::isCleanHtml((string)$gallerylanguage->description,true) ? (string)$gallerylanguage->description :'';
                                            if(isset($gallerylanguage->image) && self::isImage((string)$gallerylanguage->image))
                                                $gallery->image[$id_lang] = (string)$gallerylanguage->image;
                                            if(isset($gallerylanguage->thumb) && self::isImage((string)$gallerylanguage->thumb))
                            ...
                            if(isset($languageGalleryDefault))
                            {
                                foreach(Language::getLanguages(false) as $lang)
                                {
                                    ...
                                    {
                                        $gallery->title[$lang['id_lang']] = Validate::isCleanHtml((string)$languageGalleryDefault->title) ? (string)$languageGalleryDefault->title :'';
                                        $gallery->description[$lang['id_lang']] = Validate::isCleanHtml((string)$languageGalleryDefault->description,true) ? (string)$languageGalleryDefault->description :'';
                                        if(isset($languageGalleryDefault->image) && self::isImage((string)$languageGalleryDefault->image))
                                            $gallery->image[$lang['id_lang']] = (string)$languageGalleryDefault->image;
                                        if(isset($languageGalleryDefault->thumb) && self::isImage((string)$languageGalleryDefault->thumb))
                ...
                if(in_array('module_configuration',$data_imports))
                {
                    ...
                    {
                        $this->importXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsGlobal(),$config_xml);
                        $this->importXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsSeo(),$config_xml);
                        $this->importXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigSiteMap(),$config_xml);
                        $this->importXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsHome(),$config_xml);
                        $this->importXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsCategoryPage(),$config_xml);
                        $this->importXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsProductPage(),$config_xml);
                        $this->importXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsSidebar(),$config_xml);
                        $this->importXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsEmail(),$config_xml);
                        $this->importXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsSocials(),$config_xml);
                        $this->importXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsRss(),$config_xml);
                        $this->importXmlConfiguration(Ybc_blog_defines::getInstance()->getCustomerSettings(),$config_xml);
                        $this->importXmlConfiguration(Ybc_blog_defines::getInstance()->getConfigsImage(),$config_xml);
                    }
                }
            }
        }
