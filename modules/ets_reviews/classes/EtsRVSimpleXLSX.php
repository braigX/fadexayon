...
class EtsRVSimpleXLSX
{
    ...
    protected function _unzip($filename, $is_data = false)
    {
        ...
        if ($is_data) {
            ...
            // Read file
            $vZ = Tools::file_get_contents($filename);
        }
    ...
    public function getEntryXML($name)
    {
        if ($entry_xml = $this->getEntryData($name)) {
            ...
            if (LIBXML_VERSION < 20900 && PHP_VERSION_ID < 80000 && function_exists('libxml_disable_entity_loader')) {
                $_old = call_user_func('libxml_disable_entity_loader');
            }
            ...
            if (LIBXML_VERSION < 20900 && PHP_VERSION_ID < 80000 && function_exists('libxml_disable_entity_loader')) {
                ...
                call_user_func('libxml_disable_entity_loader', $_old);
            }
    ...
    */
    ...
    {
        $s = EtsRVTools::htmlOpenTag('table', ['class' => 'excel']);
        foreach ($this->readRows($worksheetIndex) as $r) {
            $s .= EtsRVTools::htmlOpenTag('tr');
            foreach ($r as $c) {
                $s .= EtsRVTools::displayText(($c === '' ? '&nbsp' : htmlspecialchars($c, ENT_QUOTES)), 'td', ['nowrap' => null]);
            }
            $s .= EtsRVTools::htmlCloseTag('tr') . "\r\n";
        }
        $s .= EtsRVTools::htmlCloseTag('table');
