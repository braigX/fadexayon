        ...
        public function validateSampleCart($ipa, $idProduct)
{
    ...
    if ($idProduct) {
        ...
        $product = new Product($idProduct);
        $productPrice = $product->price; // This is always HT (tax excluded)
        ...
        $finalPriceHT = $productPrice; // Start with original HT price
        if ($sample['price_type'] == 2) { // Deduct fixed amount from product price
            $amountHT = $sample['amount'];
            // If deduction amount is TTC, convert to HT (uncomment/adjust if needed)
            // if ($sample['price_tax']) { $amountHT = $this->getTaxIncludedReduction($idTaxRulesGroup, $sample['amount'], false); }
            $finalPriceHT = (float) $productPrice - (float) $amountHT;
        } elseif ($sample['price_type'] == 3) { // Deduct percentage of price from product price
            // Percentage is typically applied on HT, no tax conversion needed
            $finalPriceHT = (float) $productPrice - (float) $productPrice * (float) $sample['amount'] / 100;
        } elseif ($sample['price_type'] == 4) { // Custom Price - your fixed price case
            $finalPriceHT = (float) $sample['price'];
            // If custom price is TTC (tax included), convert to HT
            if ((int) $sample['price_tax']) {
                $finalPriceHT = $this->getTaxIncludedReduction($idTaxRulesGroup, $finalPriceHT, false);
            }
        } elseif ($sample['price_type'] == 5) {
            $finalPriceHT = 0;
        } else { // price is original as product price
        ...
        $specificPrice->from_quantity = (int) 1;
        $specificPrice->reduction_tax = (int) $sample['price_tax']; // 1 if TTC
        $specificPrice->id_customer = (int) $context->customer->id;
        $specificPrice->reduction = 0;
        $specificPrice->from = pSQL('0000-00-00 00:00:00');
        $specificPrice->to = pSQL('0000-00-00 00:00:00');
