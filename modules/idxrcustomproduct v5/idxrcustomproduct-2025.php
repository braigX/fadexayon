...
class IdxrCustomProduct extends Module
{
    ...
    public function fetchLatestExtraConfig() {
        ...
        $sql = 'SELECT `prix_de_decoupe`, `demensions`, `prix_de_collage`, `prix_fixe`, 
        ...
        if (Tools::isSubmit('submitModConfiguration')) {
            ...
            if ($exists > 0) {
                ...
                $sqlInsert = 'INSERT INTO `ps_idxrcustomproduct_extra_config`
                              (`id`, `prix_de_decoupe`, `prix_de_collage`, `demensions`, `prix_fixe`, 
                               `cut_price_4mm`, `cut_price_5mm`, `cut_price_6mm`, `cut_price_8mm`, `cut_price_10mm`,
                               `glue_price_4mm`, `glue_price_5mm`, `glue_price_6mm`, `glue_price_8mm`, `glue_price_10mm`,
                               `polish_price_4mm`, `polish_price_5mm`, `polish_price_6mm`, `polish_price_8mm`, `polish_price_10mm`)
                              VALUES (1, ' . (float) $prixdepecoupe . ', ' . (float) $prixdecollage . ', 
                                      "' . pSQL($maximumdememnsions) . '", ' . (float) $prix_fixe . ',
    ...
    public function displayCustomization($params, $hook = '')
    {
        ...
        if ($id_configuration) {
            ...
            if ($steps['add_base']) {
                $base_price = Product::getPriceStatic($id_product, true, false,6,null,false,false);//without reductions);
                $base_price_wot = Product::getPriceStatic($id_product, false, false,6,null,false,false);//without reductions);
            } else {
                $base_price = 0;
                $base_price_wot = 0;
            }
                ...
                foreach ($notes_array as &$line) {
                    if (stripos($line, 'Console') !== false) {
                        $line = '';
                            ...
                            } else if (stripos($line, 'Aperçu') !== false) {
                                $line='';
                            }else{
                            $colonPos = strpos($line, ':');
                            if ($colonPos !== false) {
                ...
                foreach ($extra_options as $extra) {
                    if ($extra['id_product'] == $product['id_product']) {
                        ...
                        if ($extra['original_name']) {
                            $data['customization'] .= '<tr><td>' . $extra['title'] . ':</td><td>'.(isset($extra['qty'])?$extra['qty'].' ':'').'<a href="' . $file_controller . $extra['target_name'] . '" target="_blank">' . $extra['original_name'] . '</a></td></tr>';
                        } else {
                            ...
                            /*Add with team wassim novatis*/
                           // $words = explode(' ', $extra['title']);
                           // $firstWord = isset($words[0]) ? $words[0] : ''; // Obtient le premier mot de la chaîne                    
                           // $data['customization'] .= '<tr><td>' . $extra['title'] . ':</td><td  class="' . $firstWord . '">' . $extra['extra'] . '</td></tr>';
                            /*End*/
                        }
                    }
                }
                    ...
                    if ((float)$data['product_depth'] != 0) {
                        $data['customization'] .= '<tr><td>Épaisseur:</td><td id="depth"><span id="product_depth_value">' . (float)$data['product_depth'] . ' mm</span></td></tr>';
                    } 
                if ($quantity > 1) {
