...
$(document).ready(function() {
	...
	// Your allowed product IDs (as numbers or strings)
	  const allowedProductIds = [15897, 18897];

	  // Get current product ID from hidden input
	  const currentProductId = +$('#product_page_product_id').val();

	  // Check if current ID is NOT in the allowed list
	  if (!allowedProductIds.includes(currentProductId)) {
		$('.product-variants').remove();
	  }
	console.log('we are in caches..');
    if(es17){
