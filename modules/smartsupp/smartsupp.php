...
class Smartsupp extends Module
{
    ...
    protected function getSmartsuppJs($smartsupp_key)
    {
        ...
        */
        ...
        // 1) Récupère et nettoie le champ Optional API
        $optional_api = trim(Configuration::get('SMARTSUPP_OPTIONAL_API'));

        // 2) Initialise vide par défaut
        $custom_code = '';

        // 3) Ajoute un seul <script> si le contenu ressemble à une instruction JS
        if ($optional_api !== '' && preg_match('/[a-z_]\w*\s*\(/i', $optional_api)) {
            $custom_code = '<script>' . $optional_api . '</script>';
        }

        // 4) Retourne le code principal Smartsupp + éventuel bloc optionnel
        return $chat->render() . $custom_code;

        // // $custom_code = '<script>';
        // // $custom_code .= trim(Configuration::get('SMARTSUPP_OPTIONAL_API'));
        // $optional_api = trim(Configuration::get('SMARTSUPP_OPTIONAL_API'));
        // if (!empty($optional_api) && preg_match('/[a-z]+\s*\(/i', $optional_api)) {
        //     $custom_code = '<script>' . $optional_api . '</script>';
        // } else {
        //     $custom_code = ''; // Ignore invalid/malformed entries
        // }
        // // $custom_code .= '</script>';
    ...
    public function hookBackOfficeHeader()
    {
        ...
        if (strcmp(Tools::getValue('configure'), $this->name) === 0) {
            $path = $this->_path;
            $js .= '<script src="' . $path . 'views/js/smartsupp.js"></script>';
