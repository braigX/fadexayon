...
class ProductController extends ProductControllerCore
{
    public function init()
    {
        ...
        if (Module::isEnabled('seointernallinking') == true) {
            ...
            if (isset($productFields) && $productFields) {
                foreach ($productFields as $field) {
                    if (isset($this->product->{$field}) && !empty($this->product->{$field})) {
                        $this->product->{$field} = $module->getFilterContent($page_name, $this->product->{$field});
                    }
                }
            }
        }
