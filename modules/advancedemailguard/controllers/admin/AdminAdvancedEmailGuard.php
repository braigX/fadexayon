...
class AdminAdvancedEmailGuardController extends ModuleAdminController
{
    ...
    protected function getCommonVars()
    {
        $shared = array(
            ...
            'username' => $this->context->employee->firstname,
    ...
    {
        if (!filter_var(ini_get('allow_url_fopen'), FILTER_VALIDATE_BOOLEAN) && !extension_loaded('curl')) {
            $msg = $this->module->l(
                'The allow_url_fopen option or CURL extension must be enabled for reCAPTCHA validations to work.'
            ) . ' ' . $this->module->l(
                'Please edit your server\'s PHP configuration or contact your hosting provider.'
            );
            $this->addWarning($msg);
        }
    }
    ...
    protected function processPostRequest()
    {
        switch ($this->action) {
            ...
            case 'forms.settings':
                ...
                break;
            case 'forms.previewMode':
                $this->processFormsPreviewModeRequest();
                break;
            case 'logs.settings':
    ...
    /**
     * Process forms preview mode form request.
     *
     * @return void
     */
    protected function processFormsPreviewModeRequest()
    {
        if ($this->module->isDemo()) {
            return;
        }

        $this->setFormsPreviewModeRequestData();
    ...
    /**
     * Process forms preview mode request data.
     *
     * @return void
     */
    protected function setFormsPreviewModeRequestData()
    {
        foreach ($this->configs['forms.previewMode'] as $key) {
            $value = Tools::getValue($key);
