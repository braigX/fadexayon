...
class PrestaHelper {
	...
	public $headers, $body;
	public static $id_content_global,$id_content_primary_global,$id_editor_global, $hook_current, $id_lang_global, $id_shop_global, $disable_activity, $current_error, $current_url, $isprdlayout, $is_edit_mode=false, $is_preview_mode=false, $sitebuilder_ob = array('hbuilder'=>0,'fbuilder'=>0,'fzfbuilder'=>0,), $cart_obj = array();
	private static $licence_url    = 'https://classydevs.com/';
	private static $item_id        = '7231';
	...
	public static function getAjaxUrl() {
		...
		}elseif($http_build_query['hook']=='prdlayouts'){
			if (isset($_GET['prdid'])) {
				$str = (int)$_GET['prdid'];
				$ajaxUrlFinal .= '&prdid='.$_GET['prdid'];
			}
		}
	...
	public static function set_preview_mode(){
		if ( isset( $_GET['elementor-preview'] ) ) {
			PrestaHelper::$is_preview_mode = true;
		}
	}

	public static function set_edit_mode(){
		$actions = array(
			'elementor',
			// Templates
		...
		if ( isset( $_REQUEST['action'] ) && in_array( $_REQUEST['action'], $actions ) ) {
			PrestaHelper::$is_edit_mode = true;
		}
	}
	...
	public static function get_lience_expired_date() {
		...
		$lcdata = self::get_option( 'ce_licence_dataset' );
		$lcdata = (array) json_decode( $lcdata );
		if(isset($lcdata) && !empty($lcdata)){
			if(isset($lcdata['item_id'])){
				if(self::$item_id != $lcdata['item_id']){
					self::refresh_licence($ce_licence);
				}
			}else{
				self::refresh_licence($ce_licence);
			}
	...
	public static function get_lience( $license_data ) {
		...
		if ( $license_data ) {
			//raz0r
			$responsearray = ['success' => 'true', 'license' => 'valid', 'expires' => '2040-10-10'];
	...
	public static function refresh_licence( $license_data ) {
		$array = array(
		...
		if ( $license_data ) {
			...
			if ( $responsearray['success'] == true ) {
				...
				self::update_option( 'ce_licence', '' );
				self::update_option( self::$licence_status, '' );
				self::update_option( self::$licence_data, '' );
				self::update_option( self::$licence_ex, '' );
				self::update_option( 'ce_licence_date', '' );
			}
		}
	}

	public static function deactivated_licence( $license_data ) {
		$array = array(
	...
	public static function crazy_body_classes($defaults, $layout){
		$defaults['crazyelements-pro-default'] = 1;
		...
		if(self::$hook_current == 'page' || self::$hook_current == 'cms' || self::$hook_current == 'product'){
	...
	public static function setPreviewForHook( $hook ) {
		...
		if ( $hook == 'displayLeftColumn' ) {
		...
		} elseif ( $hook == 'displayFooterProduct' ) {
			$table_name = _DB_PREFIX_ . 'product';
			$results    = Db::getInstance()->executeS( "SELECT * FROM $table_name WHERE state = 1 AND active = 1 AND visibility = 'both'  ORDER BY id_product DESC " );
			$id_product = $results[0]['id_product'];
			$preview    = $context->link->getProductLink( $id_product, null, null, null, $id_lang );
		} elseif ( $hook == 'displayShoppingCart' ) {
			$preview = $context->link->getPageLink( 'cart', null, $id_lang, null, false, null, true );
		} elseif ( $hook == 'displayFooterCategory' ) {
			$table_name  = _DB_PREFIX_ . 'category';
			$results    = Db::getInstance()->executeS( "SELECT * FROM $table_name WHERE active = 1 ORDER BY id_category DESC " );
			$id_category = $results[0]['id_category'];
			$preview    = $context->link->getCategoryLink( $id_category, null, null, null, $id_lang );
  		} elseif ( $hook == 'prdlayouts' ) {
			$id = Tools::getValue('id');
			$preview = Context::getContext()->link->getModuleLink( 'crazyelements', 'prdlayouts',  array('id_crazyprdlayouts' => $id,'hook' => 'prdlayouts') , null, self::$id_lang_global );
			if (isset($_GET['prdid'])) {
				$preview .= '&prdid='.$_GET['prdid'];
			}
  		} elseif ( $hook == 'elementor_library' ) {
			$id = Tools::getValue('id');
			$preview = Context::getContext()->link->getModuleLink( 'crazyelements', 'display',  array('elementor_library' => $id,'hook' => 'display') , null, self::$id_lang_global );
			...
			$custom_hooks = self::get_option( 'crazy_custom_hooks' );
			$custom_hooks = json_decode( $custom_hooks, true );
			if(isset($custom_hooks[$hook])){
				if($default_lang != $id_lang){
					$preview = self::get_base_url() . \MetaCore::getEquivalentUrlRewrite($id_lang,$default_lang,$custom_hooks[$hook]);
				}else{
					$preview = self::get_base_url() . $custom_hooks[$hook];
				}				
			}else{
...
<?php
	...
	public static function gct_optiom($option_name, $option_value = false){
	...
	public static function EditorAlternateUrl( $context ) {
		...
		$current_lang = Tools::getValue('id_lang');
		$curr_url = Tools::getHttpHost( true, true ) . Tools::htmlentitiesUTF8( str_replace( array( '\'', '\\' ), '', urldecode( $_SERVER['REQUEST_URI'] ) ) );
		foreach ( $languages as $lang ) {
			$pathToImage = Tools::getShopDomain(true) . _PS_IMG_ . 'l' . DIRECTORY_SEPARATOR . $lang['id_lang'] . '.jpg';
			$replaceble = 'id_lang='.$current_lang;
			$replacby = "id_lang=".$lang['id_lang'];
			$alternativeLangs[ $lang['language_code'] ]['name'] = $lang['name'];
			$alternativeLangs[ $lang['language_code'] ]['iso'] = $lang['iso_code'];
			$alternativeLangs[ $lang['language_code'] ]['lang_id'] = $lang['id_lang'];
			$alternativeLangs[ $lang['language_code'] ]['url']  = str_replace($replaceble, $replacby , $curr_url);
			$alternativeLangs[ $lang['language_code'] ]['thumb']  = $pathToImage;
		}
		$alternativeLangs['current_lang_url'] = $curr_url;
		$alternativeLangs['current_lang_id'] = $current_lang;
		return $alternativeLangs;
	}

	public static function get_allowed_controllers(){
		return array('AdminCrazySetting','AdminCrazyPrdlayouts','AdminCrazyFonts','AdminCrazyPseIcon','AdminCrazyExtendedmodules','AdminCrazyContent','AdminDashboard');
	}
	...
	public static function crazy_promo() {
		...
		if($ce_new_changelog != ''){
			$ce_new_changelog = json_decode( $ce_new_changelog, true );
...
<?php 
	...
	public static function get_template_content($id){
		...
		$results_frontend         = Db::getInstance()->executeS("SELECT * FROM $table_name,$table_name_lang WHERE $table_name.id_crazy_content = $id AND $table_name.id_crazy_content = $table_name_lang.id_crazy_content AND id_lang =1 AND id_content_type = 0");
		PrestaHelper::$hook_current = 'page';
		$content = 'No Content Found For ID : ' . $id;
	...
	public static function wp_remote_retrieve_response_code( $response ) {
		if ( PrestaHelper::is_wp_error( $response ) || ! isset( $response['response'] ) || ! is_array( $response['response'] ) ) {
			return '';
		}
	...
	public static  function esc_html_e( $value, $domain = 'elementor' ) {
		echo PrestaHelper::esc_html($value, $domain);
	}
