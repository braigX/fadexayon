...
class SteSitemapGenerator
{
    ...
    public function generateForShop($idShop, $basePath)
    {
        ...
        $urls = $this->db->executeS('
            SELECT loc, priority, changefreq 
    ...
    private function syncIndex($idShop, $idLang)
    {
        ...
        foreach ($products as $prod) {
            ...
            $this->upsertIndex($idShop, 'product', $prod['id_product'], $url, '0.9', 'daily');
        }
    ...
    private function upsertIndex($idShop, $type, $idEntity, $url, $priority, $freq)
    {
        ...
        if ($exists) {
            ...
            $this->db->execute('UPDATE ' . _DB_PREFIX_ . 'ste_seo_index SET loc = "'.pSQL($url).'", last_mod = NOW() WHERE id_seo_index = '.(int)$exists);
        } else {
            $this->db->execute('INSERT INTO ' . _DB_PREFIX_ . 'ste_seo_index (id_shop, entity_type, id_entity, loc, last_mod, changefreq, priority, is_indexable)
            VALUES ('.(int)$idShop.', "'.pSQL($type).'", '.(int)$idEntity.', "'.pSQL($url).'", NOW(), "'.pSQL($freq).'", '.(float)$priority.', 1)');
        }
    }
    ...
    {
        return $this->db->executeS('
            SELECT p.id_product, pl.link_rewrite, p.ean13, cl.link_rewrite as category
