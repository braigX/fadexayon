...
{block name="legend"}
  <div class="panel-heading">
    {if isset($input.image) && isset($input.title)}<img src="{$input.image|escape:'htmlall':'UTF-8'}" alt="{$input.title|escape:'html':'UTF-8'}" />{/if}
            ...
            <div class="col-lg-6">
              <table class="table table-bordered">
                ...
                <span class="title_box">
                  {l s='Check'  mod='quantityupdate'}
                </span>
                  </th>
                  <th>
                    <span class="id-box">
                     {l s='ID'  mod='quantityupdate'}
                    </span>
                  </th>
                  {if $input.search}
                    <th class="buttons-block">
                      <a href="#" id="show_checked" class="btn btn-default"><i class="icon-check-sign"></i> {l s='Show Checked'  mod='quantityupdate'}</a>
                      &nbsp;
                      <a href="#" id="show_all" class="btn btn-default"><i class="icon-check-empty"></i> {l s='Show All'  mod='quantityupdate'}</a>
                    </th>
                  {/if}
                  <th>
                    <span class="title_box">
                      {if $input.search}
                        <input type="text" class="search_checkbox_table" placeholder="{l s='search...'  mod='quantityupdate'}">
                      {/if}
                    </span>
                  </th>
                </tr>
                  ...
                  <tr>
                    ...
                    <td>
                      <label for="{$input.name|escape:'htmlall':'UTF-8'}_{$setings[$id]|escape:'htmlall':'UTF-8'}">
                          ...
                          {/if}
                          {if isset($all_setings['name2']) && $all_setings['name2']} {$setings[$all_setings['name2']]|escape:'htmlall':'UTF-8'}{/if}</label>
                    </td>
                  </tr>
                {/foreach}
                </tbody>
      ...
      <div class="col-lg-9 col-lg-offset-3">
        {foreach $input['values']['query'] as $field}
          <div class="checkbox {$input.class|escape:'htmlall':'UTF-8'}">
            <label>
              <input type="checkbox" {if isset($input['values']['checked'][$field[$input['values']['value']]])}checked="checked" {/if}  {if isset($field['disabled'])}disabled="disabled" {/if} name="{$input['values']['array']|escape:'htmlall':'UTF-8'}[{$field[$input['values']['value']]|escape:'htmlall':'UTF-8'}]" value="{$field[$input['values']['name']]|escape:'htmlall':'UTF-8'}">
              {if isset($field['disabled'])}<input type="hidden" name="{$input['values']['array']|escape:'htmlall':'UTF-8'}[{$field[$input['values']['value']]|escape:'htmlall':'UTF-8'}]" value="{$field[$input['values']['name']]|escape:'htmlall':'UTF-8'}" >{/if}
