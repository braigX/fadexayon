...
class NewsletterProFrontSubscriptionController extends NewsletterProController
{
    ...
    public function initContent()
    {
        ...
        $this->js_data = [
            ...
            'subscription_template_body_info' => [
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit($action)) {
            ...
            switch (Tools::getValue($action)) {
                ...
                case 'updateListOfInterest':
                    $this->display($this->updateListOfInterest(Tools::getValue('data'), Tools::getValue('date')));
                    break;
            }
        } elseif (Tools::isSubmit('getSubscriptionTemplateView')) {
            @ob_clean();
            @ob_end_clean();
    ...
    {
        if (!isset($id_lang) || false == $id_lang) {
    ...
    public function addNewListOfInterest($values)
    {
        ...
        $default_lang = pqnp_config('PS_LANG_DEFAULT');
        ...
        } elseif (empty($fields[$default_lang])) {
    ...
    private function fillMissingLangs($data)
    {
        ...
        foreach ($languages as $lang) {
            ...
            if (!array_key_exists($id_lang, $data) && array_key_exists($default_lang, $data)) {
                $data[$id_lang] = $data[$default_lang];
            }
        }
    ...
    public function getTemplateFields($template, $id_lang = null)
    {
        ...
        return [
            ...
            'content' => $this->fillMissingLangs($template->content),
            'subscribe_message' => $this->fillMissingLangs($template->subscribe_message),
            'email_subscribe_voucher_message' => $this->fillMissingLangs($template->email_subscribe_voucher_message),
            'email_subscribe_confirmation_message' => $this->fillMissingLangs($template->email_subscribe_confirmation_message),
            'email_unsubscribe_confirmation_message' => $this->fillMissingLangs($template->email_unsubscribe_confirmation_message),
            ...
            'render_loader' => $template->render_loader,
    ...
    public function getSubscriptionTemplateViewLink($id_template, $id_lang = null)
    {
        ...
        return $this->controller->getLink([
    ...
    public function getTemplatesDataGrid()
    {
        ...
        return NewsletterProTools::jsonEncode($templates);
    }
    ...
    public function deleteTemplatesDataGrid($id)
    {
        try {
            ...
            if (Validate::isLoadedObject($template)) {
                if ((int) $template->id == (int) pqnp_config_get('SUBSCRIPTION_CONTROLLER_TEMPLATE_ID')) {
                    pqnp_config('SUBSCRIPTION_CONTROLLER_TEMPLATE_ID', 0);
                }
                // if the template name is default, the template cannot be deleted
                if ('default' == $template->name) {
                    return 0;
                } elseif ($template->name == NewsletterProConfigurationShop::get('SUBSCRIPTION_TEMPLATE')) {
    ...
    public function ajaxSaveTemplate($id_template, $content, $id_lang = 0)
    {
        ...
        if (Validate::isLoadedObject($template)) {
            try {
                ...
                $template->email_unsubscribe_confirmation_message = $content['tinyEmailUnsubscribeConfirmationMessage'];

                $global_info = $template->getCssGlobalStyleInfo();

                if (false === @file_put_contents($global_info['dir_path_full'], $content['css_style_global'])) {
    ...
    public function ajaxSaveAsTemplate($name, $data, $id_lang = 0)
    {
        ...
        try {
            $idTemplate = array_key_exists('templateId', $data) ? (int) $data['templateId'] : 0;
            $content = $data['content'];
            $settings = $data['settings'];
            $sliders = $data['sliders'];

            $template = new NewsletterProSubscriptionTpl($idTemplate);
            ...
            if (false === @file_put_contents($global_info['dir_path_full'], $content['css_style_global'])) {
                ...
                if (!$template->add()) {
                    ...
                    pqnp_config('SUBSCRIPTION_TEMPLATE', $template->name);
    ...
    public function getSubscriptionTemplateBackup()
    {
        ...
        if (NewsletterPro::BACKUP_TYPE_XML == NewsletterPro::getBackupType()) {
            $index = 1;
    ...
    public function ajaxGetActiveTemplatesVouchersErrors()
    {
        ...
        return NewsletterProTools::jsonEncode($voucher_errors);
    }
    ...
    public function ajaxCreateBackupSubscriptionTemplate($name, $check_duplicate = true)
    {
        ...
        try {
            ...
            $required_config = NewsletterProConfigurationShop::getAllShopsConfiguration(['SUBSCRIPTION_TEMPLATE']);
    ...
    private function updateListOfInterest($data, $date)
    {
        $response = $this->response([
            'mgs' => '',
        ]);
        ...
        if (0 == trim(Tools::strlen($date))) {
            $customers = array_map(function ($row) {
                return (int) $row['id_customer'];
            }, Db::getInstance()->executeS('
				SELECT c.id_customer FROM `'._DB_PREFIX_.'customer` c
				WHERE c.`id_customer` NOT IN (SELECT loi.`id_customer` FROM `'._DB_PREFIX_.'newsletter_pro_customer_list_of_interests` loi) 
			'));
        } elseif (Validate::isDate($date)) {
            $customers = array_map(function ($row) {
                return (int) $row['id_customer'];
            }, Db::getInstance()->executeS('
				SELECT c.id_customer FROM `'._DB_PREFIX_.'customer` c
            ...
            $loi = Db::getInstance()->executeS('
				SELECT loi.`id_customer` FROM `'._DB_PREFIX_.'newsletter_pro_customer_list_of_interests` loi WHERE loi.date_add >= "'.pSQL($date).'"
			');
        ...
        foreach ($customers as $customerId) {
            $customerLoi = NewsletterProCustomerListOfInterests::getInstanceByCustomerId($customerId);
            $customerLoi->setCategories($data);
            $customerLoi->save();
        }

        if (0 == trim(Tools::strlen($date))) {
            $loi = Db::getInstance()->executeS('
				SELECT loi.`id_newsletter_pro_subscribers` FROM `'._DB_PREFIX_.'newsletter_pro_subscribers` loi WHERE loi.`list_of_interest` = ""
			');
        } elseif (Validate::isDate($date)) {
            $loi = Db::getInstance()->executeS('
				SELECT loi.`id_newsletter_pro_subscribers` FROM `'._DB_PREFIX_.'newsletter_pro_subscribers` loi WHERE loi.`date_add` >= "'.pSQL($date).'"
			');
        }

        foreach ($loi as $row) {
            $id = (int) $row['id_newsletter_pro_subscribers'];
            $subscriber = new NewsletterProSubscribers($id);
            if (Validate::isLoadedObject($subscriber)) {
                $subscriber->list_of_interest = trim(implode(',', $data), ',');
                $subscriber->update();
            }
        }
