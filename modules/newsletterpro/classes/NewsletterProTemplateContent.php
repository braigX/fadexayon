...
class NewsletterProTemplateContent
{
    ...
    private $variables_chimp = [];

    /**
     * @var NewsletterProTemplate
     */
    private $template;
    ...
    private $compare_type_functions = [
        'isset',
    ];

    private $compare_type_operations = [
        '==', '!=', '<=', '>=', '===',
    ];
    ...
    public function getRenderConditions()
    {
        ...
        foreach ($conditions as $id_lang => $conditions_array) {
            foreach ($conditions_array as $key => $condition) {
                ...
                if (preg_match($match_compare_functions, $compare, $compare_type_match)) {
                ...
                } elseif (preg_match($match_compare_operations, $compare, $compare_type_match)) {
                    ...
                    } elseif (preg_match('/^[A-Za-z]+\w+$/', $compare_rigt)) {
                ...
                if (self::COMPARE_TYPE_FUNCTION == $compare_type) {
                    ...
                    if (self::COMPARE_RIGHT_TYPE_STRING == $compare_rigt_type) {
                        $compare_rigt = (string) $compare_rigt;
                    } elseif (self::COMPARE_RIGHT_TYPE_VARIABLE == $compare_rigt_type) {
    ...
    public function render($content_type = NewsletterProTemplateContent::CONTENT_FULL)
    {
        ...
        foreach ($render as $id_lang => $content) {
            ...
            foreach ($this->variables as $variable_name => $value) {
                $this->variables[$variable_name] = preg_replace_callback('/\{(\w+)\}/', [$this, 'replaceVariables'], $value);
            }

            // render chimp variables, I've made this for mailchimp
            foreach ($this->variables_chimp as $variable_name => $value) {
                $this->variables_chimp[$variable_name] = preg_replace_callback('/\{(\w+)\}/', [$this, 'replaceVariables'], $value);
            }

            // replace variables
            $content = preg_replace_callback('/\{(\w+)\}/', [$this, 'replaceVariables'], $content);
            // replace chimp variables
            $content = preg_replace_callback('/\*\|(\w+)\|\*/', [$this, 'replaceChimpVariables'], $content);
        ...
        foreach ($render as $id_lang => $render_value) {
            ...
            if (isset($header_lang[$id_lang]['convertInlineCss'])) {
                if ('true' == $header_lang[$id_lang]['convertInlineCss']) {
                    $render[$id_lang] = NewsletterProCSStoInlineStyle::convert($render_value);
                } else {
                    // make sure the eomticons are displayed
                    $render[$id_lang] = $this->encodeEmoticons($render[$id_lang]);
                }
            } elseif ((bool) pqnp_config('CONVERT_CSS_TO_INLINE_STYLE')) {
                $render[$id_lang] = NewsletterProCSStoInlineStyle::convert($render_value);
            } else {
                // make sure the eomticons are displayed
                $render[$id_lang] = $this->encodeEmoticons($render[$id_lang]);
            ...
            if (isset($header_lang[$id_lang]['unsubscribe']) && 'true' === $header_lang[$id_lang]['unsubscribe']) {
                NewsletterProSubscriptionManager::newInstance()->unsubscribe($this->variables['email']);
            }
        }
    ...
    private function encodeEmoticons($data)
    {
        $current_encode = mb_detect_encoding($data, 'UTF-8, ISO-8859-1, ISO-8859-15, HTML-ENTITIES', true);
        $data = html_entity_decode($data, ENT_COMPAT | ENT_HTML401 | ENT_HTML5, $current_encode);

        return mb_convert_encoding($data, 'UTF-8', $current_encode);
    }
    ...
    public function getHeaders()
    {
        ...
        foreach ($content_lang as $id_lang => $html) {
            $header = [];

            if (preg_match('/<!-- start header -->[\s\S]*?<!--([\s\S]*?)-->[\s\S]*?<!-- end header -->/', $html, $match)) {
                $match = explode(';', $match[1]);
    ...
    private function replaceTemplateVariables($content)
    {
        ...
        if ($this->template instanceof NewsletterProTemplateHistory) {
            ...
            if (!Tools::isSubmit('viewInBrowser')) {
                $tracking_link = NewsletterProApi::getLink('openedEmail', [
                    'token' => $module->getTokenByIdHistory((int) $this->template->id_history),
                    'email' => (string) $this->variables['email'],
                ], false, true, null, (int) $this->template->user->id_lang, (int) $this->template->user->id_shop);

                $tracking_img = '<img style="display: none; margin: 0; padding: 0; float: left; position: absolute;" src="'.$tracking_link.'" alt="'.$module->l('open email').'" height="1" width="1" data-embed="0"/>';
