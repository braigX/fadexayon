...
class NewsletterProUpgrade
{
    ...
    /**
     * @var NewsletterProCookie
     */
    public static $warningCookie = null;
    ...
    {
        if (!isset(self::$warningCookie)) {
            self::$warningCookie = new NewsletterProCookie('pqnp_update_warning', time() + 108000);
        }

        return self::$warningCookie;
    }
    ...
    {
        $results = Db::getInstance()->executeS('SHOW INDEXES FROM `'._DB_PREFIX_.$table.'` WHERE `Key_name` = "PRIMARY"');

        return count($results) > 0;
    }

    public function setPrimaryKey($table, $query)
    {
        try {
            if ($this->tableExists($table) && !$this->hasPrimaryKey($table)) {
                $this->refreshDb();

                if (!Db::getInstance()->execute($query)) {
                    $this->addError(sprintf($this->module->l('Cannot set the primary key in the table "%s".'), $table));
    ...
    public function alter($table, $query)
    {
        try {
            if ($this->tableExists($table)) {
                $this->refreshDb();

                if (0 == Tools::strlen(trim($query))) {
                    return true;
                }
                ...
                if (!Db::getInstance()->execute($query)) {
                    $this->addError(sprintf($this->module->l('Cannot update the table "%s".'), $table));
    ...
    public function addColumnIfNotExists($table, $column, $query, $comma = true)
    {
        if ($this->columnExists($table, $column)) {
            return '';
        }

        return ' ADD COLUMN `'.$column.'` '.trim($query).($comma ? ',' : '');
    }
     ...
     */
    public function execute($fromVersion = null)
    {
        $files = $this->getUpgradeFiles($fromVersion);
     ...
     * @return array
     */
    public function getUpgradeFiles($fromVersion = null)
    {
        $files = [];
        ...
        foreach ($iterator as $key => $file) {
            if (preg_match('/^Upgrade-(?P<version>.*)\.php/', $file->getFilename(), $match)) {
                // This will include the current targeted version
                $equal = false;
                if (isset($fromVersion)) {
                    $equal = 0 == version_compare($match['version'], $version);
                }

                $grather = 1 == version_compare($match['version'], $version);
                if ($grather || $equal) {
    ...
    {
        return NewsletterProTools::getDbVersion();
    }

    public function updateDbVersion($version)
    {
        return Db::getInstance()->update('module', [
    ...
    public function updateTabName($new_name = 'Newsletter Pro')
    {
        if ($id = (int) Tab::getIdFromClassName($this->module->class_name)) {
            return Db::getInstance()->update('tab_lang', [
                'name' => pSQL($new_name),
            ], '`id_tab` = '.(int) $id);
        }
    ...
    {
        $this->updateConfiguration('SHOW_CLEAR_CACHE', 1);
    }

    public static function showWarningMessage($value)
    {
        if (!is_array($value)) {
            $value = [$value];
        }
    ...
    public static function clearWarningCookie()
    {
        $cookie = new NewsletterProCookie('pqnp_update_warning');
        $cookie->destroy();
    }

    public static function getWarningContent()
    {
        $cookie = self::getCookie();
        if (!$cookie->exists('content')) {
            return [];
