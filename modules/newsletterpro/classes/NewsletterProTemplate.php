...
class NewsletterProTemplate
{
    ...
    {
        if (!in_array(get_class($this), ['NewsletterProTemplateString', 'NewsletterProTemplateFile', 'NewsletterProTemplateHistory'])) {
    ...
    private function setPaths()
    {
        ...
        $this->dir_template_lang = [];
        $this->dir_template_lang_file = [];
    ...
    public function html($content_type = NewsletterProTemplateContent::CONTENT_FULL, $add_css = false)
    {
        ...
        if (NewsletterProTemplateContent::CONTENT_FULL == $content_type) {
            $content = self::htmlReplaceModuleUrl($content, 'body');
        } elseif (NewsletterProTemplateContent::CONTENT_BODY == $content_type) {
            $content = self::htmlReplaceModuleUrl($content);
        }
    ...
    public static function getModuleUrl()
    {
        return [
            'key' => 'module_url',
            'value' => Tools::getHttpHost(true).NewsletterPro::relplaceAdminLink(__PS_BASE_URI__).'modules/newsletterpro/',
        ];
    }

    /**
     * For the tinyMCE tempalte module_url display and edit the images
     * self::htmlReplaceModuleUrl($template->htmlInvert());.
     *
     * @param mixed $html
     *
     * @return mixed
     */
    public static function htmlReplaceModuleUrl($html, $target = null)
    {
        $moduleUrl = self::getModuleUrl();

        if (isset($target) && isset($html[$target])) {
            foreach ($html[$target] as $idLang => $content) {
                $html[$target][$idLang] = str_replace('{'.$moduleUrl['key'].'}', $moduleUrl['value'], $content);
            }
        } else {
            foreach ($html as $idLang => $content) {
                $html[$idLang] = str_replace('{'.$moduleUrl['key'].'}', $moduleUrl['value'], $content);
            }
        }
    ...
    public static function htmlRestoreModuleUrl($html)
    {
        $moduleUrl = self::getModuleUrl();

        foreach ($html as $idLang => $content) {
            $html[$idLang] = str_replace($moduleUrl['value'], '{'.$moduleUrl['key'].'}', $content);
        }
    ...
    public function renderMailChimp($content_type = NewsletterProTemplateContent::CONTENT_FULL, $user = null, $load_user_lang = false, $id_lang = null)
    {
        ...
        $unsubscribe_link_redirect = urldecode($context->link->getModuleLink($module->name, 'unsubscribe', [
            'email' => '{email}',
            'mc_token' => $mc_token_obj->token,
            'msg' => false,
        ], null, $this->user->id_lang, $this->user->id_shop));

        $subscribe_link = urldecode($context->link->getModuleLink($module->name, 'subscribe', [
            'email' => '{email}',
            'mc_token' => $mc_token_obj->token,
        ], null, $this->user->id_lang, $this->user->id_shop));

        $forward_link = urldecode($context->link->getModuleLink($module->name, 'forward', [
            'email' => '{email}',
            'mc_token' => $mc_token_obj->token,
        ], null, $this->user->id_lang, $this->user->id_shop));
        ...
        if ($this instanceof NewsletterProTemplateHistory) {
            $view_in_browser_link_share = Context::getContext()->link->getModuleLink(NewsletterProTools::module()->name, 'newsletter', [
                'token_tpl' => $module->getTokenByIdHistory((int) $this->id_history),
                'email' => '{email}',
            ], null, (int) $this->user->id_lang, (int) $this->user->id_shop);

            $view_in_browser_link = urldecode($view_in_browser_link_share);
    ...
    private function saveEn()
    {
        ...
        if (!array_key_exists($en_id, $this->dir_template_lang_file)) {
            if (isset($this->content->html[pqnp_config('PS_LANG_DEFAULT')])) {
                ...
                if (false === fwrite($handle, $en_html)) {
    ...
    public function save()
    {
        ...
        if (false === file_put_contents($this->css_path, $this->css)) {
    ...
    public function setContent($content, $restoreModuleUrl = true)
    {
        foreach ($content as $id_lang => $html) {
            $this->changeTitle([$id_lang => $html['title']]);
        ...
        if ($restoreModuleUrl) {
            $this->content->html = self::htmlRestoreModuleUrl($this->content->html);
        }
    }

    public static function buildContent($content, $restoreModuleUrl = true)
    {
        $html_content = [];
        ...
        if ($restoreModuleUrl) {
            $html_content = self::htmlRestoreModuleUrl($html_content);
        }
    ...
    {
        if (false === file_put_contents($this->global_css_path, $content)) {
    ...
    {
        return str_replace(['(', ')'], '', Tools::strtolower(preg_replace('/\s+|(%20)+/i', '_', $name)));
    }
