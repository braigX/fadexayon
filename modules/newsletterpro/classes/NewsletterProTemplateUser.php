...
class NewsletterProTemplateUser
{
    ...
    private $has_employee_customer_data = false;
    ...
    {
        return self::getTypesNameStatic();
    }

    public static function getTypesNameStatic()
    {
        return [
    ...
    public static function convertUserType($userType)
    {
        $types = self::getTypesNameStatic();
        if (!array_key_exists($userType, $types)) {
            throw new InvalidArgumentException('The user type is not valid.');
        }
    ...
    public function create($use_type = null, $id_lang = null)
    {
        ...
        $sql_registred = 'SELECT count(*) FROM `'._DB_PREFIX_.pSQL(NewsletterProDefaultNewsletterTable::getTableName()).'` WHERE `email` = "'.pSQL($this->email).'"';
        ...
        if (!isset($use_type)) {
            ...
            } elseif (!((bool) pqnp_config('SUBSCRIPTION_ACTIVE')) && NewsletterProDefaultNewsletterTable::getTableName() && Db::getInstance()->getValue($sql_registred)) {
                $type = self::USER_TYPE_REGISTRED;
            } elseif (((bool) pqnp_config('SUBSCRIPTION_ACTIVE')) && Db::getInstance()->getValue($sql_registred_np)) {
                $type = self::USER_TYPE_REGISTRED_NP;
            } elseif (Db::getInstance()->getValue($sql_added)) {
            ...
            case self::USER_TYPE_REGISTRED:
                ...
                $row = Db::getInstance()->getRow('
					SELECT 
						...
						l.`name` AS `language`
					FROM `'._DB_PREFIX_.NewsletterProDefaultNewsletterTable::getTableName().'` n
    ...
    private function createDefault($type, $id_lang_load = null)
    {
        ...
        $id_lang = (int) pqnp_config('PS_LANG_DEFAULT');
        $firstname = '';
        $lastname = '';
        $id = 0;
        ...
        switch ((int) $type) {
            ...
            case self::USER_TYPE_EMPLOYEE:
                ...
                if (isset($context->employee)) {
                    ...
                    // serach for customer in database the the email is from a customer
                    ...
                    if (Validate::isLoadedObject($customer)) {
                        ...
                        $id = $customer->id;
                        $this->has_employee_customer_data = true;
                    }
                } else {
                    if (!isset($this->email)) {
                        $this->email = pqnp_config('PS_SHOP_EMAIL');
                    }
                }
                break;
        ...
        if (!$row) {
            $row['name'] = pqnp_config('PS_SHOP_NAME');
        }

        $this->shop_name = $row['name'];
    }

    public function hasEmployeeCustomerData()
    {
        return $this->has_employee_customer_data;
    }
    ...
    private function initVariables()
    {
        ...
        $moduleUrl = NewsletterProTemplate::getModuleUrl();
        $this->module_url = $this->default_shop_url.'modules/newsletterpro/';
        $this->{$moduleUrl['key']} = $moduleUrl['value'];
        ...
        $this->unsubscribe_link = urldecode($context->link->getModuleLink($module->name, 'unsubscribe', [
            'email' => $this->email,
        ...
        $this->unsubscribe_link_redirect = urldecode($context->link->getModuleLink($module->name, 'unsubscribe', [
            'email' => $this->email,
            'u_token' => Tools::encrypt($this->email),
            'msg' => false,
        ], null, $this->id_lang, $this->id_shop));
        ...
        $this->subscribe_link = urldecode($context->link->getModuleLink($module->name, 'subscribe', [
            'email' => $this->email,
        ...
        $this->forward_link = urldecode($context->link->getModuleLink($module->name, 'forward', [
            'email' => $this->email,
            'token' => Tools::encrypt($this->email),
        ], null, $this->id_lang, $this->id_shop));
        ...
        if ($this->template instanceof NewsletterProTemplateHistory) {
            ...
            $this->view_in_browser_link = urldecode(Context::getContext()->link->getModuleLink(NewsletterProTools::module()->name, 'newsletter', [
                'token_tpl' => $module->getTokenByIdHistory((int) $this->template->id_history),
                'email' => $this->email,
            ]));
