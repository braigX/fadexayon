 ...
 * @author     Chris Corbyn
 */
class NewsletterPro_Swift_KeyCache_DiskKeyCache implements NewsletterPro_Swift_KeyCache
{
     ...
     *
     * @var NewsletterPro_Swift_KeyCache_KeyCacheInputStream
     */
    private $_stream;
     ...
     *
     * @param NewsletterPro_Swift_KeyCache_KeyCacheInputStream $stream
     * @param string                             $path   to save to
     */
    public function __construct(NewsletterPro_Swift_KeyCache_KeyCacheInputStream $stream, $path)
    {
        $this->_stream = $stream;
        $this->_path = $path;
     ...
     * @param int     $mode
     *
     * @throws NewsletterPro_Swift_IoException
     */
    public function setString($nsKey, $itemKey, $string, $mode)
    {
        ...
        switch ($mode) {
            ...
            case self::MODE_APPEND:
                ...
                break;
            default:
                throw new NewsletterPro_Swift_SwiftException(
     ...
     * @throws NewsletterPro_Swift_IoException
     */
    public function importFromByteStream($nsKey, $itemKey, NewsletterPro_Swift_OutputByteStream $os, $mode)
    {
        $this->_prepareCache($nsKey);
        switch ($mode) {
            case self::MODE_WRITE:
     ...
     * @param string                $nsKey
     * @param string                $itemKey
     * @param NewsletterPro_Swift_InputByteStream $writeThrough
     *
     * @return NewsletterPro_Swift_InputByteStream
     */
    public function getInputByteStream($nsKey, $itemKey, NewsletterPro_Swift_InputByteStream $writeThrough = null)
    {
     ...
     * @param string                $nsKey
     * @param string                $itemKey
     * @param NewsletterPro_Swift_InputByteStream $is      to write the data to
     */
    public function exportToByteStream($nsKey, $itemKey, NewsletterPro_Swift_InputByteStream $is)
    {
        if ($this->hasKey($nsKey, $itemKey)) {
            $fp = $this->_getHandle($nsKey, $itemKey, self::POSITION_START);
            if ($this->_quotes) {
    ...
    private function _prepareCache($nsKey)
    {
        ...
        if (!is_dir($cacheDir)) {
            if (!mkdir($cacheDir)) {
                throw new NewsletterPro_Swift_IoException('Failed to create cache directory '.$cacheDir);
            }
