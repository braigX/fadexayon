 ...
 * @author     Chris Corbyn
 */
class NewsletterPro_Swift_KeyCache_SimpleKeyCacheInputStream implements NewsletterPro_Swift_KeyCache_KeyCacheInputStream
{
     ...
     *
     * @param string                $bytes
     * @param NewsletterPro_Swift_InputByteStream $is    optional
     */
    public function write($bytes, NewsletterPro_Swift_InputByteStream $is = null)
    {
        $this->_keyCache->setString(
            $this->_nsKey,
            $this->_itemKey,
            $bytes,
            NewsletterPro_Swift_KeyCache::MODE_APPEND
        );
        if (isset($is)) {
            $is->write($bytes);
        }
