 ...
 */
class NewsletterPro_Swift_Signers_DomainKeySigner implements NewsletterPro_Swift_Signers_HeaderSigner
{
    /**
     * PrivateKey
     *
     ...
     *
     * @var NewsletterPro_Swift_Mime_Headers_ParameterizedHeader
     */
     ...
     * @param string $domainName
     * @param string $selector
     * @return NewsletterPro_Swift_Signers_DomainKeySigner
     */
     ...
     * @return Swift_Signers_DomainKeysSigner
     */
    public function bind(NewsletterPro_Swift_InputByteStream $is)
    {
     ...
     * @return Swift_Signers_DomainKeysSigner
     */
    public function unbind(NewsletterPro_Swift_InputByteStream $is)
    {
        // Don't have to mirror anything
            ...
            switch ($string[$i]) {
                ...
                case "\n":
                    if ($this->_bodyCanonLastChar == "\r") {
                        ...
                        // Wooops Error
                        throw new NewsletterPro_Swift_SwiftException('Invalid new line sequence in mail found \n without preceding \r');
                    }
                    break;
                case " ":
                case "\t":
    ...
    /**
     * @throws NewsletterPro_Swift_SwiftException
     * @return string
     */
    private function _getEncryptedHash()
    {
        ...
        if (!$pkeyId) {
            throw new NewsletterPro_Swift_SwiftException('Unable to load DomainKey Private Key ['.openssl_error_string().']');
        }
        if (openssl_sign($this->_canonData, $signature, $pkeyId, OPENSSL_ALGO_SHA1)) {
            return $signature;
        }
        throw new NewsletterPro_Swift_SwiftException('Unable to sign DomainKey Hash  ['.openssl_error_string().']');
    }
}
