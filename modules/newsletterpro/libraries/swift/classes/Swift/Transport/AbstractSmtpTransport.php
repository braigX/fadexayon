 ...
 * @author     Chris Corbyn
 */
abstract class NewsletterPro_Swift_Transport_AbstractSmtpTransport implements NewsletterPro_Swift_Transport
{
     ...
     *
     * @param NewsletterPro_Swift_Transport_IoBuffer     $buf
     * @param NewsletterPro_Swift_Events_EventDispatcher $dispatcher
     */
    public function __construct(NewsletterPro_Swift_Transport_IoBuffer $buf, NewsletterPro_Swift_Events_EventDispatcher $dispatcher)
    {
    ...
    /**
     * Set the name of the local domain which NewsletterPro_Swift will identify itself as.
     *
     ...
     * @param string $domain
     *
     * @return NewsletterPro_Swift_Transport_AbstractSmtpTransport
     */
    ...
    /**
     * Get the name of the domain NewsletterPro_Swift will identify as.
     *
     * @return string
     */
    ...
    public function start()
    {
        if (!$this->_started) {
            ...
            } catch (NewsletterPro_Swift_TransportException $e) {
     ...
     * @return int
     */
    public function send(NewsletterPro_Swift_Mime_Message $message, &$failedRecipients = null)
    {
        $sent = 0;
        ...
        if (!$reversePath = $this->_getReversePath($message)) {
            $this->_throwException(
                new NewsletterPro_Swift_TransportException(
        ...
        if ($evt) {
            if ($sent == count($to) + count($cc) + count($bcc)) {
                $evt->setResult(NewsletterPro_Swift_Events_SendEvent::RESULT_SUCCESS);
            } elseif ($sent > 0) {
                $evt->setResult(NewsletterPro_Swift_Events_SendEvent::RESULT_TENTATIVE);
            } else {
                $evt->setResult(NewsletterPro_Swift_Events_SendEvent::RESULT_FAILED);
            }
     ...
     * Register a plugin.
     *
     * @param NewsletterPro_Swift_Events_EventListener $plugin
     */
    public function registerPlugin(NewsletterPro_Swift_Events_EventListener $plugin)
    {
     ...
     *
     * @return NewsletterPro_Swift_Transport_IoBuffer
     */
    ...
    protected function _streamMessage(NewsletterPro_Swift_Mime_Message $message)
    {
    ...
    protected function _getReversePath(NewsletterPro_Swift_Mime_Message $message)
    {
    ...
    protected function _throwException(NewsletterPro_Swift_TransportException $e)
    {
    ...
    protected function _getFullResponse($seq)
    {
        ...
        try {
            ...
            $this->_throwException($e);
        } catch (NewsletterPro_Swift_IoException $e) {
            $this->_throwException(
                new NewsletterPro_Swift_TransportException(
                    $e->getMessage()
                )
    ...
    private function _sendTo(NewsletterPro_Swift_Mime_Message $message, $reversePath, array $to, array &$failedRecipients)
    {
        if (empty($to)) {
            return 0;
        }

        return $this->_doMailTransaction(
            $message,
            $reversePath,
            array_keys($to),
            $failedRecipients
    ...
    private function _sendBcc(NewsletterPro_Swift_Mime_Message $message, $reversePath, array $bcc, array &$failedRecipients)
    {
        $sent = 0;
