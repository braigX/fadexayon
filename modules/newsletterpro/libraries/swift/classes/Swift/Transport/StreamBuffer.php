 ...
 * @author     Chris Corbyn
 */
class NewsletterPro_Swift_Transport_StreamBuffer extends NewsletterPro_Swift_ByteStream_AbstractFilterableInputStream implements NewsletterPro_Swift_Transport_IoBuffer
{
    /** A primary socket */
    private $_stream;
     ...
     *
     * @param NewsletterPro_Swift_ReplacementFilterFactory $replacementFactory
     */
    public function __construct(NewsletterPro_Swift_ReplacementFilterFactory $replacementFactory)
    {
    ...
    public function readLine($sequence)
    {
        if (isset($this->_out) && !feof($this->_out)) {
            ...
            if (strlen($line) == 0) {
                ...
                if ($metas['timed_out']) {
                    throw new NewsletterPro_Swift_IoException(
                        'Connection to '.
    ...
    private function _establishSocketConnection()
    {
        ...
        // Modified
        // [SOLVE] Connection could not be established with host smtp.gmail.com [ #0]
        if (version_compare(PHP_VERSION, '5.6.0', '>=') && (int)Configuration::get('PS_SSL_ENABLED') == 0 && (int)pqnp_ini_config('swift_ssl_verify_peer') == 1) {
            $options['ssl'] = array(
