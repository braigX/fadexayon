...
{literal}
<script type="application/ld+json" id="adpmicrodatos-store-ps17v{/literal}{$module_version|escape:'htmlall':'UTF-8'}{literal}">
{
    ...
    "openingHoursSpecification": [
        {/literal}
            {assign var="addsemicolon" value=false}
            {foreach from=$tienda.hours item=hours name=hoursIterator}
                {if !empty($hours.opens)}
                    {if $addsemicolon},{/if}{literal}{ "@type": "OpeningHoursSpecification","dayOfWeek": "http://schema.org/{/literal}{$hours.day}{literal}","opens": "{/literal}{$hours.opens}{literal}","closes": "{/literal}{$hours.closes}{literal}" }{/literal}{assign var="addsemicolon" value=true}
                {/if}
            {/foreach}
        {literal}
    ],
