...
*}
<script type="application/ld+json" id="adpmicrodatos-itemlist-ps16v{$module_version}">
{
...
{if !empty($listado_producto.url)}
            "mainEntityOfPage": "{$listado_producto.url}",
            "url": "{$listado_producto.url}",
{/if}
{if !empty($listado_producto.nombre)}
            "name": {$listado_producto.nombre|@json_encode nofilter},
{/if}
{if !empty($listado_producto.imagen)}
            "image": "{$listado_producto.imagen}",
{/if}
